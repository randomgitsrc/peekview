...
v4 的派发协议把每次阶段推进翻译成**明确的工具调用 + 精确的输入输出规范**，解决早期版本「主 Agent 写入上下文」这类不可执行的模糊描述。
...
v4 的标准模式假设主 Agent 有 `task` 工具。若 `executor_env.has_task_tool: false`（如 Claude Project 会话），整个派发机制降级为**单 Agent 顺序执行模式**：
