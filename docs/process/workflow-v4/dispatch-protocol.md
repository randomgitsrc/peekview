...
- 调试/验证必须使用 debug_env 声明的环境，严禁直接操作 prod_env
- 任何涉及 prod_env 的写操作（包括意外触发）必须立即停止，标注 [PROD_TOUCHED] 报告主 Agent
...
- **数据删除**：批量删除、DROP TABLE、清空集合等
- **数据迁移**：生产数据的结构变更或搬移
- **生产环境写入**：直接写入生产 DB、文件系统、外部服务
- **不可逆的外部调用**：发送邮件/通知、扣费、第三方 API 写操作
