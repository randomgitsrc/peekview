...
0. 任务启动检查（仅首次，任务刚收到时）
   收到任务后，主 Agent 第一步输出只允许两种内容之一：
   a) 派发 P1 analyst 子代理（task 工具调用）
   b) 判断为微/小任务并声明「直接执行」的理由（一句话，含任务类型判断）

   任何其他输出（分析、建议方案、直接改代码）视为违规。
   —— T004 教训 B1：收到"CSP 拦了 CDN"后主 Agent 直接给出方案，绕过了 P1 需求质疑。
      表面明确的需求恰恰最需要 P1 质疑——"加一下就行" 常常不是根治方案。
...
**packages 动态注入（B4/B6）**：派发 P8 subagent 时，主 Agent 必须先读 P2-design.md 的 `packages:` 声明，把"需要 bump 哪些包"明确写进 prompt，并据此生成 gate 命令集（单包 `make pre-publish`；多包追加 `make test-mcp-unit` 等）。不能用固定的单包命令。
