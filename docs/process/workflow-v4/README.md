...
| 风险特征 | 不可跳过的阶段 | 原因 |
|---------|--------------|------|
| 涉及生产环境写操作（DB 写入、文件系统写入、外部服务调用）| P6 验收 | 需要人确认行为符合预期，防止误操作不可逆 |
| 涉及数据删除 / 数据迁移 | P6 验收 + `[NEED_CONFIRM]` 硬中断 | 不可逆操作，必须人工确认影响范围 |
...
|------|------|----------|----------|--------------------------|
| P0 | 任务简报 | **主 Agent 亲自写**（非 subagent）| — | P0-brief.md 完成，含 env_constraints + known_risks + irreversible |
