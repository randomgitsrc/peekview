...
> workflow-v3，解决"角色库只有评审角色、没有执行角色、不支持自定义"的问题

---

## 为什么分两层

v2 引用的 gstack 角色库全是**评审角色**（reviewer）——它们的职责是审查已有产出。但 P1-P7 流程里大部分阶段需要的是**执行者**（写问题定义、写设计、写代码），不是审查者。
...
负责各阶段的实际产出。每个对应 P1-P7 的某些阶段：

| 角色 | 文件 | 负责阶段 | 职责 |
|------|------|----------|------|
| 问题分析师 | analyst.md | P1 | 定义问题、写测试策略 |
| 方案设计师 | architect.md | P2、P6 | 写设计方案、一致性检查 |
| 测试设计师 | test-designer.md | P3 | 写测试用例和测试代码（TDD 红灯）|
| 实现工程师 | implementer.md | P4、P7 | 写代码、发布 |
| 验证工程师 | verifier.md | P5 | 逐项验证、跑测试 |
...
角色定义：读取 docs/process/workflow-v3/assets/review-roles/plan-eng-review.md
...
1. **自包含**：workflow-v3 不依赖外部文档，所有资产在 assets/ 下
