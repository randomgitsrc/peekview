...
> 验证日期：2026-06-12
> 验证者：OpenCode Agent
> 验证目的：验证 workflow-v3 的派发机制在 OpenCode 上是否可用
...
2. **注入方式**：prompt 里只写文件路径，不塞内容（如"读取 docs/process/workflow-v3/assets/execution-roles/architect.md"）
...
在派发 prompt 中统一使用：
```
角色定义：读取 docs/process/workflow-v3/assets/execution-roles/{role}.md
```
