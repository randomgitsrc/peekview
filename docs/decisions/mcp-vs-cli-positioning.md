...
3. **无额外延迟**：CLI 是同步调用；MCP 有 HTTP 连接、认证等开销
...
## 补充：架构约束下的工具边界（2026-05-24 更新）

### 部署拓扑决定工具能力

```
A=B→C（本地模式）：MCP Server 和 Agent 同机，路径可达
A��B→C（远程模式）：MCP Server 在远程，无法访问 Agent 本地文件
```

任何传给 MCP 工具的参数都经过 LLM 推理（上下文），因此：
- 远程模式下传大文件内容 → 上下文爆炸 → ~2 分钟延迟
- 这是 Agent LLM-in-the-loop 工具调用架构的根本限制（参数必须经过 LLM 上下文），不是传输协议或可绕过的实现问题

### 最终工具策略

| 模式 | 暴露工具 | 原因 |
|------|---------|------|
| local（A=B） | 只有 `publish_files` | MCP 读本地文件，Agent 只传路径 |
| remote（A≠B） | 只有 `create_entry` 等 | 只适合 Agent 生成的轻量内容 |

**两种模式工具不重叠，Agent 无需感知部署拓扑。**

详细设计见：`docs/specs/spec-mcp-local-remote-mode.md`
