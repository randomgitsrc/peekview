...
> 修订：2026-06-12 — 对照 v4 原文逐条校验，修正定性偏差、补充遗漏项
...
**综合评分：7.2/10** — 架构优秀，细节和文档需一轮打磨。

| 维度 | 评分 | 说明 |
|------|------|------|
| 架构设计 | 9 | 主从隔离 + 状态落盘 + SCOPE+ 反馈，设计优雅 |
| 可执行性 | 6 | P3 门槛依赖未创建的脚本，P3 跳过后 P4 门槛未定义，P7 门槛可空壳通过 |
| 文档一致性 | 5 | v3/v4 混标（8 处残留），P7/P8 小结触发点矛盾，已实现改进项未清理 |
| 实战适配 | 7 | 微任务已有适用边界豁免，BDD 质量无检查，并行依赖未定义 |
| 抗风险 | 7 | A1/C7/重试上限/硬中断点设计扎实，但连锁上溯、PAUSED 恢复、P8 gate 脱节有缺口 |
...
### S4. P7 一致性检查门槛晦涩且覆盖度不足

**位置**：`state-machine.md:90`

**问题 1**：`grep -L 'BLOCKER' P7-consistency.md 有输出` 逻辑本身正确（`-L` 打印不含匹配的文件名→有输出=无 BLOCKER→过门槛），但表达反直觉，容易误读为"有 BLOCKER 才过"。且 `grep -L` exit code 在无匹配时为 1，依赖"有输出"而非 exit code 判定易踩坑。

**建议**：改用 `! grep -q '\[BLOCKER\]' P7-consistency.md`，语义直观、exit code 可判定。
...
### S5. P8 gate 与 state-machine.md 脱节（本次新增）

**位置**：`state-machine.md:95` vs `dispatch-protocol.md:171`

state-machine.md P8 门槛只说"每个声明的 package 的发布检查命令 exit 0"，未指明命令是什么、从哪获取。dispatch-protocol.md 提到"由 P2 packages 动态生成"，但 state-machine 没引用这条规则。两文件脱节——主 Agent 读 state-machine 造 gate 命令时无据可依。

**建议**：state-machine.md P8 门槛补注"gate 命令集由 P2-design.md 的 packages: 声明动态生成，规则见 dispatch-protocol.md「packages 动态注入」节"。

### S6. `scripts/check-tdd-red.sh` 未实际创建（本次新增）

**位置**：`state-machine.md:129-158`

P3 门槛依赖 `scripts/check-tdd-red.sh exit 0`，文档中给出了完整脚本设计，但**仓库中未实际创建该文件**。主 Agent 跑 P3 gate 时命令不存在，门槛无法判定。这是 P3 的实操阻塞项。

**建议**：将脚本实际创建到 `scripts/check-tdd-red.sh`，或在 README 明确标注"P3 门槛脚本需项目自行创建，模板见 state-machine.md"。
...
| # | 位置 | 问题 |
|---|------|------|
| D1 | `validation-plan.md` 标题 | 写"Workflow v3 落地验证方案"，应为"v4" |
| D2 | `validation-report.md` 全篇 | 写"v3"，应为"v4" |
| D3 | `state-machine.md:11` | "所以 v3 不依赖 LLM 记住状态"，应为"v4" |
| D4 | `loop-orchestration.md:207` | "v3 当前严格串行派发"，应为"v4" |
| D5 | `dispatch-protocol.md:339` | 结尾写"v3"，应为"v4" |
| D6 | `state-machine.md` P5 L2 上溯 | P5 上溯到 P4，P4 上溯到 P2，组合等于跨多阶段回退，但"跨多阶段回退禁止自动"规则只拦截单跳，不拦截链式上溯（与 R3 交叉） |
| D7 | `loop-orchestration.md:210` | "评审角色选择的可判定化"列为未实现改进项，但 `role-system.md` 已实现 domains→评审角色机械映射 |
| D8 | `dispatch-protocol.md:293` vs `state-machine.md:108` | 前者说"P7 gate 通过、状态进入 READY 时"输出小结，后者是 P8 gate 通过后进 READY — 不一致，应为 P8 |
...
README.md:82 已定义适用边界："微任务（typo、文案、单行配置）→ 直接做，不走 v4"。但"小任务"裁剪到 P1+P4+P5 时，P1 仍需写 requirements.md（含 BDD + 裁剪说明），对一个明确 bug 修复仍有摩擦。

**建议**：小任务 P1 可降级为一句话基线（需求 + 验收条件各一句），不必完整六节结构。在 P1 裁剪说明中标注 `P1_simplified: true`。

### R3. P5→P4→P2 连锁上溯风险

P5 失败 MAX → 上溯 P4，P4 也耗尽 → 上溯 P2。两步单跳都合法，但组合等于跨多阶段回退。当前只有 P6→P1 等大跨度单跳被禁止自动，但链式上溯未拦截。

**建议**：L2 上溯增加"跨阶段上溯前先 PAUSED 通知人"规则。当上溯目标与当前阶段不相邻时（即上溯会导致跳过至少一个中间阶段），先 PAUSED 报告再执行。
...
| 优先级 | 项 | 理由 |
|--------|-----|------|
| P0 | 创建 `scripts/check-tdd-red.sh`（S6） | P3 gate 跑不通，实操阻塞 |
| P0 | 统一 D8：确认小结在 P8 后输出 | 文档互相矛盾 |
| P0 | 修正 P7 grep 命令（S4 问题 1） | 当前表达晦涩易误读，改用 `! grep -q` |
| P1 | 定义 P3 跳过后 P4 门槛（S1） | 小任务高频场景，不定义走不通 |
| P1 | 修复 D1-D5 v3→v4 标注（8 处残留） | 文档身份混乱 |
| P1 | 增加 PAUSED→恢复协议（G1） | 实操中断后无恢复路径 |
| P1 | P8 gate 补注 packages 动态生成规则（S5） | state-machine 与 dispatch-protocol 脱节 |
| P2 | 增加 SCOPE+ 影响范围决策矩阵（S3） | 核心机制缺操作规则 |
| P2 | 增加 BDD 质量门槛（R1） | 脊梁不稳全流程摇晃 |
| P2 | 增加 P7 覆盖度门槛（S4 问题 2） | 空壳 P7 可过 gate |
| P2 | 清理 D7 已实现改进项 | 减少信息噪音 |
| P2 | 链式上溯拦截规则（D6/R3） | 防止自动跨多阶段回退 |
| P3 | 小任务 P1 降级（R2） | 降低小任务摩擦 |
