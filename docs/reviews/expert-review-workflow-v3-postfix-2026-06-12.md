...
### 发现 D（🟠 重要｜架构）：v3 从未用自己跑过真实任务

**现象**：T001/T002 用的是 v2；T003（修 v3 自己）也没走 v3 流程；validation-report 只验证了 Phase 1-3 的 echo 级派发（读 input.md 写行数），**没有验证过一次真实的「门槛判定 → 重试 → L2 上溯 → 回退」闭环**。

**为什么是架构问题**：v3 的高价值机制——A1 独立验证、retry 计数落盘、L2 单规则上溯、阶段回退 retry 不清零——全部只在文档里成立，没有一次运行证据。validation 证明的是「subagent 能被派发且上下文隔离」，这是地基的最底层，不是上层编排逻辑。**最可能出问题的恰恰是上层**：主 Agent 会不会在 P5→P4 回退时把 P4 retry 错误清零？会不会在门槛模糊时跳过独立验证直接信 subagent？这些都没跑过。

**锁定决策（建议）**：在加任何新机制之前，**用 v3 完整跑一个真实小任务**（从 improvement-backlog 挑一个 🔵 长期项，如「#10 SQLite 并发写边界文档化」或「#3 MCP/CLI 定位写进用户文档」——纯文档、低风险、适合首跑）。跑的目标不是产出，是**暴露过度设计**：哪些阶段是空转、哪些门槛判不动、哪个角色产出对不上下游输入。让实践做减法，回应自评审发现1。

这是本轮最重要的一条：v3 已经是「设计得很好的文档」，但还没跨越到「被真实任务验证过的系统」。
...
| D | 🟠 决策 | （流程）| 用 v3 完整跑一个真实小任务，再决定做哪些减法 | 需用户拍板 |
| E | 🟠 决策 | （记入 backlog）| owner+lease 任务锁，等真实任务暴露并发瓶颈再实现 | 暂不做 |
| F | 🟡 可选 | state-machine.md | check-tdd-red 数字解析改机器可读，当前标已知限制 | 暂不做 |

A/B/C/G 是文档级机械修复，可立即落地。D 是本轮核心建议，需要你拍板。E/F 记入 backlog，不现在做。

---

## 四、一句话总结

发现2 的修复方向对（硬中断点确实该有），但落地时引入了编号 bug（发现A）和悬空层级 L3（发现B），加上一直没消化的 active-tasks 双写矛盾（发现C），构成本轮必修的 3 个阻塞项。更深一层：v3 仍是「没被自己验证过的系统」（发现D）——下一步不是继续加机制，而是**用 v3 跑一个真任务，让实践告诉你哪些是过度设计**。减法比加法重要，这一点上一轮自评审已经说对了，本轮再次确认，且给出了具体的首跑任务建议。
