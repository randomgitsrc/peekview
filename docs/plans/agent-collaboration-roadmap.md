...
**NEW-2：后端 Admin User API**
- `GET/PATCH/DELETE /api/v1/admin/users/{id}`
- `GET /api/v1/admin/users`
- 认证：`require_admin`（现有机制）
- 目的：让 `peekview admin user *` 命令在 remote 模式下可用

**NEW-3：peekview admin user 命令组**（依赖 NEW-2）
- `peekview admin user list`（local 直连 DB + remote 调 API）
- `peekview admin user delete <username>`（含级联确认）
- `peekview admin user promote/demote <username>`（remote 模式补充）
- `peekview admin user disable/enable <username>`
- 认证：`PEEKVIEW_ADMIN_KEY` 环境变量或 `.env` 优先
...
**MCP-3：list_entries 加过滤参数**
- MCP 工具 `list_entries` 加 `author`/`tag`/`since`/`limit` 参数
- 同步 PeekClient 和 CLI `peekview list` 也补上 `--author` 过滤
...
| 任务 | 内容 | 包 | 优先级 | 依赖 |
|------|------|-----|--------|------|
| T016 | apikey local 模式解锁 | backend | P0 | 无 |
| T017 | Admin User API（后端）| backend | P0 | 无 |
| T018 | peekview admin user CLI | backend | P0 | T017 |
| T019 | user delete（local + remote）| backend | P0 | T017 |
| T014 | MCP path_namespace CLI | mcp-server | P0 | 无（已设计）|
| T015 | MCP config verify + unset | mcp-server | P1 | 无（已设计）|
| T020 | MCP list_entries 过滤参数 | mcp-server + backend | P1 | 无 |
| T021 | peekview raw CLI | backend | P1 | 无 |
| T022 | user password 修改 | backend | P2 | T017 |
| T023 | peekview entry update CLI | backend | P2 | 无 |

**并行说明**：
- T016/T017/T014 可并行（互不依赖）
- T018/T019 依赖 T017（Admin User API）
- T015/T020/T021 可并行

---

## 五、peekview admin 认证设计（展开）

你提出的「管理员 key 类似 LLM provider key 管理」值得单独展开：

```
认证优先级（从高到低）：
1. 环境变量：PEEKVIEW_ADMIN_KEY=pv_xxx
2. .env 文件：项目根目录或 ~/.peekview/.env
3. 配置文件：~/.peekview/config.yaml 的 remote.api_key（若该 key 是管理员 key）
4. 交互式输入（带 --password 提示）
```

命令结构：
```bash
# 用户管理
peekview admin user list
peekview admin user show <username>
peekview admin user delete <username>      # --yes 跳过确认
peekview admin user disable <username>
peekview admin user enable <username>
peekview admin user promote <username>
peekview admin user demote <username>

# 原有 admin 命令保留
peekview admin stats
peekview admin cleanup
```

local 模式（无 `--remote-url`）：直连 DB，无需 key 认证（本机操作）
remote 模式（有 `--remote-url` 或 `PEEKVIEW_REMOTE_URL`）：需要管理员 API Key

---

## 六、与 cli-api-completeness.md 的关系
...
- T010 拆分为 T017（后端 API）+ T018（admin CLI）+ T019（user delete）
- T011 降级为 T021/T023（非 Agent 协作必需）
- T012/T013 合并到更小粒度任务
- 新增 T016（apikey local 解锁，最小代价最大收益）
