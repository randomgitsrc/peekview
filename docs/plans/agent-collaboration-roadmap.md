...
| T012 | peekview user 命令组 remote 模式（PeekClient + CLI + 测试）| backend | P0 | T011 |
| T013 | peekview user delete（local + remote，级联确认）| backend | P0 | T011 |
...
- T012/T013 依赖 T011（API 稳定后再做 CLI）
- T015/T016/T017 可并行

**任务粒度说明**：
- T011：高风险（级联删除 + 磁盘文件清理），独立验收，gate = pytest admin user API 全绿
- T012：机械性工作（7个命令各对应1个 PeekClient 方法），gate = pytest CLI user remote 全绿
- T013：`delete` 单独拆出，因为它是危险操作，需要额外的回滚/确认测试，不和其他命令混
...
- T011 拆分为三个任务：T011（Admin User API）+ T012（user 命令 remote 模式）+ T013（user delete）
  理由：级联删除高风险独立验收；API 和 CLI 有自然验收边界；delete 是危险操作单独测试
