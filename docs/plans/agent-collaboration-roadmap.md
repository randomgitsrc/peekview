...
peekview user password <username>  # local: 直连DB | remote: admin reset 或 self change-password
peekview user list                 # local: 直连DB | remote: GET /admin/users（remote 化顺带）
```
