...
升级后完整命令集：
```
peekview user create <username>    # local: 直连DB | remote: POST /auth/register（admin 触发）
...
| T011 | Admin User API + peekview user 命令组升级 | backend | P0 | 无 |
...
> T012/T013 取消：合并进 T011（Admin User API + CLI 一起交付，避免返工和合并冲突）
...
- T010（原 T010）拆分为：T010 apikey local 解锁 + T011 Admin User API + CLI 升级（T012/T013 合并进 T011）
