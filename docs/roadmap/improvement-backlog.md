...
> 当前版本：Backend v0.11.2 / MCP Server v0.10.0
...
| 38 | Code 内置搜索（匹配高亮 + 计数 + 跳转） | 体验 | 🟡 中期 | ⬜ roadmap（浏览器 Ctrl/F 够用，等用户反馈触发） |
...
| 41 | 前端时间线视图（时间轴 UI + entry tag 标签） | 产品/体验 | 🟡 中期 | ⬜ 依赖 T077 timeline 数据模型落地 |
