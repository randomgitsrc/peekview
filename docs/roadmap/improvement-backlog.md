...
> 当前版本：Backend v0.24.1 / MCP Server v0.12.0
...
| 38 | Code 内置搜索（匹配高亮 + 计数 + 跳转） | 体验 | 🟡 中期 | ❌ 取消（浏览器 Ctrl/F 够用，不做） |
| 39 | 非编码检测（GBK/BOM 嗅探，后端 decode 修正） | 体验/安全 | 🟡 中期 | ⬜ roadmap（当前 utf-8 only，等真实乱码场景触发） |
| 40 | 文件版本保留（覆盖+保留3版 + 并排查看） | 产品 | 🟡 中期 | ⬜ 依赖 T077 timeline MVP 验证 |
| 41 | 前端时间线视图（时间轴 UI + entry tag 标签） | 产品/体验 | 🟡 中期 | ⬜ 待重新想清楚（产品形态未定，不只是依赖 T077 数据模型） |
| 42 | Timeline 嵌套（project → 多条 timeline） | 产品 | 🔵 长期 | ⬜ 等单层 timeline 使用数据验证 |
| 43 | 行级 diff 高亮 | 产品 | 🔵 长期 | ⬜ 和 git 功能重叠，看需求强度 |
| 44 | 3D 模型查看器（Three.js） | 产品 | 🔵 长期 | ⏸️ 数据触发 |
...
| 56 | 全屏模式链接 `/{slug}/f`（分享场景直入锁定纯内容视图） | 产品/分享 | 🟠 近期 | 🔄 TPV0099 |
...
**来源**：2026-09-14 债务盘点实测（用户要求"明确是问题的入 roadmap，可合并的合并立项"）
...
**状态**：🔄 TPV0097（2026-09-14 立项 v2，待开工）

---

### 🟠 51. E2E 统一入口注册表 + CI E2E job

**来源**：2026-09-05 用户提问"全量超 560s 不正常，可以优化么 并行啥的 ci"；2026-09-14 范围重划
...
**来源**：由 TPV0097 v2 范围修订析出（2026-09-14）
...
**来源**：2026-09-16 用户分享场景——entry 分享出去的非 raw 链接（`/{slug}`）是完整页面，接收者有时只想看内容本身；页内 f 键 zen 模式已存在，但无法通过链接直达。

**问题**：需要一个可分享的 URL，打开即"只剩主体内容"（无 header/侧栏/移动端 chrome/元信息条），且不可被误操作退出。

**方案**：新增路由 `/{slug}/f` → 复用现有 zen 模式并锁定（f/Escape 短路、无页内出口）。关键取舍（P0 已与用户确认）：

- **URL 用 path 后缀而非 `?f` query**：`/{slug}/raw` 已是 mode 后缀先例；query 形态会被聊天客户端剪参数（分享场景重灾区）；path 形态下内部导航（切文件）天然保模式，query 形态丢一次即意外退出全屏
- **完全锁死**：键盘用户想看完整页需手动删 `/f`（已接受）
- **zen 外观统一**：元信息条在 zen 模式下一并隐藏（同时影响 f 键 zen，P1 确认）
- 后端零改动（SPA catch-all 已放行）；`/{slug}/f?share=token` 组合天然可用

**状态**：🔄 TPV0099（2026-09-16 立项，待开工）
