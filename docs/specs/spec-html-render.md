...
### P1 — 测试设计 ✅
- [x] Blob URL 正确创建和释放（无内存泄漏）
- [x] DOMParser 相对路径检测触发警告条
- [x] > 2MB 文件不自动渲染，手动触发正常
- [x] Load 事件后 Loading 态消失
- [x] HTML 文件时 Wrap 按钮不显示
- [x] Copy tooltip 显示"Copy HTML source"
- [x] 切换 .html 和 .css 渲染方式正确
- [x] **安全 negative test**：iframe 内 `top.location` 修改失败
- [x] **安全 negative test**：iframe 内无法读取父页面 cookie

### P2 — 代码实现 ✅
- [x] `HtmlViewer.vue`
- [x] `EntryDetailView.vue` 改动
- [x] `entry.ts` store 改动
...
- [x] CHANGELOG 更新
- [x] FEATURES.md 更新
- [x] INDEX.md spec 状态更新
- [x] 后端测试：393 passed（无回归）
- [x] 前端单元测试：22 passed（全绿）
- [x] 文档版本同步检查通过
- [x] spec 与实现交叉验证通过
