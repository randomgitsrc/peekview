...
export function createTools(client: PeekViewClient, config: ServerConfig): ToolDefinition[] {
  if (config.mode === 'local') {
    // 本地模式：只暴露 publish_files
    return [publishFilesTool(client, config)];
  } else {
    // 远程模式（默认）：只暴露 create_entry
    return [
...
```typescript
const MAX_SINGLE_FILE_BYTES = 5 * 1024 * 1024;   // 5MB（后端上限 10MB，留余量）
...
```
⚠️ For Agent-generated content only.
...
- 本次改动为 **MCP Server v0.6.0**（Breaking Change：工具集根据模式变化）
