# P4 dispatch-context: implementer (重试 #3 — 修 E2E 选择器)

## 目标
P5 E2E 失败：admin.spec.ts 选择器与 AdminView.vue class 命名不一致。对齐选择器让 E2E 通过。

## 问题
- admin.spec.ts 用 `.admin-user-list` / `.admin-user-row` / `.overflow-menu-trigger`
- AdminView.vue 实现是 `.user-list` / `.user-row`
- BDD-01 等不到元素超时，serial 模式后续 26 测试未执行

## 修复方案（最小改动）
给 AdminView.vue 关键元素加 `data-testid`（稳定不变动 class 命名），admin.spec.ts 改用 data-testid 选择器：
- 列表容器：data-testid="admin-user-list"
- 每行：data-testid="admin-user-row"
- OverflowMenu 触发器：data-testid="overflow-menu-trigger"（或复用现有 aria-haspopup）
- 分页：data-testid="pagination"
- badge：data-testid="user-badge"

## 约束
- 只改 admin.spec.ts + AdminView.vue（加 data-testid），不改逻辑
- 修后重跑 E2E admin.spec.ts 确认通过
- 严禁 --no-verify
