...
## 目标
执行 P2-design.md gate_commands.P5 + P5_e2e，验证实现正确性。产出 P5-test-results/。

## gate_commands（从 P2-design.md）
- P5: `make test-quick && make test-frontend && make typecheck`
- P5_e2e: `make debug-test`（ui_affected=true）

## 约束
- 紧凑输出模式（pytest --tb=no / vitest --reporter=dot）
- 严禁碰 :8080/~/.peekview/，E2E 用 :8888（make debug）
- [PROD_NOT_TOUCHED]
- 预存失败登记到 known-failures.md（test_t073 ruff env issue）

## 输入
1. P2-design.md（gate_commands）
2. P4-implementation.md（实现清单）
3. P3-test-cases.md（测试映射）

## 执行
1. 跑 `make test-quick`（后端 pytest 全套，紧凑模式）
2. 跑 `make test-frontend`（vitest 非 watch）
3. 跑 `make typecheck`（vue-tsc）
4. 跑 `make debug-test`（Playwright E2E，需先 make debug-start + make build-frontend）
   - E2E 用 admin.spec.ts（T080 新增）
   - 若 E2E 超时或失败，记录到 e2e.md，不强制全绿（UI 验收在 P6）
5. 写 P5-test-results/unit.md（后端+前端结果 + failed 计数）
6. 写 P5-test-results/e2e.md（E2E 结果 + 截图路径）
7. 预存失败登记 known-failures.md（test_t073 ruff env）

## 产出
- docs/tasks/T080-admin-user-management/P5-test-results/unit.md
- docs/tasks/T080-admin-user-management/P5-test-results/e2e.md
- docs/tasks/T080-admin-user-management/P5-test-results/fail-list.txt（可为空）
- docs/tasks/T080-admin-user-management/known-failures.md（test_t073）

Header: phase=P5, type=test-results, parent=P4-implementation.md, status=draft, agent=verifier
