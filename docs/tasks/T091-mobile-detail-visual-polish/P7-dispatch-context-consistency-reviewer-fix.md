...
# 派发指引 — T091 P7 格式修正（DESIGN_GAP 标记格式不符合 gate 正则）

## 问题

`check-gate.sh P7` 跑出 WARNING：`P4 检测到设计偏差相关关键词但 [DESIGN_GAP:] 计数为 0`。诊断：gate 脚本用正则 `^\s*>?\s*-?\s*\[DESIGN_GAP:`（行首）和 `^\s*>?\s*-?\s*\[DESIGN_GAP_REVIEWED`（行首）扫描 `P7-consistency.md`，但当前文件里这两处标记分别写成：

- L17：`` > `[DESIGN_GAP: markdown-body ...]` ``——反引号包裹，`[` 前多了一个反引号字符，不匹配正则
- L23：`` **`DESIGN_GAP_REVIEWED`**：... ``——完全没有方括号 `[`，用的是加粗+反引号，不是协议要求的 `[DESIGN_GAP_REVIEWED]` 字面格式

内容本身（DESIGN_GAP 转抄 + 闭环说明）是对的、完整的，**只是这两处标记的字面格式不对**，导致 gate 脚本用 grep 扫描时计数为 0（两边都是 0，恰好"配对"通过没有硬拦截，但这不是真正的格式合规，只是巧合）。

## 你要做的事

只改这两处标记本身的字面写法，不改任何分析内容：

1. L17：把 `` > `[DESIGN_GAP: ...]` `` 改成不带反引号包裹的 `[DESIGN_GAP: ...]`（方括号本身在行首附近，前面最多允许 `>`/`-`/空白，不能有反引号）
2. L23：把 `` **`DESIGN_GAP_REVIEWED`**： `` 改成 `[DESIGN_GAP_REVIEWED]`（带方括号的字面标记，作为该段落的开头），后面正常接你的分析文字

修改后请自己跑一遍确认：
```bash
grep -cE '^\s*>?\s*-?\s*\[DESIGN_GAP:' docs/tasks/T091-mobile-detail-visual-polish/P7-consistency.md
grep -cE '^\s*>?\s*-?\s*\[DESIGN_GAP_REVIEWED' docs/tasks/T091-mobile-detail-visual-polish/P7-consistency.md
```
两条命令都应该输出 `1`（不是 0）。
...
- 不改动其他任何章节/结论/证据引用
- 不改变 DESIGN_GAP 的技术内容和 REVIEWED 说明的实质意思，只改标记本身的字面格式
