...
# 派发指引 — T091 P5 重新验证（P4 重试 #1 后）

## 背景

P4 走过一轮完整实现→P5 全绿→P6 视觉验收发现真实缺陷（`EntryMetaTagsBar.vue` 的 `flex-wrap` 与 `layout.css` 遗留全局规则冲突导致 meta-tags-bar 在 `.content-area` 可滚动场景下坍缩到 33px）→ 规范回退到 P4 → 定向修复（`EntryMetaTagsBar.vue` scoped 规则新增 `overflow-x:visible; white-space:normal;`，design-review approved）。现在需要重新走一遍完整的 P5 技术验证（不是只验证改动的那一行，是全量重跑，因为这是新一版代码）。

## gate_commands.P5（P2-design.md 第 5 节固化，不变）
...
两条都要跑，跟上一轮 P5 一样的标准：`make test-frontend`（vitest 全量）+ `E2E_SPEC=e2e/t09 make debug-test`（Playwright E2E，覆盖 t090 手术式修改的 2 条 + t091 新建的 13 条 BDD）。

## 与上一轮 P5 的区别

- 上一轮的 `docs/tasks/T091-mobile-detail-visual-polish/P5-test-results/` 已经提交在历史 commit 里（`08418467`），是**旧代码**（P4 第一版实现）的验证结果，本轮要**覆写**这些文件为新代码的验证结果，不要保留旧数据或说"和上次一样"
- 本轮修复本身不改变任何 E2E 断言（t09 spec 文件本身没有被本轮 implementer 改动），所以理论上 50/50 应该依然全绿——但你必须真实重跑确认，不能因为"代码变化很小"就假设结果不变
- **重点关注**：t091 spec 的 BDD-1（`test_bdd_1_meta_tags_bar_wraps_no_horizontal_scroll`）和 BDD-2（`test_bdd_2_meta_tags_bar_breathing_room`）两条测试，这正是本轮修复的目标断言，请在 `unit.md`/`e2e.md` 里明确点出这两条的通过情况（不要淹没在汇总数字里）

## 环境

debug backend 已在 127.0.0.1:8888。跑之前请先确认最新代码已反映（`make build-frontend` 或 `make debug-quick`），因为 orchestrator 在验证修复时已经跑过 `make build-frontend`，但请你自己再确认一次，不要假设。严禁触碰生产 `:8080`/`~/.peekview/`。

## 产出规格（同上一轮标准，硬性）

- `docs/tasks/T091-mobile-detail-visual-polish/P5-test-results/unit.md`：`make test-frontend` 结果，含原生测试运行器输出行（grep 签名校验）
- `docs/tasks/T091-mobile-detail-visual-polish/P5-test-results/e2e.md`：`E2E_SPEC=e2e/t09 make debug-test` 结果，含 25 条唯一 BDD × 2 browser project 逐项一览表
- `docs/tasks/T091-mobile-detail-visual-polish/P5-test-results/fail-list.txt`：失败列表，无失败可为空

如实报告，不要为了让结果好看而隐瞒失败。若发现预存失败按标准流程登记 known-failures.md。
