...
独立评审 TPV0088 的 P1 需求基线 `P1-requirements.md`，产出 `P1-review.md`（status: approved/rejected/needs-revision）。

## 评审重点

1. **BDD 可二值判定**：9 条 BDD 每条 Given/When/Then 是否 PASS/FAIL 明确可判
2. **子任务 A 覆盖**：viewer.spec.ts 20 用例修复的验收标准是否可验证（路由修正 + slug 替换 + 单文件 entry 数据依赖）
3. **子任务 B 覆盖**：static mtime 校验的 BDD 是否覆盖过期拦截/新鲜放行/正常流程不误伤三态
4. **隐含需求覆盖**：逐维度（数据/前端/多端/边界/兼容）
5. **裁剪合理性**：P7 不可裁剪（多文件改动）；risk_level: medium 是否匹配
6. **P1 纯净性**：是否掺入方案设计

## 上游关联

- 输入文件（必读）：
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P1-requirements.md`（被评审对象）
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P0-brief.md`（上游约束，判断 BDD 是否偏离 P0 审计事实）
  - `/home/kity/.agate/assets/review-roles/requirements-review.md`（角色定义）
- 可选：`frontend-v3/e2e/viewer.spec.ts`、`Makefile:540-650`（如需要核实 BDD 断言）
...
`docs/tasks/TPV0088-e2e-test-infra-hardening/P1-review.md`
...
trace_id: TPV0088-P1-review-20260812
status: draft
created: 2026-08-12
...
- 关键标识：评审对象 P1-requirements.md（9 条 BDD）
- 查证结果：P0 审计已定位两子任务失效清单
</objective_info>
