...
## 目标
按 P2-design.md 实现 T079 代码：新增 AuthButton + UserMenu 组件 + 修改三个页面 + 移除 Explore 按钮 + tag 改 BaseTag。

## 约束
- 按 P2-design.md 方案实现
- 后端无改动
- SCOPE_RESOLVED: Settings URL 保持 /settings?tab=apikeys，文案改为 "Settings"
- 现有测试零回归（1078 passed + 1 skipped）

## 输入文件（按 P2 files_to_read）
1. `frontend-v3/src/views/LandingView.vue`
2. `frontend-v3/src/views/EntryListView.vue`
3. `frontend-v3/src/views/EntryDetailView.vue`
4. `frontend-v3/src/components/EntryDetailHeader.vue`
5. `frontend-v3/src/components/BaseTag.vue`
6. `frontend-v3/src/components/BaseButton.vue`（参照模式）
7. `frontend-v3/src/stores/auth.ts`
8. `frontend-v3/src/components/__tests__/AuthButton.spec.ts`（P3 测试，了解预期行为）
9. `frontend-v3/src/components/__tests__/UserMenu.spec.ts`
10. `frontend-v3/src/components/__tests__/T079-entry-detail-header.spec.ts`
