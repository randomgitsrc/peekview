...
逐条验证 P1 的 24 条 BDD 验收条件，产出 P6-acceptance.md + P6-evidence/。

## 约束

  每条 BDD 只允许 PASS 或 FAIL（不允许中间态）
  PASS 必须有证据文件引用
- BDD-1~17 可用自动化测试结果作为证据（P5 全绿）
- BDD-18~24 需手动验证文档和工具描述
- ui_affected: false — 不需要 vision-helper
...
2. `packages/mcp-server/src/tools/publishFiles.ts` — 验证 CWD guard 修复
3. `packages/mcp-server/src/config/merge.ts` — 验证 allowed_paths 容错
4. `packages/mcp-server/src/server.ts` — 验证 /health 增强
5. `packages/mcp-server/src/cli/config.ts` — 验证 config list/verify 增强
6. `packages/mcp-server/README.md` — 验证文档修正
7. `README.md` — 验证 Docker 场景指引 + OpenCode/Cursor
8. `backend/README.md` — 验证 Docker 场景指引

## BDD 验证方式

| BDD | 验证方式 | 证据类型 |
|-----|----------|----------|
| BDD-1~17 | 自动化测试（P5 全绿）| test-output.log |
| BDD-18~22 | 文档搜索/检查 | doc-check.log |
| BDD-23~24 | 工具描述检查 | tool-desc-check.log |
...
- `docs/tasks/T070-mcp-docker-deployability/P6-acceptance.md`
- `docs/tasks/T070-mcp-docker-deployability/P6-evidence/`
