...
### 目标
逐条验收 P1 的 41 条 BDD，产出 P6-acceptance.md + P6-evidence/。每条 BDD 只允许 PASS 或 FAIL。

### 约束
- 每条 BDD 必须有证据文件引用
- 验收行格式：行首 PASS BDD-NN: 描述 (证据路径)
- 证据存入 P6-evidence/ 目录
- ui_affected=false，不需 Playwright 截图——行为零回归类 BDD 用测试通过作为证据
- 证据可以是：grep 输出日志、测试输出、文件行数检查等
- agent 字段填 verifier
- 严禁操作生产环境

### 上游关联
- P5 测试全绿：后端 985 passed, 前端 1078 passed, typecheck exit 0, lint exit 0
- P4 实现完成 + 评审通过
- P2-design.md：方案设计

### BDD 验收策略

**grep 验证类（BDD-1~6, 11~13）**：执行 grep 命令检查代码结构，输出存入 P6-evidence/grep-results.log
- BDD-1: `rg "StorageManager(config=" backend/peekview/api/files.py` → 0 匹配
- BDD-2: `rg "Session(engine)" backend/peekview/api/files.py` → 0 匹配
- BDD-3: `rg "EntryService(engine=" backend/peekview/services/admin_service.py` → 0 匹配
- BDD-4: `rg "ApiKeyService(engine=" backend/peekview/auth.py` → 0 匹配
- BDD-5: `rg "ReadTrackingService(engine=self.engine)" backend/peekview/services/entry_service.py` → 0 匹配
- BDD-6: `rg "ShareService(engine=self.engine" backend/peekview/services/entry_service.py` → 0 匹配
- BDD-11: `rg "def _looks_like_jwt" backend/peekview/` → 1 匹配
- BDD-12: `rg "def _is_global_api_key_auth" backend/peekview/` → 1 匹配
- BDD-13: `rg "def _record_read_async" backend/peekview/` → 1 匹配

**错误格式验证类（BDD-7~10）**：grep HTTPException 残留 + 检查 PeekError 格式
- BDD-7: `rg "raise HTTPException" backend/peekview/api/` → 0 匹配（main.py 基础设施除外）
- BDD-8~10: 确认 entries.py/auth.py/admin.py 使用 PeekError 子类

**事务验证类（BDD-14~15）**：P3 测试已覆盖（test_t082_transaction.py），P5 测试全绿
**测试覆盖类（BDD-16, 40, 41）**：P5 测试结果（985 + 1078 passed, typecheck exit 0）

**前端拆分验证类（BDD-17~24）**：
- BDD-17: `ls frontend-v3/src/stores/entryList.ts entryDetail.ts` → 存在
- BDD-18: `wc -l frontend-v3/src/stores/entryList.ts entryDetail.ts` → 各 < 150
- BDD-19: `rg "loadSeq" frontend-v3/src/stores/entryList.ts` → 存在
- BDD-20: 前端测试通过（store 拆分测试）
- BDD-21: make test-frontend 中 searchUrl 逻辑测试通过
- BDD-22: 前端测试通过（EntryListView 测试）
- BDD-23: `wc -l frontend-v3/src/views/EntryDetailView.vue` → < 300
- BDD-24: `wc -l frontend-v3/src/components/EntryDetail*.vue` → 各 < 200

**行为零回归类（BDD-25~38）**：P5 前端测试 1078 passed 覆盖行为零回归
**错误格式兼容（BDD-39）**：grep 检查 3 处 .error.message

### 输入文件
- docs/tasks/T082-arch-refactor/P1-requirements.md（41 条 BDD）
- docs/tasks/T082-arch-refactor/P5-test-results/unit.md（P5 测试结果）
...
<objective_info>
### P5 测试结果
- 后端 985 passed, 0 failed
- 前端 1078 passed, 0 failed
- typecheck exit 0
- lint exit 0

### 关键文件路径
- 后端代码：backend/peekview/
- 前端代码：frontend-v3/src/
- P3 测试：backend/tests/test_t082_*.py, frontend-v3/src/stores/t082-*.spec.ts
- P5 结果：docs/tasks/T082-arch-refactor/P5-test-results/unit.md
</objective_info>
