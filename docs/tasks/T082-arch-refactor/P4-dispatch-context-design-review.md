...
### 目标
P4 前端实现评审。审查 R5/R6/R7 的代码改动，找视觉/交互/状态管理问题。

### 约束
- 只审不写——不改代码，产出评审意见 P4-review-design.md
- agent 字段填 design-review
- 审查范围：frontend-v3/src/ 下的改动文件
- 本任务是纯重构（行为零回归），重点审拆分是否破坏现有交互

### 上游关联
- P4 实现完成：R5 store 拆分 / R6 EntryDetailView 拆分 / R7 错误格式兼容
- P2-design.md：R5/R6/R7 方案设计 + props/emit 契约表（已 approved）
- 新建文件：stores/entryList.ts, stores/entryDetail.ts, components/EntryDetail*.vue, composables/use*.ts

### 输入文件
- docs/tasks/T082-arch-refactor/P2-design.md（方案设计 + props/emit 契约表——对照实现是否忠实）
- docs/tasks/T082-arch-refactor/P4-implementation-frontend.md（实现记录）
- docs/tasks/T082-arch-refactor/P1-requirements.md（BDD-16~39 验收条件）
- AGENTS.md + DESIGN.md（项目约定、设计系统）
- 改动的前端代码文件（用 git diff 查看，或直接读取改后文件）
</dispatch_guide>
...
### P4 前端改动文件清单
新建文件：
- frontend-v3/src/stores/entryList.ts（从 entry.ts 拆分）
- frontend-v3/src/stores/entryDetail.ts（从 entry.ts 拆分）
- frontend-v3/src/components/EntryDetailHeader.vue
- frontend-v3/src/components/EntryDetailContent.vue
- frontend-v3/src/components/EntryDetailBanners.vue
- frontend-v3/src/components/EntryDetailDialogs.vue
...
- frontend-v3/src/composables/useEntryDetailActions.ts
- frontend-v3/src/composables/useEntryDetailComputed.ts
- frontend-v3/src/composables/entryDetailKeys.ts

修改文件：
- frontend-v3/src/views/EntryDetailView.vue（1003行→拆分为主组件+子组件）
- frontend-v3/src/views/EntryListView.vue（import 更新）
