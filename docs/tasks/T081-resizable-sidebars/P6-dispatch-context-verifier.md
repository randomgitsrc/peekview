...
逐条验收 P1 的 16 条 BDD，产出 P6-acceptance.md + P6-evidence/。用 Playwright CDP 实跑验证真实浏览器行为，截图存入 P6-evidence/screenshots/。

## 约束

- 每条 BDD 只允许 PASS 或 FAIL，不允许中间态
- 所有 PASS 必须有文件引用（截图路径 / 断言日志）
- 操作类 BDD 截图必须互不相同（md5 去重）
- 证据文件末行须有 EXIT_CODE 声明
- P6 是 self-authored gate——主 Agent 只跑 gate 脚本，不写验证脚本
- ui_affected=true：至少一条 PASS 基于 vision-helper 报告

## 上游关联

- P1-requirements.md：16 条 BDD 验收条件
- P5-test-results/：单测全绿（1212 passed），E2E 未跑

## 输入文件

1. `docs/tasks/T081-resizable-sidebars/P1-requirements.md` — 16 条 BDD
2. `docs/tasks/T081-resizable-sidebars/P2-design.md` — 方案设计（resize handle 位置、CSS 变量、min/max 值）
3. `docs/tasks/T081-resizable-sidebars/P5-test-results/unit.md` — 单测结果

## BDD 验收策略

### composable 单测已覆盖（P5 验证 14 测试全绿）
BDD-01~07 + BDD-12~16 的逻辑路径已由 vitest 单测覆盖。P6 需用 Playwright 实跑验证真实浏览器行为。

### Playwright CDP 验证脚本
- Chrome 151 :18800 CDP 在线
- 连接方式：`chromium.connectOverCDP('http://127.0.0.1:18800')`
- debug backend: http://127.0.0.1:8888
- 测试页面：含长文件名的 entry（如 k8s-deployment）
- 脚本必须 `try/finally { page.close() }` + `process.exit(0)`
- 截图存入 P6-evidence/screenshots/

### BDD 分组
- BDD-01/02: 拖拽改变宽度 → Playwright mouse 拖拽 + 截图 + getComputedStyle 验证
- BDD-03/04: clamp 到 max/min → 拖拽超出范围 + 验证宽度值
- BDD-05: localStorage 恢复 → 设置 localStorage + 刷新 + 验证宽度
- BDD-06/07: 非法值/超范围回退 → 设置非法 localStorage + 验证默认宽度
- BDD-08: <1024px 不显示 handle → setViewportSize + 验证 handle display:none
- BDD-09: zen mode 隐藏 handle → 触发 zen mode + 验证 handle 不可见
- BDD-10/11: 条件渲染联动 → 无文件/无 TOC 时验证 handle 不存在
- BDD-12: 拖拽期间 user-select:none → 拖拽中验证 body class
- BDD-13: 拖拽不触发滚动 → 拖拽中验证 scrollTop 不变
- BDD-14/15: 双击 reset → dblclick + 验证宽度恢复默认
- BDD-16: 键盘聚焦 → Tab 到 handle + 验证 focus-visible

## 客观查证信息

- debug backend: http://127.0.0.1:8888（200 OK）
- Playwright CDP: Chrome 151 :18800 在线
- seed 数据含 k8s-deployment（3 文件，长路径）、multi-format-demo（4 文件）
- useSidebarResize composable 已实现（P4）
- CSS 变量 --sidebar-width / --toc-width 已统一
