...
执行 T081 gate_commands.P5（vitest 全量单测）+ gate_commands.P5_e2e（Playwright E2E），产出 P5-test-results/。

## 约束

- 执行 P2-design.md 声明的 gate_commands
- 紧凑输出模式（--reporter=dot / --reporter=line）
- 产出 P5-test-results/unit.md + P5-test-results/e2e.md
- 产出 P5-test-results/fail-list.txt（可为空）
...
1. `docs/tasks/T081-resizable-sidebars/P2-design.md` — gate_commands
2. `docs/tasks/T081-resizable-sidebars/P4-implementation.md` — 实现文件清单
...
- vitest 1.6.1 可用
- Playwright 1.60.0 + CDP Chrome 151 :18800
- debug backend: http://127.0.0.1:8888（PEEKVIEW_DEBUG_MODE=1，隔离数据 /tmp/peekview-debug/）
- E2E 测试文件 `e2e/t081-resizable-sidebars.spec.ts` 可能不存在（P3 只写了 composable 单测，E2E 可能需要在此阶段创建或确认不存在）

## 特别注意

- 如果 E2E 测试文件不存在，在 P5-test-results/e2e.md 标注"E2E 测试文件未创建，P6 验收时补"
- 全量 vitest 可能发现预存失败（与 T081 无关），在 unit.md 标注"预存失败"
- 产出文件末行须有 EXIT_CODE 声明
