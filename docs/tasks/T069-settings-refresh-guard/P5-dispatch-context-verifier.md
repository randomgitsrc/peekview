...
<dispatch_guide>
### 目标
执行 P2-design.md gate_commands.P5，验证前端测试通过。

### 约束
- 只读验证，不改代码
- gate_commands.P5: `cd frontend-v3 && npx vitest run --reporter=dot 2>&1 | tail -30`

### 上游关联
- P4 已完成 auth guard + header/bottom bar UI polish
- 预期结果：1021 passed, 1 预存失败(T068)

### 输入文件
- /home/kity/oclab/peekview/docs/tasks/T069-settings-refresh-guard/P2-design.md
- /home/kity/oclab/peekview/docs/tasks/T069-settings-refresh-guard/P4-implementation.md
</dispatch_guide>
...
<objective_info>
- 环境状态：Vue 3 + Vite + TypeScript
- 预期：1021 passed, 1 预存失败(T068)
</objective_info>
