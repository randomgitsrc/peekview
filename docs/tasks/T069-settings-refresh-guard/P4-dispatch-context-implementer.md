...
<dispatch_guide>
### 目标
按 P2-design.md 实现 T069：auth guard 修复 + header/bottom bar 设计打磨。

### 约束
- 改动范围：router.ts、EntryDetailView.vue、layout.css、FileTree.vue
- 不改后端
- 不改 authStore 的 fetchMe 逻辑
- 环境隔离：前端开发走 `make debug`（:8888）

### 上游关联
- P2-design.md：方案A async guard + Promise.race 超时
- P1-requirements.md：24 条 BDD
- P3-test-cases.md：24 BDD 测试覆盖

### 输入文件（按 P2 files_to_read）
- /home/kity/oclab/peekview/docs/tasks/T069-settings-refresh-guard/P2-design.md
