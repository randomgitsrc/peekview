...
## 目标
逐条实跑 P1 的 17 条 BDD 验收条件，产出 P6-acceptance.md + P6-evidence/。本任务为后端任务（ui_affected: false），无 UI 截图需求，证据为 API 响应日志 / pytest 输出。

## 约束
- 每条 BDD 只允许 PASS 或 FAIL
- 所有 PASS 必须有证据文件引用
- P6-evidence/ 必须非空
- BDD-16 用 P5 test-quick 输出作为证据
- BDD-17 用 pytest 超时断言作为证据
- 无 PROD_TOUCHED
- 不改项目源码（只验证）

## 上游关联
- P1-requirements.md：17 条 BDD（BDD-1~17）
- P5-test-results/unit.md：make test-quick 输出（BDD-16 证据）

## 输入文件
1. `docs/tasks/T083-cjk-search-fix/P1-requirements.md` — 17 条 BDD 定义
2. `docs/tasks/T083-cjk-search-fix/P5-test-results/unit.md` — P5 测试输出（BDD-16 证据）
3. `backend/tests/test_cjk_search.py` — P3 测试代码（BDD-1~15+17 可用 pytest 实跑）

## 客观查证信息
- P5 已验证：1001 passed + 2 skipped
- test_cjk_search.py 16 个测试覆盖 BDD-1~15+17，全部 PASS
- ui_affected: false → 无需截图
- BDD-16（make test-quick 全绿）：P5-test-results/unit.md 已含输出
- BDD-17（jieba 预加载不阻塞首请求）：test_cjk_search.py::TestBDD17JiebaPreload

## 验证方式
BDD-1~15+17：通过 pytest 跑 test_cjk_search.py，输出作为证据
BDD-16：引用 P5-test-results/unit.md

## 执行命令
```
cd /home/kity/oclab/peekview/backend && .venv/bin/python -m pytest tests/test_cjk_search.py -v --tb=short 2>&1 | tee /home/kity/oclab/peekview/docs/tasks/T083-cjk-search-fix/P6-evidence/test-output.log
```

## 门槛
- P6-acceptance.md 含 17 条 BDD 验收结果（全 PASS 或 FAIL）
- P6-evidence/ 非空
- 每条 PASS 有证据引用
- BDD 结果格式：行首 `- PASS BDD-NN:` 或 `- FAIL BDD-NN:`
