...
### 目标
按 P2-design.md 方案A 修复 19 处 SQLAlchemy Column 比较误改 + pyproject.toml 添加 E711/E712 ignore。

### 约束
- 只改 P2-design.md §2 修复映射表列出的 19 处 + pyproject.toml
- 修复语法：`not Column` → `~Column`，`Column is not None` → `Column.isnot(None)`，`Column is None` → `Column.is_(None)`，裸 Column → `Column.is_(True)`
- 不改 Python 上下文的 `is None`/`is not None`/`not`（如 `if entry.expires_at is None`）
- 不改前端、不改 MCP
- 环境隔离：测试用 `make test-quick`（venv pytest），调试用 `make debug`（:8888）

### 上游关联
- P2-design.md 方案A：全局 ignore E711/E712 + 19 处 SQLAlchemy 修复
- P3-test-cases.md：13 BDD 全覆盖，45 真红灯
- 当前 45 个测试失败

### 输入文件（按 P2 files_to_read 清单）
- /home/kity/oclab/peekview/backend/peekview/services/admin_service.py:123-160,189-223
- /home/kity/oclab/peekview/backend/peekview/services/share_service.py:68-73,175-181,198-227,240-246,264-269
- /home/kity/oclab/peekview/backend/peekview/services/apikey_service.py:158-164
- /home/kity/oclab/peekview/backend/peekview/services/entry_service.py:90-97,441-452
- /home/kity/oclab/peekview/backend/peekview/database.py:482-487
- /home/kity/oclab/peekview/backend/tests/test_read_tracking.py:363-367
- /home/kity/oclab/peekview/backend/pyproject.toml:80-95
</dispatch_guide>
...
- 关键标识：45 个测试失败
- 查证结果：P2-design.md §2 修复映射表列出 19 处误改 + 1 处 pyproject.toml 配置
- 修复语法对照：
  ...
  - pyproject.toml: 添加 E711, E712 到 [tool.ruff.lint] ignore
</objective_info>
