...
### 目标
工程评审 P2-design.md，检查数据流、状态机、接口契约、错误边界、测试策略、多方案探索、实现就绪度。产出 P2-review.md。

### 约束
- 只审不写——不直接改 P2-design.md
- review 结论必须引用具体设计锚点
- agent≠main
- risk_level=high 触发本评审（硬规则）

### 上游关联
- P2-design.md 含 3 个候选方案，选择方案A（全局ignore E711/E712 + 19处SQLAlchemy修复）
- P1-requirements.md：13 条 BDD，risk_level=high
...
- 关键标识：43 个测试失败，6 个文件被误改
- P2-design.md 关键字段：packages=[backend/peekview], domains=[backend], ui_affected=false, gate_commands 已声明
</objective_info>
