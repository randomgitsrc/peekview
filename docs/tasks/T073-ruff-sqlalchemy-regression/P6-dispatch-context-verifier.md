...
### 目标
逐条验收 P1 的 13 条 BDD，产出 P6-acceptance.md。

### 约束
- 每条 BDD 只允许 PASS 或 FAIL（不允许中间态）
- BDD 结果格式：行首 `- PASS BDD-NN: 描述` 或 `- FAIL BDD-NN: 描述`
- 每条 PASS 必须有证据引用
- ui_affected: false，不需要 Playwright
- 证据存入 P6-evidence/

### 上游关联
- P1-requirements.md：13 条 BDD（BDD-1 到 BDD-13）
- P5-test-results/unit.md：971 passed, 0 failed
- P2-design.md：BDD 覆盖映射
...
- /home/kity/oclab/peekview/docs/tasks/T073-ruff-sqlalchemy-regression/P5-test-results/unit.md
</dispatch_guide>
...
<objective_info>
- 环境状态：pytest 9.1.1, ruff 0.15.18
- P5 结果：971 passed, 0 failed
- 13 条 BDD 需逐条验收
</objective_info>
