...
# P1 派发指引 — requirements-review（第二轮）

## 目标

对修订后的 P1-requirements.md 做第二轮评审，确认 5 个必须修改项已全部处理。

## 必须修改项（验证清单）

1. BDD-1/4/5 "或"字句已消除
2. BDD-2 用户菜单 And 子句已删除
3. BDD-8 声明期望 reads 格式 + readStats null/0 处理
4. BDD-9 "视觉权重大于"改为可客观判定条件
5. phases YAML 含 P7

## 输入文件

1. docs/tasks/T067-detail-page-framework/P1-requirements.md — 修订后的需求基线
2. docs/tasks/T067-detail-page-framework/P1-review.md — 第一轮 review 意见

## 约束

- 逐条验证 5 个必须修改项
- 检查新增 BDD-12 是否可二值判定
- 若全部通过 → status: approved
