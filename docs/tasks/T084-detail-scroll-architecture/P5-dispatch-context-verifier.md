# P5 派发指引 — T084 verifier

## 目标

执行 P2-design.md 的 gate_commands.P5，产出 `P5-test-results/`。

## gate_commands（从 P2-design.md 读取，不可修改）
...
- 执行全部 4 个命令（vitest + typecheck + build + e2e）
- 紧凑输出模式
- 严禁直接操作生产环境
- 状态标记：未触发写 `[PROD_NOT_TOUCHED]`

## 上游关联

- P2-design.md：gate_commands 声明
- P4-implementation.md：代码改动清单

## 输入文件

1. `docs/tasks/T084-detail-scroll-architecture/P2-design.md` — gate_commands 来源
2. `docs/tasks/T084-detail-scroll-architecture/P4-implementation.md` — 实现清单

## 客观查证信息

- P4 自查结果：vitest 1129 passed, typecheck 0 errors, build success
- E2E 需要 debug backend 在 :8888 运行（当前已启动）
- 当前 vitest 全绿，P5 验证应确认零回归

## 门槛

- P5-test-results/unit.md（vitest 结果，含 failed 计数 + test runner 输出签名）
- P5-test-results/typecheck.md（vue-tsc 结果）
- P5-test-results/build.md（vite build 结果）
- P5-test-results/e2e.md（Playwright 结果）
- 无 PROD_TOUCHED
