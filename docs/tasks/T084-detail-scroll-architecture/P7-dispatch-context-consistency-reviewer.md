# P7 派发指引 — T084 consistency-reviewer

## 目标

跨文件一致性检查，产出 `P7-consistency.md`。

## 检查清单

1. **DESIGN_GAP 配对**：P4-implementation.md 中无 [DESIGN_GAP] 声明（P4 已声明"无 DESIGN_GAP"）
2. **SCOPE+ 闭环**：P1-requirements.md 有 2 个 [SCOPE_RESOLVED]（BDD-08 + BDD-09），确认所有 SCOPE+ 增补已纳入基线
3. **跨文件一致性**：
   - P2 packages (frontend-v3) 与 P4 实现路径一致？
   - P1 BDD 14 条 vs P6 验收 14 条（14/14 PASS）？
   - P4 实现路径和 P2 方案 A 吻合？
   - P2 gate_commands 与 P5 执行命令一致？
4. **未决项清零**：无残留 [NEED_CONFIRM]、[BLOCKER]、[DEVIATION-CRITICAL]

## 上游关联

- P1-requirements.md（14 BDD，2 [SCOPE_RESOLVED]）
- P2-design.md（方案 A，4 字段，gate_commands）
- P4-implementation.md（6 文件改动，无 DESIGN_GAP）
- P6-acceptance.md（14/14 PASS）

## 输入文件

1. `docs/tasks/T084-detail-scroll-architecture/P1-requirements.md`
2. `docs/tasks/T084-detail-scroll-architecture/P2-design.md`
3. `docs/tasks/T084-detail-scroll-architecture/P4-implementation.md`
4. `docs/tasks/T084-detail-scroll-architecture/P4-diagnosis.md`
5. `docs/tasks/T084-detail-scroll-architecture/P6-acceptance.md`

## 门槛

- P7-consistency.md 存在
- 无 [BLOCKER] / [DEVIATION-CRITICAL]
- DESIGN_GAP 全部 REVIEWED 配对（本任务无 DESIGN_GAP，声明"无"）
- SCOPE+ 闭环（2 个 [SCOPE_RESOLVED]）
