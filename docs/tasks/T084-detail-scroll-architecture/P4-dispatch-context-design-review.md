# P4 派发指引 — T084 design-review

## 目标

评审 T084 的 P4 代码实现，产出 `docs/tasks/T084-detail-scroll-architecture/P4-review.md`。

## 约束

- 只审不写——不直接改代码
- 按 design-review 检查清单审查
- status: approved / rejected / needs-revision

## 上游关联

- P4-implementation.md：`docs/tasks/T084-detail-scroll-architecture/P4-implementation.md`
- P2-design.md：方案 A 设计

## 输入文件

1. `docs/tasks/T084-detail-scroll-architecture/P4-implementation.md` — 实现清单
2. `docs/tasks/T084-detail-scroll-architecture/P2-design.md` — 方案设计
3. `/home/kity/.agate/assets/review-roles/design-review.md` — 评审角色定义
4. `frontend-v3/src/components/MarkdownViewer.vue` — 改动文件
5. `frontend-v3/src/styles/code.css` — 改动文件
6. `frontend-v3/src/styles/markdown.css` — 改动文件
7. `frontend-v3/src/composables/useResponsiveLayout.ts` — 改动文件
8. `DESIGN.md` — §9 Scroll Architecture 新增

## 客观查证信息

- 6 文件改动（CSS 移除 + composable 简化 + E2E 修正 + 文档）
- 自查全绿（vitest 1129 passed + typecheck + build）
- 无 [DESIGN_GAP] / [SCOPE+] / [CLARIFY]

## 门槛

- P4-review.md 含 status 字段
- 检查清单逐项审查
- 结论引用具体文件/行号
