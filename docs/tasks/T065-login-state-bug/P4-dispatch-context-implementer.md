...
### 目标
实现 P2 方案 A 的代码修复：watch immediate + Sign in 条件渲染

### 约束
- 按 P2-design.md 的 files_to_read 清单读取代码，不要在项目里盲目搜索
- 只修改 P2 声明的文件：LandingView.vue + router.ts
- 遵守 TDD：P3 测试已写好红灯，实现后测试应变绿
- 不要加范围外改动——发现新需求标 [SCOPE+] 不直接做
- 自跑测试确认基本功能（自查），但自查通过 ≠ P5 gate 通过
- 生产环境隔离：任何写入生产的操作必须 PAUSED 报告人工

### 上游关联
- P2 选定方案 A：watch immediate + Sign in 条件渲染
- P3 测试：4 个红灯测试（landing-auth.spec.ts）
- P1 BDD 共 6 条

### 输入文件
- docs/tasks/T065-login-state-bug/P2-design.md（方案设计 + files_to_read）
- docs/tasks/T065-login-state-bug/P3-test-cases.md（测试用例）
- docs/tasks/T065-login-state-bug/P0-brief.md（环境约束）
- frontend-v3/src/__tests__/landing-auth.spec.ts（TDD 红灯测试）
- frontend-v3/src/views/LandingView.vue（待修改）
- frontend-v3/src/router.ts（待修改）
- frontend-v3/src/stores/auth.ts（authState，只读不修改）
- frontend-v3/src/views/EntryListView.vue（认证 UI 模式参照，只读不修改）
</dispatch_guide>
...
- P3 测试文件: frontend-v3/src/__tests__/landing-auth.spec.ts (4 failed / 0 collection error)
- vitest 命令: cd frontend-v3 && ./node_modules/.bin/vitest run
- 修改文件: LandingView.vue + router.ts
</objective_info>
