# P4 评审派发指引 — T085 design-review

## 目标

评审 T085 前端实现（5 个缺陷修复），产出 `P4-review.md`。

## 输入文件

1. `docs/tasks/T085-render-regression-fix/P2-design.md`（方案）
2. `docs/tasks/T085-render-regression-fix/P4-implementation.md`（实现产出）
3. `docs/tasks/T085-render-regression-fix/P1-requirements.md`（BDD）
4. 修改的前端文件（按 P2-design.md files_to_read）

## 评审重点

- DESIGN.md 一致性（§6 padding 32px/16px、§9 滚动架构、§10 a11y ≥44px）
- P5 自定义下拉组件交互状态 + 键盘操作
- P3 padding 恢复是否只影响 Markdown（不波及其他 viewer）
- P1 SVG isSvg/isRichRenderable 逻辑正确
- 无 AI Slop

## 门槛

- P4-review.md status: approved/rejected/needs-revision
- agent ≠ main
