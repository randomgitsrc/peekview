# P6 派发指引 — T085 verifier（验收）

## 目标

按 P1-requirements.md 的 11 条 BDD 逐条验收，产出 `P6-acceptance.md` + `P6-evidence/`。UI 任务需 vision 验证。

## BDD 清单

| BDD | 描述 | 验收方式 |
|-----|------|---------|
| BDD-1 | SVG 默认图片预览 | E2E + vision |
| BDD-2 | XML 仍树视图（防回归） | E2E |
| BDD-3 | SVG 不显示切换按钮 | E2E |
| BDD-4 | 源码视图可滚动 | E2E |
| BDD-5 | fallback 源码可滚动 | E2E |
| BDD-6 | 桌面 Markdown padding ≥32px | vision 测量 |
| BDD-7 | 移动 Markdown padding ≥16px | vision 测量 |
| BDD-8 | 底端滚动不抖动 | E2E |
| BDD-9 | per-page 真实点击 | E2E（真实点击，非 selectOption） |
| BDD-10 | per-page 触达目标 ≥44px | E2E 测量 |
| BDD-11 | per-page 键盘操作 | E2E |

## 约束

- BDD 结果只允许 PASS 或 FAIL
- P5 的 E2E 因 CDP 环境问题未跑通——P6 必须实跑
- 真实点击验证 per-page 下拉框（禁用 selectOption）
- 截图存入 P6-evidence/
- 日志末行 EXIT_CODE: <n>
...
- P5-test-results/：单元测试全绿

## 输入文件

1. `docs/tasks/T085-render-regression-fix/P1-requirements.md`
2. `docs/tasks/T085-render-regression-fix/P5-test-results/frontend-unit.md`

## 门槛

- P6-acceptance.md 11 BDD 全覆盖
- P6-evidence/ 非空
- vision blocker_count=0
- [PROD_NOT_TOUCHED]
