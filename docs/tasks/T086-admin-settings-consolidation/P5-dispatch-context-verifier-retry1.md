...
# P5 派发指引 — verifier（重试 #1，全量重跑）

## 目标

上一轮 P5（见 `P5-gate-diagnosis.md`）E2E 发现真失败（router.ts `/admin` 被 `/:slug` 拦截，未落到真正 404），已回退 P4 修复（新增显式 `/admin` → `NotFoundView` 路由，插在 `/:slug` 之前）。本次重新执行**全量** gate_commands.P5，不能只测上次失败的用例——T027 教训：修复可能引入回归，必须全量重跑。

## 约束

- **gate_commands（P2 已固化）**：
  ```yaml
  P5: "make test-frontend"
  P5_e2e: "E2E_SPEC=e2e/admin.spec.ts make debug-test"
  P6_typecheck: "make typecheck"
  ```
- 与上一轮相同：跑 E2E 前 `make debug-start` + `make debug-seed`，跑完 `make debug-stop`
- **全量重跑**，包括上一轮因 serial 级联被跳过的 10 条用例（`BDD-15`/`T086 BDD-08`/`T086 BDD-07`/`T086 BDD-11`/`T086 BDD-12` 等，见 `P5-gate-diagnosis.md`"影响的 BDD"节），这些必须真正执行并给出 PASS/FAIL，不能因为"上次没跑"就默认通过
- 特别关注修复的直接目标：`T086 BDD-9`（legacy BDD-14，非 admin 访问 `/admin` 应 404）必须真正通过，且断言 `.not-found` 可见
- 上一轮有 2 个 flaky（`BDD-01`/`BDD-02` 首屏渲染超时，重试后过），本轮若再次出现同类 flaky，记录但不算真失败；若变成稳定失败，判定为真失败
- 严禁触碰生产环境，E2E 必须走 debug backend :8888
- **签名要求**：`P5-test-results/unit.md`/`e2e.md` 必须含可 grep 匹配的 test runner 输出签名

## 上游关联

parent: P4-implementation-retry2.md（路由修复）
祖先: P5-gate-diagnosis.md（上一轮根因诊断）、P5-test-results/e2e.md（上一轮失败记录，可参考对照本轮是否真正修复）

## 输入文件

1. `docs/tasks/T086-admin-settings-consolidation/P5-gate-diagnosis.md`（上一轮根因）
2. `docs/tasks/T086-admin-settings-consolidation/P4-implementation-retry2.md`（本轮修复内容）
3. `docs/tasks/T086-admin-settings-consolidation/P5-test-results/e2e.md`（上一轮失败详情，供对照）
4. `docs/tasks/T086-admin-settings-consolidation/P2-design.md`（§5 gate_commands，§9 Definition of Done）

## 客观查证信息

- 主 Agent 已独立复核修复：`git diff` 确认只改了 `router.ts` 一处（新增一条路由），`vue-tsc --noEmit` 0 错误，`t080-admin-route-guard.test.ts` 7 passed 3 skipped 无回归
- `e2e/admin.spec.ts` 共 18 个 test()，chromium + Mobile Chrome 两个 project，预期约 36 条用例全部真正执行（不接受因级联被跳过）
