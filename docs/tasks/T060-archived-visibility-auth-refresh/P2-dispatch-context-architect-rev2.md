...
### 目标
根据 P2-review.md 的 7 项修改意见修订 P2-design.md。

### 约束
- 只修改 review 指出的问题，不扩展新范围
- 7 项问题优先级：2 HIGH + 2 MEDIUM + 2 LOW + 1 MEDIUM(移动端)
- 移动端考虑：至少声明范围（是否在本次任务范围内）
- a11y：至少为 auth 转换增加 aria-live 通知设计
- 竞态/失败：声明策略（不需要实现，P4 实现）

### 上游关联
- P2-review.md 7 项问题：
  1. HIGH: 重载请求竞态未处理
  2. HIGH: 重载失败 fallback 未定义
  3. MEDIUM: 初始化 loading→authenticated 误触发
  4. MEDIUM: Auth 转换后无 a11y 通知
  5. MEDIUM: 移动端延迟和闪烁未考虑
  6. LOW: watcher flush 模式未声明
  7. LOW: Archived tab 重置后焦点管理

### 输入文件
- docs/tasks/T060-archived-visibility-auth-refresh/P2-design.md（被修订对象）
- docs/tasks/T060-archived-visibility-auth-refresh/P2-review.md（评审意见）
</dispatch_guide>
...
- 环境状态：纯文档修订
- 关键标识：7 项修改意见 #1-#7
- 查证结果：P2-review.md 存在，status: needs-revision
</objective_info>
