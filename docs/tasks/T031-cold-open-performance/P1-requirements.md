...
skipped:
  P7: 改动集中在 frontend-v3 单一包内，无跨包一致性问题
coupling_checklist: [api-schema: checked, data-model: checked, cross-package: checked]
跳过风险: 低——所有改动限于 frontend-v3/src/ 内 6 个文件，无跨包接口变更（不改后端 API 契约、不改数据库 schema、MCP/CLI 不受影响）。若实现中发现新隐含需求涉及后端则恢复 P7。
```
