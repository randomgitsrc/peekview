...
### 目标
评审 P4 实现代码是否符合 P2-design.md 方案设计，产出 P4-review.md

### 约束
- 只审不写——不直接改代码
- 对照 P2-design.md 的 6 个子项逐一检查实现是否匹配
- 检查是否有范围外改动
- 检查 a11y（username span 的 role/tabindex/keydown）

### 输入文件
- docs/tasks/T031-cold-open-performance/P2-design.md（方案）
- frontend-v3/src/components/EntryCard.vue（实现）
- frontend-v3/src/components/EntryListRow.vue（实现）
- frontend-v3/src/views/EntryListView.vue（实现）
- frontend-v3/src/views/EntryDetailView.vue（实现）
- frontend-v3/src/stores/entry.ts（实现）
- frontend-v3/src/views/LandingView.vue（实现）
</dispatch_guide>
