...
产出 P8-release.md 发布准备文件。**不执行** bump-version / git commit / git tag。

## 输入

- P2-design.md packages 声明：`peekview (backend)`
- 当前版本：peekview 0.10.1, mcp_server 0.9.3（VERSIONS.json）
- CHANGELOG.md [Unreleased] 区域

## 执行步骤

1. 读取 P2-design.md packages 声明
2. 读取 VERSIONS.json 确认当前版本
3. 读取 CHANGELOG.md [Unreleased] 区域
4. 确定 bump_type：T068 新增 PATCH /auth/me 端点 + 前端新页面 → minor（新功能）
5. 新版本号：0.10.1 → 0.11.0
6. 更新 CHANGELOG.md：[Unreleased] → [0.11.0]
7. 产出 P8-release.md

## CHANGELOG 条目

从 git log 提取 T068 相关改动：
- 新增 PATCH /api/v1/auth/me 端点（display_name 编辑）
- 新增 /settings 页面（Profile/Security/API Keys 三 tab）
- 修复 API key 创建 500 错误（apikey_service.py is None → .is_(None)）
- 修复 change-password 旧密码错误返回 401 → 400
- 旧路由 /settings/apikeys → /settings?tab=apikeys 重定向
- auth guard：未登录访问 /settings 重定向到 landing

## 临时资源清单

- Debug backend: http://127.0.0.1:8888 (PID: 3928180)
- 临时数据: /tmp/peekview-debug/
- 临时截图: /tmp/t068-p6r2-screenshots/

## 输出

docs/tasks/T068-account-settings/P8-release.md
