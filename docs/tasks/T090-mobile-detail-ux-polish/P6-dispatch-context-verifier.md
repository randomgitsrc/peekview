...
# 派发指引 — T090 P6 验收（用户视角，逐条实跑 12 条 BDD）

## 目标

把 P1-requirements.md 的 12 条 BDD **逐条实跑**，产出 P6-acceptance.md + P6-evidence/。这不是重复 P5 的测试执行——P5 问"测试过了吗"，P6 问"用户要的行为真的做到了吗"，请按你自己角色定义（P6 验收模式）的流程独立操作，不要直接照抄 P5 的 E2E 通过结论当作 P6 结论。

## 上游关联

- P1-requirements.md（approved，12 条 BDD，`#### BDD-1` 至 `#### BDD-12`，含 `#### BDD-8` 后追加的 `[BASELINE_CHANGE]` 澄清注释——验收 BDD-8 时按澄清后的口径判定，即"单侧留白相对基线40px缩减≥75%"，不是两侧相加）
- P2-design.md 第 4 节 data-testid 清单（截图/断言定位依据）
- P4-implementation.md（实现改动清单，含哪些文件改了什么）
- P5-test-results/（技术验证已确认 12/12 E2E 通过 + vitest 全绿，可作为参考起点，但 P6 需要你自己用 Playwright 实跑产出用户视角证据，不能只引用 P5 的结论了事）
- `ui_affected: true`，capability_requirements 声明 `browser-vision: available`（vision-analyst / playwright-cdp skill）、`mobile-viewport-emulation: available`

## 环境隔离（强制）

本任务的环境约束见 P0-brief.md 的 env_constraints 字段。debug backend 应已在 127.0.0.1:8888 运行（`curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:8888/` 应返回 200），若未运行用 `make debug-quick` 启动。测试数据 entry（`t090-long-markdown`/`t090-long-code`/`t090-md-multifile`/`t090-py-multifile`）应已存在，可复用（不要用 CLI 创建）。严禁触碰生产 :8080。

## 12 条 BDD 的证据类型建议（你可自行判断调整，但需说明理由）

**操作类（需要截图，且截图必须互不相同，md5 去重）**：
- BDD-4/5：底部操作栏固定可见性 + 两种可视高度不遮挡——截图移动端 viewport 滚动到顶/中/底三个位置的底部栏状态，以及两种视口高度下的状态
- BDD-6/7：底部栏按钮功能（file-tree/toc/source-toggle/wrap/copy/overflow）——每个交互动作前后各截一张（抽屉打开、wrap 切换等）
- BDD-9：375px 极小屏——截图确认无横向溢出
- BDD-10/12：桌面端不出现移动端专属 UI——桌面 viewport 截图

**查询类（可用断言日志代替截图，断言值是唯一证据）**：
- BDD-1/2/3：滚动跳变检测——这是"位移是否连续"的数值判定，截图很难体现"连续性"，建议用采样数据写入断言日志（`getBoundingClientRect` 序列 + 位移量计算），不强制截图
- BDD-8/11：边距数值缩减比例——用 `getBoundingClientRect` 测量值 + 计算公式写入断言日志，不强制截图（但截一张辅助也可以）

## 执行流程（按你自己角色定义的 P6 验收模式 6 步流程）

1. 用 Playwright 实跑每条 BDD，操作类的截图存入 `docs/tasks/T090-mobile-detail-ux-polish/evidences/`（同时复制/软链到 `P6-evidence/screenshots/` 供 gate 检查），查询类的断言数据写入日志文件存入 `P6-evidence/`
2. 截图完成后，按你角色定义派发 vision-analyst（用 Agent 工具，subagent_type 指定为一个能理解并产出 vision-analyst YAML 结构的角色；若你的执行环境不支持从子任务再派发子任务，改为自己按 vision-analyst 角色文件（`{agate_root}/assets/execution-roles/vision-analyst.md`）的 YAML 结构规范，直接产出结构化视觉分析 YAML，效果等价——但必须是你**实际观察截图**后填写的结构化描述，不能凭空编造）
3. 产出 `docs/tasks/T090-mobile-detail-ux-polish/P6-vision-{timestamp}.yaml`（vision_analysis 结构，含每条操作类 BDD 的 bdd_results + anomalies + summary.blocker_count）
4. 若 blocker_count > 0：不能只用程序化指标反驳，需追查根因（curl -I / DevTools / API 日志），追查结果写入 P6-acceptance.md
5. 汇总所有结果，写 P6-acceptance.md（每条 `- PASS BDD-N: 描述 (证据路径)` 或 `- FAIL BDD-N: 描述 (证据路径)`，格式见下方"gate 格式契约"）
6. **返回前必须自跑三个预检脚本**：
   ```bash
   bash ~/.agate/scripts/check-p6-format.sh --fix docs/tasks/T090-mobile-detail-ux-polish/P6-acceptance.md
   bash ~/.agate/scripts/check-p6-evidence.sh docs/tasks/T090-mobile-detail-ux-polish
   bash ~/.agate/scripts/check-p6-provenance.sh docs/tasks/T090-mobile-detail-ux-polish
   ```
   全部 exit 0 才返回给主 Agent；exit 非 0 则按报错修复后重试（最多 2 轮），仍失败则如实返回错误信息，不要隐瞒。

## gate 格式契约（精确正则，必须严格遵守）

- 结果行（PASS/FAIL）格式：`^\s*- (PASS|FAIL)\b`（行首，`-` 后空格，PASS/FAIL 大写）
- 总结行**不能**用行首 `- PASS`/`- FAIL`（用 `**Summary**: PASS: 12, FAIL: 0` 这种格式）
- 每条 PASS 后必须引证据路径：`- PASS BDD-1: 描述 (P6-evidence/xxx.png)` 或 `(P6-evidence/xxx.log)`
- 操作类 BDD 的 PASS 若引用截图，必须同时含 vision 引用：`(screenshots/b04.png) (vision: vision-reports/b04.yaml)` 或直接引用 P6-vision-{timestamp}.yaml 中对应条目
- 查询类 BDD 的 PASS 可只引用断言日志，不强制 vision

## 门槛（什么算完成）

- P1 全部 12 条 BDD 各有一条 PASS 或 FAIL 结果（无遗漏、无中间态）
- 操作类 BDD 截图互不相同（md5 去重）
- P6-evidence/ 非空，每个文件被至少一条 PASS/FAIL 引用
- vision YAML 的 blocker_count == 0（若 >0 需在 P6-acceptance.md 写明追查过程）
- 三个预检脚本自跑 exit 0
