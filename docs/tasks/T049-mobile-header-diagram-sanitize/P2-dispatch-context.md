# P2 Dispatch Context — T049 mobile-header-diagram-sanitize
...
- P1 已识别的 23 条 BDD + 22 项隐含依赖全部列在 P1-requirements.md
- **4 个子域**：header-shrink（纯前端）、diagram-sanitize（前端新模块）、diagram-config（后端+CLI）、diagram-error-ui（前端组件重构）
- risk_level: medium（配置不影响服务，清洗规则可逆）
- 已知：mermaid ^10.9.0 支持 suppressErrors 选项
- 已知：CLI config_set 已有 "Restart service to apply" 模式
- 已知：前端 PeekAPI 类目前无通用 config 获取方法（只有 captcha/limits 端点）
