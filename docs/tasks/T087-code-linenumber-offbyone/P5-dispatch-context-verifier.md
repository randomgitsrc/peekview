...
# P5 dispatch-context — T087 verifier

## 目标

执行 P2 固化的 gate_commands，产出 P5-test-results/。T087 是 UI 任务，必须跑 P5_e2e。

## gate_commands（从 P2-design.md 读取，主 Agent 已确认）
...
1. **P5 单元测试**：跑 `cd frontend-v3 && ./node_modules/.bin/vitest run --reporter=dot 2>&1 | tail -30`
   - 期望：全量 vitest 全绿（含 T087 新增 useShiki.linenumber.spec.ts 9 测试 + 现有全部）
   - 产出：P5-test-results/unit.md（标注 passed/failed 数量 + failed 清单）

2. **P5 typecheck**：跑 `cd frontend-v3 && ./node_modules/.bin/vue-tsc --noEmit`
   - 期望：exit 0（CI 强制）
   - 产出：P5-test-results/typecheck.md

3. **P5_e2e**：跑 `cd frontend-v3 && E2E_SPEC=e2e/viewer.spec.ts make debug-test`
   - 需要 debug backend :8888（make debug-test 自带启动）
   - 期望：viewer.spec.ts 全绿（BDD-8/9/10 回归）
   - 产出：P5-test-results/e2e.md（Playwright 实跑结果 + 截图路径）

## 全量测试要求

P5 阶段应运行全量测试套件（含非本任务测试）。发现预存失败时：
- 在 P5-test-results/unit.md 标注"预存失败：X（与本次改动无关）"
- 主 Agent 判断是否登记 known-failures.md

## 判定规则

- exit 0 + failed=0 → 通过
- exit ≠0 或 failed>0 → 主 Agent 判定真 bug / 环境问题 / flaky

## PROD_TOUCHED 检查

E2E 走 make debug-test（:8888 隔离），严禁触碰 :8080 生产。未触生产写 [PROD_NOT_TOUCHED]。

## 产出规格

- `P5-test-results/unit.md`：failed 数量 + failed 清单
- `P5-test-results/fail-list.txt`：failed 测试 id 逐行列出（无失败时为空文件）
- `P5-test-results/typecheck.md`：typecheck 结果
- `P5-test-results/e2e.md`：E2E 结果 + 截图路径

## 输入文件

- `docs/tasks/T087-code-linenumber-offbyone/P2-design.md`（gate_commands 源）
- `docs/tasks/T087-code-linenumber-offbyone/P4-implementation.md`（实现摘要）

## 输出

产出文件写入 `docs/tasks/T087-code-linenumber-offbyone/P5-test-results/` 目录。
