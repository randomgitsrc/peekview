...
# P6 dispatch-context — T087 verifier (acceptance)

## 目标

逐条对照 P1 的 10 BDD 做验收，产出 P6-acceptance.md + P6-evidence/。UI 任务必须 vision-analyst 分析截图（blocker_count=0）。

## BDD 验收清单（10 条，逐条 PASS/FAIL）

| BDD | 描述 | P5 证据 | 验收方式 |
|-----|------|---------|---------|
| BDD-1 | 末尾换行 `"a\nb\n"` → 2 行号 + 2 .line | P5 evidence t087-tc001-trailing-newline.png + E2E PASS | 截图 + vision |
| BDD-2 | 无换行 `"a\nb"` → 2+2 | P5 evidence t087-tc002-no-trailing-nl.png + E2E PASS | 截图 + vision |
| BDD-3 | 单行 `"a"` → 1+1 | vitest 单测 PASS（useShiki.linenumber.spec.ts BDD-3） | 断言记录 |
| BDD-4 | 空文件 `""` 不渲染 | vitest 单测 PASS（CodeViewer 短路） | 断言记录 |
| BDD-5 | 仅换行符 `"\n"` → 1+1 | P5 evidence t087-tc003-newline-only.png + E2E PASS | 截图 + vision |
| BDD-6 | 中间空行+末尾换行 `"a\n\n"` → 2+2 | P5 evidence t087-tc004-mid-empty.png + E2E PASS | 截图 + vision |
| BDD-7 | Markdown 代码块对齐 | P5 evidence t087-tc005-md-block.png + E2E PASS | 截图 + vision |
| BDD-8 | Markdown 多代码块不回归 | vitest 全量 1226 passed（含 useMarkdown 测试） | 断言记录 |
| BDD-9 | wrap 对齐 | P5 evidence t087-tc006-nowrap.png + E2E PASS | 截图 + vision |
| BDD-10 | 源码视图切换 | E2E T087 专用 spec 覆盖 CodeViewer 路径 | 断言记录 |

## P5 已有证据（复用）

P5-test-results/evidence/ 含 6 张截图：
- t087-tc001-trailing-newline.png（BDD-1）
- t087-tc002-no-trailing-nl.png（BDD-2）
- t087-tc003-newline-only.png（BDD-5）
- t087-tc004-mid-empty.png（BDD-6）
- t087-tc005-md-block.png（BDD-7）
- t087-tc006-nowrap.png（BDD-9）

这些截图可复制到 P6-evidence/ 并用 vision 分析。

## vision-analyst 任务

对 6 张截图分析行号列与高亮列的视觉对齐：
- 行号数 == 高亮 .line 数 == 预期逻辑行数
- 无尾部空行号、无尾部空 .line
- 行号列与高亮列逐行对齐
- blocker_count 必须 = 0

用 vision-engine skill 或 vision-analyst 角色分析。每张截图产出 vision-reports/{bdd}.yaml。
...
- 验收用 P5 已有证据（不重新跑 E2E，除非必要）
- 若需重新截图，走 debug backend :8888（make debug-quick），严禁 :8080
- 未触生产写 [PROD_NOT_TOUCHED]

## 输入文件

- `docs/tasks/T087-code-linenumber-offbyone/P1-requirements.md`（10 BDD）
- `docs/tasks/T087-code-linenumber-offbyone/P5-test-results/`（unit.md + e2e.md + evidence/）
- `docs/tasks/T087-code-linenumber-offbyone/P4-implementation.md`

## 输出

- `docs/tasks/T087-code-linenumber-offbyone/P6-acceptance.md`（10 BDD 逐条 PASS/FAIL + 总结）
- `docs/tasks/T087-code-linenumber-offbyone/P6-evidence/`（截图 + 断言文件）
- `docs/tasks/T087-code-linenumber-offbyone/vision-reports/`（vision YAML，blocker_count=0）
