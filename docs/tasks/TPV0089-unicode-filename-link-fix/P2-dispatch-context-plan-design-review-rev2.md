...
复核修订后的 P2-design.md（architect 已按上轮评审意见修订 R1/R2/R3），确认全部修订落地后批准。

## 上轮评审意见（修订依据）

`docs/tasks/TPV0089-unicode-filename-link-fix/P2-review.md`（needs-revision，2 必改 + 1 建议）

## 修订内容核对清单

1. **R1**：候选方案 A 伪代码 `if (hit = matchRef(...))` → `if (hit !== null)` 显式 null 判断（matchRef 返回 `number | null`，fileId=0 不再被 falsy 误判）
2. **R2**：「12 条 BDD」→「13 条」；§3 E2E 行标签改为「BDD-10/11/12/13 E2E」
3. **R3**：gate_commands.P5 改为 `make test-frontend`
4. frontmatter 机器字段未被破坏，选型仍为方案 A

## 上游关联

- 输入文件（必读）：
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P2-design.md`（修订后被评审对象）
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P2-review.md`（上轮评审意见，核对依据）
...
`docs/tasks/TPV0089-unicode-filename-link-fix/P2-review.md`（覆盖写，gate 只认此文件名）
...
type: review
parent: P2-design.md
trace_id: TPV0089-P2-review-rev2-20260811
status: draft
created: 2026-08-11
...
- 关键标识：P2-design.md 修订轮完成（R1 伪代码 null 判断 / R2 13 条计数 / R3 gate_commands.P5 改 make test-frontend）
- 查证结果：需逐一核对 R1/R2/R3 落地情况
</objective_info>
