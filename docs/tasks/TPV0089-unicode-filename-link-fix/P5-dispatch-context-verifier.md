# P5 Dispatch Context — verifier

## 任务目标

执行 TPV0089 的技术验证（gate_commands.P5，P2 已固化），产出 `P5-test-results/`。验证范围：前端全量单测 + typecheck + E2E（ui_affected: true）。

## 上游关联

- 输入文件（必读）：
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P0-brief.md`（环境约束）
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P2-design.md`（gate_commands 声明 + env_constraints）
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P1-requirements.md`（13 BDD，P5 只验证技术正确性，用户视角验收留 P6）
  - `AGENTS.md`（项目约定、测试注意事项）

## gate_commands（P2 已固化，必须从此字段执行，不得修改/跳过）
...
1. **P5 单测全量**：`make test-frontend`（前端全部 vitest 单测，非 watch）
2. **P5_typecheck**：`make typecheck`（vue-tsc --noEmit，CI 强制）
3. **P5_e2e**：`make debug-test`（Playwright E2E 全量；debug backend :8888 已启动）
   - 若全量 E2E 超时或部分失败，优先跑 `E2E_SPEC=e2e/unicode-filename-link.spec.ts make debug-test`（本任务新增的 E2E，覆盖 BDD-10~13），并记录全量情况
4. **P5 全量后端测试**（建议）：`make test-quick`（backend pytest，确认无跨端回归）

## 环境状态

- debug backend :8888 已启动（make debug-quick 完成，22 entries + 本任务 fixture unicode-filenames 需确认已 seed）
- 若 unicode-filenames 未 seed：跑 `make debug-seed`（从 scripts/seed-data/ 加载，含新 fixture）
- CDP Chrome :18800 可用
- 严禁触碰生产 :8080 / ~/.peekview/

## 产出要求

1. `docs/tasks/TPV0089-unicode-filename-link-fix/P5-test-results/unit.md` — 前端单测结果（failed 计数 + 输出签名）
2. `docs/tasks/TPV0089-unicode-filename-link-fix/P5-test-results/fail-list.txt` — failed 测试 id 逐行（无失败则为空文件）
3. `docs/tasks/TPV0089-unicode-filename-link-fix/P5-test-results/e2e.md` — E2E 结果（Playwright 实跑 + 截图路径，本任务 E2E 结果）
4. `docs/tasks/TPV0089-unicode-filename-link-fix/P5-test-results/typecheck.md` — typecheck 结果
5. `docs/tasks/TPV0089-unicode-filename-link-fix/P5-test-results/backend.md` — 后端 pytest 结果（建议项）
6. 若发现预存失败（与本次改动无关）：登记到 `docs/tasks/TPV0089-unicode-filename-link-fix/known-failures.md`（从 `~/.agate/assets/templates/known-failures-template.md` 拷贝）

日志文件末行必须是 `EXIT_CODE: <n>`（0 = 成功）。

## 约束

- 所有命令从 gate_commands 读取，不得自行定义或降级
- 命令不存在或跑不通 → 标 `[CAPABILITY_GAP]`，不降级为目测
- 环境隔离：只走 debug backend :8888，严禁 :8080 生产
- 状态标记：触发写 `[PROD_TOUCHED] {描述}`，未触发写 `[PROD_NOT_TOUCHED]`

## 返回给主 Agent

两行：P5-test-results/ 路径 + 一句话摘要（failed 计数，不超过 30 字）
...
<objective_info>
- 环境状态：debug backend :8888 已启动（22 entries + unicode-filenames fixture）；CDP Chrome :18800 可用
- 关键标识：path-map.test.ts 51/51 全绿（P4 自查）；gate_commands.P5 = make test-frontend；P5_typecheck = make typecheck；P5_e2e = make debug-test
- 查证结果：P1 基线 13 BDD；P2 gate_commands 已固化
</objective_info>
