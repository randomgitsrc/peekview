# P2 Dispatch Context — architect（修订轮 2）

## 修复目标

上轮修订引入新缺陷（P2-review.md 第二轮评审确认）：伪代码 raw 优先分支的 `hit = matchRef(normalized, pathMap)` 赋值行被误删，只留 `if (hit !== null) return hit`，`hit` 悬空未赋值。
...
- 上轮产出：`docs/tasks/TPV0089-unicode-filename-link-fix/P2-design.md`（当前状态）
- 评审意见：`docs/tasks/TPV0089-unicode-filename-link-fix/P2-review.md`（第二轮，R1 是唯一剩余项）

## 约束

- 只做上述单点修复，不得引入其他改动
- 保持 frontmatter 不变、选型不变（方案 A）、其他已确认落地的修订（R2/R3）不动

## 产出要求

更新 `docs/tasks/TPV0089-unicode-filename-link-fix/P2-design.md`（覆盖写）
...
- 关键标识：P2-design.md 候选方案 A 伪代码 resolvePath 四步流程；R1 需补回 hit = matchRef(normalized, pathMap)
- 查证结果：第二轮评审唯一剩余项为 R1 伪代码（R2/R3/frontmatter 已确认落地）
</objective_info>
