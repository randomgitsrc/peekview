# P6 Dispatch Context — verifier

## 任务目标

执行 TPV0089 的 P6 验收：对照 P1 的 13 条 BDD 逐条实跑验证，产出 `P6-acceptance.md` + `P6-evidence/`。本任务是 UI 行为修复（ui_affected: true），必须 Playwright CDP 实跑 + 截图 + vision 引用。

## 上游关联

- 输入文件（必读）：
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P0-brief.md`（环境约束）
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P1-requirements.md`（13 BDD——P6 验收依据，注意 BDD-11 已有 [BASELINE_CHANGE from P5]）
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P2-design.md`（方案 A + UI 测试选择器）
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P5-test-results/`（P5 已验证的技术正确性 + 已知失败）
  - `AGENTS.md`（项目约定）

## 环境状态

- debug backend :8888 已启动，unicode-filenames fixture 已 seed（23 entries）
- **静态文件已是最新**（P5 已跑 make build-frontend 反映 P4 修复；若怀疑可重跑）
- CDP Chrome :18800 可用
- 严禁触碰生产 :8080 / ~/.peekview/

## BDD 验收清单（13 条）

### 单元级（BDD-1~9）——可直接引用 P5 已验证结果（path-map.test.ts 51/51），但需在 P6-acceptance.md 记录

- BDD-1 中文 path 解析 / BDD-2 中文 basename / BDD-3 日文 / BDD-4 重音 / BDD-5 空格 / BDD-6 畸形 % 防御 / BDD-7 字面 % decode-once / BDD-8 字面 % raw 命中 / BDD-9 英文不回归

### 端到端（BDD-10~13）——Playwright CDP 实跑 + 截图 + vision 引用

- **BDD-10** 中文图片实际渲染（src 改写 + naturalWidth>0 + 截图无裂图）
- **BDD-11** 中文链接可点击打开（内容区显示文件内容、无 404——[BASELINE_CHANGE from P5]：不绑定 URL 变化）
- **BDD-12** 日文/重音/空格图片渲染
- **BDD-13** 英文不回归

## 执行步骤

1. 确认 debug backend :8888 + fixture + 最新 static
2. 跑 `E2E_SPEC=e2e/unicode-filename-link.spec.ts make debug-test`（12 用例 = 6 BDD × 2 viewport）
3. 截图已由 spec 写入 `docs/tasks/TPV0089-unicode-filename-link-fix/evidences/`——P6 需将其整理/拷贝到 `P6-evidence/`（截图 >1KB，操作类 BDD 截图 md5 互不相同）
4. 用 vision-engine 分析关键截图（中文图片渲染无裂图等），产出 vision-reports/
5. 逐条写 P6-acceptance.md（PASS/FAIL 二值，每条 PASS 带证据引用）

## 产出要求

1. `docs/tasks/TPV0089-unicode-filename-link-fix/P6-acceptance.md`
   - frontmatter：pass: / fail: / ui_affected: true
   - 每条 BDD 行首 `- PASS` 或 `- FAIL` + 证据引用
   - UI 类 PASS 含 vision 引用 `(vision: vision-reports/bxx.yaml)`
2. `docs/tasks/TPV0089-unicode-filename-link-fix/P6-evidence/`（非空，截图/断言日志）
3. `docs/tasks/TPV0089-unicode-filename-link-fix/vision-reports/`（vision 分析结果）
...
- 每条 BDD 结果只允许 PASS 或 FAIL（不允许中间态）
- 无证据的 PASS = gate 不通过
- 截图必须互不相同（md5 去重），>1KB
- 状态标记：触发写 `[PROD_TOUCHED] {描述}`，未触发写 `[PROD_NOT_TOUCHED]`

## 返回给主 Agent

两行：P6-acceptance.md 路径 + 一句话摘要（X/13 PASS，不超过 30 字）
...
<objective_info>
- 环境状态：debug backend :8888 已启动（23 entries 含 unicode-filenames）；CDP Chrome :18800 可用；static 已是最新（P5 build-frontend）
- 关键标识：E2E spec frontend-v3/e2e/unicode-filename-link.spec.ts（12 用例）；P5 已验证 12/12 通过；P1 13 BDD（BDD-11 有 BASELINE_CHANGE）
- 查证结果：图片渲染 bug 本体在 P5 已验证通过；P6 需产出真实证据 + vision 引用
</objective_info>
