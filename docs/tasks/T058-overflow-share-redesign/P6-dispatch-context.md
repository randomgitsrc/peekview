...
- debug server 状态：运行中，http://127.0.0.1:8888，版本 0.8.0
- P5-test-results/ 已存在：unit.md (876 PASS), e2e.md (28 E2E PASS), 6 screenshots
- P5 发现 2 个 bug：
  1. BUG-1 (Medium): ShareDialog.handleClickOutside closes popover on trigger click — 在真实浏览器中不影响用户体验
  2. BUG-2 (Low/Design): showShareButton only for private entries — 公开条目无分享 UI
- t058-share-redesign.e2e.spec.ts 已创建（28 tests）
- Playwright CDP 可用：Chrome :18800
- Vision 分析可用：~/.claude/skills/vision-analyzer/scripts/vision-analyze.py
- E2E spec 的 base URL 必须设 BASE_URL=http://127.0.0.1:8888
- BDD-05 badge bug 已修复：EntryDetailView watch showShareButton + immediate:true

## 任务上下文（主 Agent 从 P0-brief + gate + 摘要积累）

- 目标：P6 验收——把 P1 的 26 条 BDD 逐条实跑，翻译成人话
- 关注点：
  1. ui_affected: true，必须 Playwright 实跑 + 截图 + vision 分析
  2. P1 共 26 条 BDD（BDD-01 到 BDD-24，含 10a/10b/10c），每条都要有结果
  3. 每条 PASS 必须有证据文件引用，UI 类必须有 vision YAML 引用
  4. BUG-1 和 BUG-2 需在验收中判定是否影响 BDD 结果
  5. 先验证功能（用户视角），再满足 gate 格式
- 已知风险：BUG-1 handleClickOutside 在 Playwright locator.click() 下会误关，但真实浏览器不受影响
- 上游结构化字段：
  - packages: [frontend-v3]
  - domains: [frontend, overflow-redesign, share-redesign]
  - ui_affected: true
  - gate_commands.P6: "cd frontend-v3 && E2E_GUARD_ENABLED=1 npx playwright test e2e/t058-share-redesign.e2e.spec.ts e2e/t052-header-redesign.e2e.spec.ts e2e/test_t057_ui_polish.spec.ts --reporter=line 2>&1 | tail -30"

## verification_env

- debug backend: http://127.0.0.1:8888（已运行，/tmp/peekview-debug/ 数据目录）
- Playwright: npx playwright test（chromium，E2E_GUARD_ENABLED=1，BASE_URL=http://127.0.0.1:8888）
