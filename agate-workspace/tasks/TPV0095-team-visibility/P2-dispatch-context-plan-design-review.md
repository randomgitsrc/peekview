...
type: review
parent: P2-design.md
trace_id: TPV0095-P2-plan-design-review-20260902
status: draft
agent: plan-design-review
---

# P2 派发上下文 — plan-design-review（TPV0095 team-visibility）
...
独立评审 `P2-design.md` 的**前端/UI 方案**（explore 5-tab、状态×URL 矩阵、/teams 管理页、badge/toggle 守卫、移动端 a11y、DESIGN.md 修订），产出 `P2-review-design.md`。

### 约束

1. **角色定义**：`/home/kity/.agate/assets/review-roles/plan-design-review.md`。
2. **评审重点**（frontend 域 + ui_affected + UX）：
   - **UI 设计节完整性**：渲染形态声明（layout）+ 适用维度（布局结构/交互行为）+ 布局/交互/视觉 checklist 是否符合 P2 卡片规格
   - **5-tab 互斥逻辑**（BDD-38）：All 激活判定 `!currentTeam && view!=='teams'`、Teams tab 激活 `view==='teams'`、四维互斥 setFilter 入口、URL 表达
   - **状态×URL 矩阵**（BDD-41）：单一不可用态两文案区分、URL 恢复三处收敛单一 restore、非法组合静默丢弃
   - **/teams 管理页**（BDD-42）：双入口 DOM、owner 全操作/成员退出/三错误文案互异、删除确认 alertdialog、live region
   - **卡片 toggle 守卫 + badge**（BDD-39/40）：teamId 隐藏 toggle + store 守卫、badge team 变体（禁 emoji/hex）
   - **detail 状态标签**（BDD-44 新增）：EntryDetailHeader/EntryMetaTagsBar 三态区分
   - **移动端 + a11y**（BDD-43）：tablist/aria-selected、横滚 + ≥44px、FilterChip aria-label 参数化
   - **候选方案 A(b) 合理性**：前端状态留视图层 vs store 承载的权衡
   - **UI 测试选择器**：data-testid 清单是否覆盖 P3 定位需要
3. **评审结论**：Header status 初始 draft → `approved` / `rejected` / `needs-revision`。结论引用具体设计节/BDD 锚点。**BLOCKER → rejected**。
4. **只读原则**：不修改 P2-design.md 或其它文件（除产出 P2-review-design.md + P2-review-progress.md）。
5. **产出路径**：`agate-workspace/tasks/TPV0095-team-visibility/P2-review-design.md`。
6. **环境隔离**：禁碰生产 :8080 / ~/.peekview/。状态标记二值。

### 上游关联

- P2-design.md：§5 前端设计 + §5.5 /teams + §5.6 移动端/a11y + UI 设计节。SCOPE+ detail 标签已由主 Agent 采纳 → BDD-44 增补。
- P1-requirements.md：BDD-38~44 前端验收线（含 UX 类别标注）。

### 输入文件

1. `agate-workspace/tasks/TPV0095-team-visibility/P2-design.md`（评审对象，重点 §5 + UI 设计节）
2. `agate-workspace/tasks/TPV0095-team-visibility/P1-requirements.md`（BDD-38~44）
3. `docs/design-notes/team-visibility.md`（§8 前端设计权威源）
4. `/home/kity/.agate/assets/review-roles/plan-design-review.md`（角色）
...
`FILE=P2-review-design.md agate-md-field-set` 维护 Header。

### 返回

两行：① P2-review-design.md 路径；② Status: approved/rejected/needs-revision + BLOCKER 摘要（若有）。
</dispatch_guide>
