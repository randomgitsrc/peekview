...
trace_id: TPV0095-P6-verifier-frontend-20260902
status: draft
agent: verifier
---

# P6 派发上下文 — verifier V2（frontend UI 域验收，TPV0095）
...
P6 验收（frontend UI 域）：把 P1 的 **BDD-38~44**（前端 UI + UX）用 Playwright 实跑 + 截图验证，产出每条 PASS/FAIL + 证据。你的产出是 P6-evidence/frontend/ 下的结果片段 + 截图，供汇总 verifier 整合 P6-acceptance.md。

### 约束

1. **角色定义**：`/home/kity/.agate/assets/execution-roles/verifier.md`（P6 模式）。
2. **BDD 逐条实跑**（P1-requirements.md 权威）：
   - BDD-38（5-tab 互斥）、BDD-39（badge 不叠 private）、BDD-40（toggle 隐藏）、BDD-41（团队不可用态）、BDD-42（/teams 双入口 + owner 操作 + 成员退出）、BDD-43（移动端 tab 横滚 + ≥44px + 键盘）、BDD-44（detail 三态标签）
3. **实跑方式**：
   - E2E spec 已覆盖大部分（team-visibility.spec.ts BDD-38~41/43 + teams-page.spec.ts BDD-42，P5 后 26/26 全绿）——这些 BDD 以 E2E 运行结果为证据（重跑两 spec 记录）。
   - **BDD-44（detail 三态）**：E2E 未必覆盖——用 Playwright 脚本实跑（登录 → 打开 team entry 详情 → 断言状态标签文案）+ 截图。
   - 每条 UI 类 PASS 需**截图证据**（P6-evidence/frontend/screenshots/）+ 运行证据。
4. **截图规范**：desktop 1280×800 + mobile 390×844 两档（P3 viewport 规范）；操作类 BDD 截图互不相同（md5 去重）；截图前确认过渡完成（waitForTimeout 或 opacity 检查）。
5. **vision 证据**：P1 vision status=available → 每条 UI 类 PASS 附 vision 分析（vision-engine skill 或 vision-analyst）——若无法调用，截图 + 人工复核记录（manual-review 引用）。先自查 vision 能力，不能则报告 [CAPABILITY_GAP]。
6. **证据格式**：`P6-evidence/frontend/results.md` 写 `- PASS BDD-NN: 描述 (screenshots/xxx.png) (vision: ...)` 或 FAIL 行（不进 gate——gate 只读汇总 P6-acceptance.md）。
7. **P6 BDD 二值**：只允许 PASS/FAIL。
8. **环境**：debug server :8888 已持住；Playwright CDP :18800（connectOverCDP，try/finally page.close + process.exit(0)，不 browser.close）。严禁碰 :8080 生产 / 真实数据。
9. **自查≠gate**：你产出 P6-evidence/frontend/results.md，主 Agent 派汇总 verifier 整合 + 亲自验 gate。

### 上游关联

- P5 E2E 26/26 全绿（team-visibility.spec + teams-page.spec）
- P1 BDD-38~44（frontend UI 线，UX 类别后缀）
- P2 §5.7 data-testid 清单

### 输入文件

1. `P1-requirements.md`（BDD-38~44）
2. `P2-design.md`（§5 + UI 设计节 + data-testid）
3. `frontend-v3/e2e/team-visibility.spec.ts` + `teams-page.spec.ts`（E2E 锚）
4. 既有 E2E 结果（P5-test-results/e2e.md）

### 产出

- `P6-evidence/frontend/results.md`（BDD-38~44 PASS/FAIL 行 + 证据引用）
- `P6-evidence/frontend/screenshots/`（截图）+ test-output.log

### 返回

两行：① P6-evidence/frontend/ 路径；② 摘要（BDD-38~44 X/Y PASS，≤40 字）。
</dispatch_guide>
