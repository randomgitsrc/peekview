...
# P2 派发上下文 — plan-eng-review（TPV0095 team-visibility）
...
独立评审 `P2-design.md` 的**工程/架构方案**（后端数据模型、迁移、权限收敛、API、CLI、MCP、性能），产出 `P2-review-eng.md`。

### 约束

1. **角色定义**：`/home/kity/.agate/assets/review-roles/plan-eng-review.md`。
2. **评审重点**（backend 域 + high risk + schema 变更 + 权限收敛）：
   - **迁移正确性**：teams/team_members 建表 + entries.team_id ALTER（ON DELETE SET NULL）+ 幂等（BDD-16/17）——顺序（create_all→_run_migrations）、旧库升级、索引
   - **权限收敛完整性**：can_read_entry 是否真正覆盖 7 条读路径（A1-A7 核对表），有没有漏改路径会让团队成员 404 / 非成员越权；star 2 处（BDD-14/15）；share 3 接口 403→404 防枚举（BDD-11/12/13）
   - **校验契约**：team_id 不存在/非成员/匿名 422 统一语义 + 绝不静默忽略（BDD-21/22/30）+ D3 存储不变量 clamp（BDD-27 防绕过）
   - **候选方案 A 合理性**：team_membership_exists 独立薄模块方案是否成立（免环）；list_entries SQL EXISTS 形态；files_to_read 是否覆盖
   - **性能**：BDD-26 EXPLAIN 断言设计（idx 命中 / 无逐行子查询）
   - **安全**：防枚举 404 一致性、全局 key 语义、owner 失效语义（BDD-19/20）
   - **CLI/PeekClient**：--user/--team 语义、远程透传（BDD-31~34）
   - **gate_commands**：是否引用 Makefile target、P5_e2e 合理、timeout 声明、ui_affected 时含 E2E
3. **评审结论**：Header status 初始 draft → `approved` / `rejected` / `needs-revision`。结论引用具体设计节/BDD 锚点。**BLOCKER → rejected**。
4. **只读原则**：不修改 P2-design.md 或其它文件（除产出 P2-review-eng.md + P2-review-progress.md）。
5. **产出路径**：`agate-workspace/tasks/TPV0095-team-visibility/P2-review-eng.md`。
6. **环境隔离**：禁碰生产 :8080 / ~/.peekview/。状态标记二值。

### 上游关联

- P2-design.md：候选 A 选定（权限收敛落 entry_service 域 + 前端状态留视图层 + teams 独立 service），3 SCOPE+ 已报告主 Agent（detail 标签/raw team 字段/restore merge），主 Agent 已裁定（detail 采纳增补 BDD-44、raw 采纳为实现约束、restore 不采纳记 backlog）。
- P1-requirements.md：43+1 BDD（BDD-44 主 Agent 增补）。

### 输入文件

1. `agate-workspace/tasks/TPV0095-team-visibility/P2-design.md`（评审对象）
2. `agate-workspace/tasks/TPV0095-team-visibility/P1-requirements.md`（43 BDD + BDD-44 增补验收基线）
3. `docs/design-notes/team-visibility.md`（语义权威源）
4. `/home/kity/.agate/assets/review-roles/plan-eng-review.md`（角色）

### 分阶段落盘

每读完输入文件/评审进展追加 `P2-review-progress.md`。

### 产出文件字段

`FILE=P2-review-eng.md agate-md-field-set` 维护 Header。

### 返回

两行：① P2-review-eng.md 路径；② Status: approved/rejected/needs-revision + BLOCKER 摘要（若有）。
</dispatch_guide>
