...
# P2 派发上下文 — 专家组组长汇总（TPV0095 team-visibility）
...
作为专家组组长，汇总 P2 两个独立评审的结论，产出统一 `P2-review.md`（Header status: approved / rejected）。

### 组长规则

1. **不发表新意见**，只汇总两个评审的判定。
2. 任一评审标 BLOCKER / status=rejected → 汇总 status: rejected（附 BLOCKER 清单）。
3. 两位评审分歧 → 标「专家组分歧」交人工。
4. 全票无 BLOCKER → status: approved。
5. 产出 P2-review.md 的 Header `agent` 字段 = `review`（组长角色，非 main）——check-gate P2 硬拦截 agent=main 的 approved。

### 输入文件

1. `P2-review-eng.md`（plan-eng-review 工程评审产出）
2. `P2-review-design.md`（plan-design-review 前端评审产出）
3. `P2-design.md`（评审对象，备查）

### 产出

`P2-review.md`：
- Header: phase=P2 / type=review / parent=P2-design.md / agent=review / status=approved 或 rejected
- 正文：两评审各自结论摘要（BLOCKER / 修订项 / approved）+ 组长汇总判定 + 引用锚点

### 返回

两行：① P2-review.md 路径；② Status: approved/rejected + 一句摘要。
</dispatch_guide>
