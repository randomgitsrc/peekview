...
# P2 派发上下文 — plan-design-review 复审 rev1（TPV0095 team-visibility）
...
复审 architect rev1 修订后的 `P2-design.md`（574 行），产出 `P2-review-design.md`（**覆盖写**上轮，Header status 改 approved 或继续 needs-revision）。核对上轮 N1-N3 是否落实且无新引入问题。

### 复审核对清单（对照上轮 P2-review-design.md）

- **N1**：UI 设计节交互 checklist「需人工复核」是否改为明确动作（/teams 表单输入态规格 + P6 复核落为逐态 Playwright 断言）
- **N2**：data-testid 集中清单节是否补齐（5 tab / team-chip-{slug} / team-unavailable+clear / team-empty / badge-team / visibility-toggle / teams-owned/joined / team-create-form / team-name-input / team-member-username-input / team-error / teams-status-live）；§5.3 toggle 隐藏边界（delete 保留）是否声明；BDD-44 detail 三态载体是否二选一写死；/teams 新建表单输入/输出规格；myTeams store 动作清单
- **N3**：三态文案表（暂无团队内容 / 该团队暂无内容 / 团队不可用）是否并排收口；list 视图非 owner 不渲染 badge 声明
- 非阻塞建议采纳情况（§5.1 高亮范围 / §5.2 myTeams 依赖 / 键盘导航锁一种）
- 与 BDD-38~44 验收线仍一致；无新引入问题

### 约束

1. 角色定义：`/home/kity/.agate/assets/review-roles/plan-design-review.md`。
2. 评审结论：Header status 初始 draft → approved / rejected / needs-revision。结论引用修订后设计节锚点。
3. 只读：不修改 P2-design.md。
4. 产出路径：覆盖写 `P2-review-design.md`。
5. 环境隔离：禁碰生产。

### 输入文件

1. `P2-review-design.md`（上轮评审——先读 N 清单再审新版）
2. `P2-design.md`（修订后评审对象，重点 §5 + UI 设计节 + data-testid 清单节）
3. `P1-requirements.md`（BDD-38~44）
4. `docs/design-notes/team-visibility.md`（权威源备查）

### 分阶段落盘

追加 `P2-review-progress.md`。

### 返回

两行：① P2-review-design.md 路径；② Status: approved/rejected/needs-revision + 摘要（≤30 字）。
</dispatch_guide>
