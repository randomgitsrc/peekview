...
# P1 派发上下文 — analyst（TPV0095 team-visibility）
...
产出 `P1-requirements.md`（需求基线）：为「团队内可见」权限档位（team 可见性机制）建立可二值判定的 BDD 验收条件 + frontmatter 机器字段。**P1 只定义问题，不设计方案**（方案已在 design-note 定稿，但 P1 的职责是把它转成验收基线，不是复述方案）。

### 约束

1. **输入权威**：`docs/design-notes/team-visibility.md`（v4 终版，双评审 PASS）是需求唯一真相源。P0-brief.md 是工程约束注入。读 design-note §2（用户故事）+ §13（测试清单）建立验收基线；§12（风险）转成 BDD 边界条件。**不要重写 design-note 已定的方案**——但 BDD 必须用用户可观察行为表述（"团队成员能读该 entry"），不是 API 调用表述（"GET /api/v1/entries/{slug} 返回 200"）。
2. **P0-brief 时效性质疑**：先核对 P0-brief 是否漂移（判据见 analyst 角色文件）。本任务立项于 2026-09-02 当日（commit 5525c319），design-note v4 与 P0-brief 同时落地，预计无漂移——但仍须显式核对并记录一行。
3. **frontmatter 机器字段**：risk_level / phases / packages / domains / capability_requirements 必填，写在 `---` frontmatter 块。TPV0095 涉及 schema 变更 + 权限收敛（安全）+ backend/frontend/mcp 三端 → **risk_level 建议 medium 或 high**（P0-brief 判 medium-high）；**phases 全走 [P1..P8] 无裁剪**（P0-brief 裁剪倾向：P2/P6 不可裁、P3 必走）；**domains: [backend, frontend, mcp, security]**（CLI 改动存在但归入 backend 域还是单独声明由你判断——design-note §6 有 CLI 改动，注意 MCP 域包含 packages/mcp-server）。**capability_requirements 必含视觉能力条目**（domains 含 frontend → P1 gate 硬性要求：`need` 命名含 visual/vision，status ∈ available/supplementable/GAP，P6 UI 验收要截图）。UI 形态：常规布局型（/teams 页 + explore tabs），可按需声明 `ui_render_shape: layout` + `ui_ux_dimensions`（frontmatter 可选，缺失 = 常规布局型默认；但 UX 类别 BDD 至少 1 条是 frontend 硬要求，类别写 BDD 标题后缀如 `#### BDD-N: 交互行为：...`）。
4. **标记格式**：`[NO_NEED_CONFIRM]` 和 `[SUGGEST: ...]` 必须行首无反引号。设计-note 的 9 个决策点（A-I）已被双评审定稿，不应再翻成 NEED_CONFIRM——除非你发现决策未覆盖的真实方向分叉。
5. **同类扫描（强制节）**：对「可见性/权限判定」类符号做全仓 grep——入口包括 7 处权限检查（get_entry/list_entries/_resolve_entry/resolve_entry_raw/_check_share_cookie/get_entry_with_share/download_entry_files）+ share 接口 + star 可见性 + API key 分支 + admin 分支。记录命中数量 + 文件清单 + 逐条"本次处理/不处理"判定。scan 结论写进 P1 正文。
6. **环境隔离**：只读代码分析，不得修改任何文件。可跑 grep/读文件，不要动 debug server / 生产 :8080 / ~/.peekview/。
7. **产出路径**：`agate-workspace/tasks/TPV0095-team-visibility/P1-requirements.md`（Write 工具，路径硬约束）。
8. **分阶段落盘**：每读完一个输入文件，把发现追加到 `agate-workspace/tasks/TPV0095-team-visibility/P1-progress.md`（bash 追加或 Write 追加）。不要等全部读完再一次性写。

### 上游关联

- P0-brief.md 已补齐 executor_env/env_constraints 字段（commit d41873ea）。
- design-note v4 终版已入库（commit 5525c319），决策 A-I 全部定稿。
- 当前版本 peekview 0.21.0 / mcp 0.11.0（VERSIONS.json）。

### 输入文件

1. `agate-workspace/tasks/TPV0095-team-visibility/P0-brief.md`（任务简报：核心逻辑 9 条 + 关键决策 9 项 + 改动清单 + known_risks）
2. `docs/design-notes/team-visibility.md`（**v4 终版需求真相源**——重点 §2 用户故事、§3 决策、§12 风险、§13 测试清单；§5/§7/§8 方案细节供理解验收范围）
3. `CLAUDE.md` / `AGENTS.md`（项目约定）
4. `docs/roadmap/improvement-backlog.md`（#48 条目上下文）

### 客观查证信息（已确认，勿重查）

- 权限检查点分布（backend/peekview/）：
  - `services/entry_service.py`:326 get_entry、:385 list_entries、:1155 get_entry_with_share、:1215 get_entry_by_slug、:1220 get_entry_files
  - `api/entries.py`:37 _check_share_cookie、:130 list_entries、:184 get_entry、:316 get_entry_reads、:468 download_entry_files
  - `api/files.py`:130 _resolve_entry、:352 resolve_entry_raw、:517 get_entry_raw
- 迁移逻辑在 `backend/peekview/database.py:_run_migrations`（幂等 SQLite 迁移）
- 模型在 `backend/peekview/models.py`（SQLModel：Entry/User/ApiKey + Pydantic schemas；`_looks_like_jwt` 等重复代码曾在 T082 清理）
- 前端：`frontend-v3/src/views/`（EntryList/EntryDetail/...）+ `router.ts`（⚠️ 非 src/router/index.ts）+ `stores/`（entry/auth/share/theme Pinia）+ `components/`（BaseBadge/FilterChip/UserMenu 等）+ `api/`（axios wrappers）
- MCP：`packages/mcp-server/src/`（server.ts / tools/publishFiles.ts、getEntry.ts、listEntries.ts / client.ts / types.ts）
- CLI：`backend/peekview/cli.py`（Click CLI）
- 用户故事（design-note §2.1）：Alice 建 team → 加 Bob → 发布 entry 指定 team → Bob 登录可见、匿名不可见、Carol（非成员）不可见且 404。

### 产出文件字段

用 `FILE={产出文件路径} agate-md-field-set --list` 查看本阶段应填字段；`FILE={产出文件路径} agate-md-field-set <key> <value>` 逐个写入；写入失败照错误提示修正，不要手写 frontmatter；仍失败则报告主 Agent。

### 返回

只返回两行：① 产出文件路径；② 一句话摘要（BDD 条数、domains、有无 NEED_CONFIRM，≤30 字）。
</dispatch_guide>
