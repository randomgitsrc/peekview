...
# P2 派发上下文 — plan-eng-review 复审 rev1（TPV0095 team-visibility）
...
复审 architect rev1 修订后的 `P2-design.md`（574 行），产出 `P2-review-eng.md`（**覆盖写**上轮，Header status 改 approved 或继续 needs-revision）。核对上轮 R1-R4/N1-N4 是否落实且无新引入问题。

### 复审核对清单（对照上轮 P2-review-eng.md）

- **R1**：CLI 本地 team 路径迁移方案是否锁定（推荐 init_db(run_migrations=True) 或索引移 _setup_indexes 二选一）；§3.4/§7 是否同步
- **R2**：§3.1 D2 是否锁 ParameterValidationError(422)
- **R3**：§6 gate_commands 是否拆键（无 && 链）+ frontend vitest key 是否补入 P3/P5/P6 + timeout_seconds
- **R4**：CLI 示例是否补 --user；参照对象是否修正为 apikey create 先例；P1 BDD-31~33 [BASELINE_CHANGE] 与设计是否对齐（读 P1 确认主 Agent 已批准写入）
- **N1(eng)**：FK ondelete 显式声明是否入 §2
- **N2(eng)**：team_membership 落点表述是否统一指 §3.2 锁定
- **N3(eng)**：节号漂移是否修正
- **N4(eng)**：raw team 字段是否入 §0.1 Modify 表
- 核心架构（迁移顺序/7 路径收敛/免环/EXPLAIN）是否未被修订破坏
- 无新引入问题
...
4. 产出路径：覆盖写 `P2-review-eng.md`。
5. 环境隔离：禁碰生产。

### 输入文件

1. `P2-review-eng.md`（上轮评审——先读 F 清单再审新版）
2. `P2-design.md`（修订后评审对象）
3. `P1-requirements.md`（BDD-31~34 [BASELINE_CHANGE] 确认）
...
两行：① P2-review-eng.md 路径；② Status: approved/rejected/needs-revision + 摘要（≤30 字）。
</dispatch_guide>
