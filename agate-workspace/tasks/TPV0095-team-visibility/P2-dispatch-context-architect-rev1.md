...
type: design
parent: P2-design.md
trace_id: TPV0095-P2-architect-rev1-20260902
status: draft
agent: architect
---

# P2 派发上下文 — architect 修订轮 rev1（TPV0095 team-visibility）
...
按两个 P2 评审的修订项（eng R1-R4 + N1-N4、design N1-N3）**定向回补** `P2-design.md`，消除评审指出的「约定不明/口径未锁/契约脱节」，不重写方案主体。修订后交付复审。

### 修订任务清单（对应评审文件 §架构问题/§修订项）

**来自 P2-review-eng.md：**

- **R1【关键】CLI 本地模式 DB 迁移路径遗漏**：`_get_backend`(cli.py:43-81)/`_resolve_user_local`(cli.py:2077) 本地 init_db 不带 run_migrations → 存量旧库本地跑 teams/--team 会 SchemaMismatchError、CLI 直建库缺两索引。二选一锁定（推荐方案 1）：① CLI 本地 team 路径 init_db(run_migrations=True)（幂等，与 serve 同语义）；② 索引移 _setup_indexes。选定后在 §3.4/§7 更新，并处理由此产生的 P3 测试缺口（CLI 直建库含索引断言）。
- **R2【关键】422 错误类口径**：§3.1 D2 显式锁「team 归属/存在性/匿名校验抛 `ParameterValidationError`（status_code=422）」（exceptions.py 实读：ValidationError=400、ParameterValidationError=422）；文案统一不区分存在性与成员身份。同步 §7 exceptions 注解。
- **R3【关键】gate_commands 拆键 + frontend 缺位**：§6 拆掉 `&&` 链，每键一条命令；补 frontend vitest key：
  ```yaml
  P3: "make test-quick"
  ...
  P5_typecheck / P5_lint / P5_e2e（保留原样，P5_e2e 拆两键或注明逐 spec）
  P6 / P6_frontend / P6_mcp（同法）
  ```
  各 key 补 `_timeout_seconds`（typecheck 180 / lint 120 / frontend 180 / e2e 300）。P6_e2e 覆盖 teams-page.spec（BDD-42）——E2E spec 拆分说明保留（run-e2e-tests.sh 单 spec 传参，逐 spec 跑）。
- **R4【基线变更，主 Agent 已批准 [BASELINE_CHANGE]】CLI --user 契约**：设计 §3.4 锁「本地 create/list/teams 加 --user 必填」但 P1 BDD-31~33 When 未带 --user → P6 契约打架。**P1 基线 BDD-31~34 已由主 Agent 批准增补 --user 锚**（见 P1-requirements.md 对应 BDD 的 [BASELINE_CHANGE] 标注——architect 读 P1 确认实际写法）。设计侧同步：CLI 示例补 --user；§3.4 修正「参照 user_cmd 模式」→「参照 apikey create --user 先例（cli.py:1920，required in local mode）」。
- **N1【FK ondelete 显式化】**:§2 补一行声明：Team.owner_id / TeamMember.team_id/user_id / Entry.team_id 模型字段必须带 `ondelete="CASCADE"/"SET NULL"`（与 raw DDL 双源一致，全新库 create_all 路径 FK 行为正确）。
- **N2【team_membership 落点一处权威】**:§0.1 B 行 / §1 候选 A 正文的「entry_service 顶部模块级」表述改为指向 §3.2 锁定结论「独立薄模块 services/team_membership.py」。
- **N3【节号漂移】**:§2 line 169「见 §4.4 核对」修正（删引用或改指实际章节）。
- **N4【raw team 字段入改动表】**:§0.1 Modify 表补一行：files.py resolve_entry_raw + models.EntryRawResponse 加可选 team 字段（SCOPE+2 采纳，仅 owner/成员/全局 key 附 team，share 访问者不附）。

**来自 P2-review-design.md：**

- **N1【P6 人工复核落为断言】**:UI 设计节交互 checklist 的「需人工复核」[x] 改为明确动作——§5.5 /teams 表单输入态规格（并入 N2 数据表）+ P6 复核落到「输入态逐态 Playwright 断言 + 截图」。
- **N2【组件级规格】**:P2-design 补集中 data-testid 清单节（5 tab / team-chip-{slug} / team-unavailable + team-unavailable-clear / team-empty / badge-team / visibility-toggle（EntryCard+EntryListRow 统一，断言 count=0）/ teams-owned / teams-joined / team-create-form / team-name-input / team-member-username-input / team-error / teams-status-live）+ §5.3 补「toggle 与 delete 同容器，仅隐藏 visibility toggle、delete 保留」+ BDD-44 detail 三态实现载体**二选一写死**（BaseBadge status union + team 变体 或 就地扩展 span.status-tag，推荐前者保视觉一致）+ /teams 新建表单组件级输入/输出（name 校验/成功 slug 展示/失败态）+ stores/team.ts myTeams 动作清单（加载时机/登出清零/增删改同步/成员快照过期语义与 BDD-23 无缓存窗口一致）。
- **N3【文案三态表 + list badge】**:§5.1/5.2 并排三态文案表（teams 聚合空 →「暂无团队内容」/ team chip 空 →「该团队暂无内容」/ slug∉myTeams →「团队不可用」+ 清除 CTA，不可用态不调接口）+ 一句声明「非 owner 列表项不显示可见性 badge（含 team badge），EntryCard/EntryListRow 一致」。

**非阻塞建议（design）**：§5.1 补「高亮规则=现状语义+team 扩展，不重构 archived/starred 与 All 既有激活」一句；§5.2 不可用态判定依赖 myTeams 已加载（匿名/加载中窗口不误判）；§5.6 键盘导航二选一锁一种（推荐 tablist+方向键）；不可用态可显示 team slug（可选）。

### 约束

1. 修订落在 `P2-design.md`，不重写主体；评审锁定决策（迁移顺序/7 路径收敛/免环/EXPLAIN）保持不变。
2. gate_commands 修订后仍是 P4-P6 唯一真相（本修订属 P2 阶段内补钉，合法）。
3. P1-requirements.md 的 BDD-31~34 [BASELINE_CHANGE] 已由主 Agent 批准写入，architect 只读确认、不再改 P1。
4. 环境隔离：只改 P2-design.md（+ P2-progress.md 追加），禁碰生产。
5. 完成后 check-frontmatter.py 自检。

### 上游关联

- 两评审文件：P2-review-eng.md（R1-R4/N1-N4）、P2-review-design.md（N1-N3）——本次修订清单来源。
- P2-design.md（修订对象，508 行）。
- P1-requirements.md（BDD-31~34 已含主 Agent 批准的 [BASELINE_CHANGE] --user 锚，读确认）。

### 输入文件

1. `P2-review-eng.md`（修订清单）
2. `P2-review-design.md`（修订清单）
3. `P2-design.md`（修订对象）
4. `P1-requirements.md`（BDD-31~34 --user 锚确认）
5. `backend/peekview/exceptions.py`（R2 错误类实读，已验证 ValidationError=400 / ParameterValidationError=422）

### 分阶段落盘

每完成一个 R/N 项修订追加 `P2-progress.md`（追加，不覆盖首轮）。

### 产出文件字段

`FILE=P2-design.md agate-md-field-set` 维护 Header（trace_id 可更新为 rev1）；完成后 check-frontmatter.py。

### 返回

两行：① P2-design.md 路径；② 修订摘要（落实了哪些 R/N 项，≤40 字）。
</dispatch_guide>
