...
# P3 派发上下文 — test-designer（mcp 批，TPV0095 team-visibility）
...
为 TPV0095 **MCP 域**写 TDD 测试（红灯）：schema team_id / list_teams / description 硬提示 / get_entry team 字段（BDD-35~37）。产出 P3-test-cases-mcp.md（片段）+ MCP 测试代码。

### 约束

1. **输入权威**：P1-requirements.md（BDD-35~37）+ P2-design.md（§4 MCP 设计）。
2. **测试落点**：`packages/mcp-server/tests/` 新建或扩展——createEntry/publishFiles zod schema 含 team_id optional；listTeams 工具注册与输出两分区；description 元数据含 TEAM VISIBILITY 硬提示；getEntry 输出 team 字段。参照现有 tests（createEntry.test.ts / server.test.ts / client.test.ts / getEntry.test.ts）的 mock server 模式。
3. **红灯要求**：自跑确认红灯（listTeams 不存在 / schema 无 team_id / description 无硬提示）。不要写实现。
4. **注意**：MCP 测试多用 mock（不依赖真实 backend），若需集成指向 127.0.0.1:8888 debug backend（禁 :8080）。vitest fileParallelism: false（config 测试 mutate env）。
5. **test_code_dir**：`packages/mcp-server/tests/`。
6. **环境隔离**：只写测试文件 + progress 落盘；禁碰生产。bash timeout。
7. backend/frontend 测试由其他批负责。

### 上游关联

- P1：BDD-35~37（MCP 线）
- P2：§4 MCP 设计（team_id + list_teams + description 硬提示 + get_entry team 字段）

### 输入文件

1. `P1-requirements.md`（BDD-35~37）
2. `P2-design.md`（§4 MCP + §7 files_to_read mcp 部分）
3. `P0-brief.md`
4. 现有 MCP 测试参照（packages/mcp-server/tests/*.test.ts）

### 产出

- `P3-test-cases-mcp.md`（本批用例清单片段：BDD 映射 + test_code_dir + 红灯确认；主 Agent 合并）
- packages/mcp-server/tests/ 新增/扩展测试（红灯）

### 返回

两行：① 本批测试文件路径清单；② 摘要（用例数/覆盖 BDD/红灯确认，≤40 字）。
</dispatch_guide>
