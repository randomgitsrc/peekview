...
trace_id: TPV0095-P2-architect-20260902
status: draft
agent: architect
---

# P2 派发上下文 — architect（TPV0095 team-visibility）
...
产出 `P2-design.md`（方案设计 + 实现导航）：team 可见性机制的可实现技术方案。含候选方案 ≥2 + 权衡 + 选择、影响面梳理（改什么/不改什么/风险在哪）、四字段、UI 设计节、minimal_validation、files_to_read、gate_commands（P2 固化，P4-P6 不得改）。

### 约束

1. **输入权威**：`P1-requirements.md`（43 BDD 验收基线，已 approved）是验收依据；`docs/design-notes/team-visibility.md`（v4）是方案定稿参考（9 决策点已定）。design-note 已有详细设计（§4 数据模型 / §5 后端 / §6 CLI / §7 MCP / §8 前端）——你的候选方案权衡应在此定稿设计的基础上做**实现层选型**（如：teams 建表走 create_all 还是纯 _run_migrations？can_read_entry 放哪个模块？前端 tab 状态管理的实现选型），不要推翻 design-note 已定语义（那些是需求，已被 BDD 固化）。
2. **候选方案 ≥2**（frontmatter `candidate_count: N`），每候选含权衡 + 选择理由；无法写满 2 个时须声明 design_trivial/follows_existing_pattern + 理由（本项目不走该路径——TPV0095 复杂度高，必须 ≥2 真候选）。
3. **影响面梳理（强制节，写在候选方案之前）**：三部分改什么/不改什么/风险在哪，落点须到"文件+函数/节"级。P1 同类扫描 A-E 表（43 BDD 版）是基础，你在此基础上细化；P1 §5 A-E 表引用在修订后可能编号变了（35→43 重排），重新核对行号/位置。
4. **四字段 frontmatter**：`packages: [backend/peekview, frontend-v3, packages/mcp-server]` / `domains: [backend, frontend, mcp, security]` / `ui_affected: true` / `candidate_count: N`。**gate_commands 正文**：按 Makefile target 引用（`make test-quick` / `make test-frontend` / `make typecheck` / `make lint` / `make test-mcp-unit` / E2E），不手写裸命令；P3/P5 用紧凑输出；P5_e2e 若走自定义 Playwright 脚本需写明；`{key}_timeout_seconds` 按档位声明。
   ⚠️ 平台注意：bash 沙箱中 `ruff` 不在 PATH（在 backend/.venv/bin/），`make lint` 需 `PATH=backend/.venv/bin:$PATH make lint`——gate 命令请写成 Makefile target 但注明该 PATH 前缀需求（放 env_constraints）。
5. **UI 设计节（ui_affected: true 必含）**：渲染形态声明复用 P1 frontmatter 值（`渲染形态: layout（布局型）` / `适用维度: 布局结构、交互行为`）；布局/交互/视觉三类 checklist 覆盖；**UI 测试选择器**建议 data-testid 清单（Teams tab / team chip / team badge / 不可用态 / /teams 表单操作），供 P3 定位。
6. **minimal_validation（必须声明）**：涉及 SQLite 迁移顺序（create_all vs _run_migrations 谁先建 teams 表）与 can_read_entry 权限收敛——建议最小验证：①读 database.py init_db 顺序确认 create_all 先建 metadata 表（BDD-16 旧库升级路径前提）；②读 7 处读路径现状代码确认收敛点；③如涉及浏览器/外部行为再实测。纯代码逻辑须声明理由。
7. **files_to_read**：给 P4 implementer 的上下文地图——只列实现确实要读的文件 + why + 行号范围（大文件标范围）。参考清单（P1 已查证）：backend database.py(_run_migrations:40-90/create_all 顺序/check_schema:229/_setup_indexes:318)、models.py(Entry:186/EntryShare:239/EntryCreate:466/User:105)、entry_service.py(get_entry:326/list_entries:385/get_entry_with_share:1155/update_entry:657/delete_entry:839)、api/entries.py(_check_share_cookie:37/get_entry:184/download:468)、api/files.py(_resolve_entry:130/resolve_entry_raw:352/get_entry_raw:517)、api/shares.py、api/stars.py、share_service.py(create:38/list:118/revoke:157)、star_service.py(_build_star_item:365)、cli.py、client.py、frontend router.ts/EntryListView.vue/EntryCard.vue/EntryListRow.vue/BaseBadge.vue/FilterChip.vue/UserMenu.vue/stores/api/client.ts/types、packages/mcp-server tools/*.ts。自行补充缺口但控制总量。
8. **dispatch_plan（可选机器字段）**：本任务三端多包，评估是否需要 static-batch 编排（如 P4 后端/前端/MCP 拆批并行）——若评估 high 复杂度须声明批次表。
9. **环境隔离**：只读代码/写 P2-design.md；不修改代码文件；禁碰生产。bash 命令 timeout。
10. **产出路径**：`agate-workspace/tasks/TPV0095-team-visibility/P2-design.md`。
11. **分阶段落盘**：每步追加 `P2-progress.md`。
12. **P1 基线保护**：设计中若发现 P1 未预见的新隐含需求 → 标 `[SCOPE+]`（行首格式）报告主 Agent，不擅自扩范围；发现 BDD 矛盾 → 标 `[BASELINE_CHANGE]` 需主 Agent 批准，不擅自改。

### 上游关联

- P1-requirements.md：43 BDD approved（commit 20af6de4）。BDD 分组：后端权限 BDD-1~6 / 防枚举 7~9 / share 10~12 / star 13~14 / 生命周期迁移 15~17 / 校验 18~19 / 竞态 20 / 回归 21 / API 22~23 / CLI 24~26 / MCP 27~29 / 前端 UI 30~35（修订后编号按 43 条实际为准，读文件确认）。
- design-note v4（commit 5525c319）：§4 数据模型 / §5 后端 API / §6 CLI / §7 MCP / §8 前端 / §13 测试清单。
- P0-brief：executor_env/env_constraints（debug_env: make debug-quick :8888；lint/typecheck/test target 已列出）。

### 输入文件

1. `agate-workspace/tasks/TPV0095-team-visibility/P1-requirements.md`（43 BDD 验收基线）
2. `agate-workspace/tasks/TPV0095-team-visibility/P0-brief.md`（工程约束/风险）
3. `docs/design-notes/team-visibility.md`（v4 终版方案定稿）
4. `CLAUDE.md` / `AGENTS.md`（项目约定）
5. 代码文件（按需 grep/read，上面约束 7 给了导航）

### 客观查证信息（已确认，勿重查）

- 版本：peekview 0.21.0 / mcp 0.11.0（VERSIONS.json）
- database.py: `init_db` 顺序 = SQLModel.metadata.create_all(engine)（:302）→ _run_migrations(engine)（:306）→ _setup_indexes（:318+）；check_schema(:229) 检查缺失列
- models.py 现有表模型：User(:105)/ApiKey(:154)/Entry(:186)/EntryShare(:239)/EntryRead(:266)/EntryStar(:291)/EntryTombstone(:327)/EntryReadStats(:353)；非表模型 EntryCreate(:466)
- share_service.py: create_share(:38) 非 owner/admin → ForbiddenError(403)；list(:118)/revoke(:157)；revoke_all_for_entry(:267)
- 读路径权限 7 处 + star 2 处 + share 3 处（P1 §5 同类扫描 A/B/C 表，修订后重核）
- 前端 EntryListView.vue owner-tabs 4 tab → 需加 Teams 第 5 tab；router.ts（非 router/index.ts）
- Makefile test-quick（-n auto）/ test-frontend / typecheck / lint / test-mcp-unit 均为标准 target

### 产出文件字段

`FILE=P2-design.md agate-md-field-set --list` 查字段；逐个写入（phase=P2/task_id=TPV0095/type=design/parent=P1-requirements.md/agent=architect + candidate_count/packages/domains/ui_affected）。gate_commands/files_to_read/env_constraints/minimal_validation 写正文。完成后 `python3 /home/kity/.agate/scripts/check-frontmatter.py P2-design.md` 自检。

### 返回

两行：① P2-design.md 路径；② 方案摘要（候选数/选定方案要点/UI 是否受影响，≤40 字）。
</dispatch_guide>
