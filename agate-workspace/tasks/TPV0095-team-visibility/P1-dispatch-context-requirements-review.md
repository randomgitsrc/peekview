...
# P1 派发上下文 — requirements-review（TPV0095 team-visibility）
...
独立评审 `P1-requirements.md`，产出 `P1-review.md`（结论引用具体 BDD 编号 + 覆盖维度标注）。只审不写——不直接修改 P1-requirements.md。

### 约束

1. **角色定义**：`/home/kity/.agate/assets/review-roles/requirements-review.md`（检查清单：BDD 可二值判定 / 隐含需求覆盖 / 跨条一致性 / UI/UX 机制 / 裁剪合理性 / P1 纯净性）。
2. **评审重点**（本任务特性）：
   - BDD 是否可二值判定（无中间态）——35 条 BDD 逐条过
   - 权限矩阵表述（BDD-2/5 的"7 条读路径"）是否完整可测
   - 同类扫描（第 5 节 A-E 表）是否覆盖全、判定是否合理
   - frontmatter：risk_level=high、phases 全走、domains=[backend frontend mcp security]、packages=[backend/peekview frontend-v3 packages/mcp-server]、ceremony=standard、ui_render_shape=layout、ui_ux_dimensions=[布局结构, 交互行为]——声明与正文/实际改动是否匹配
   - capability_requirements 四条目均 available，含 browser-vision（frontend 硬要求）——三态判断是否正确
   - UX 类别 BDD（BDD-30~35 后缀标注）判据可量化、不绑 CSS 类名/组件名
   - P1 纯净性：无方案设计混入（design-note 已定稿方案，但 P1 文件本身是否保持"验收基线"定位）
3. **审声明（TAG0019）**：risk_level / ceremony / phases 声明 vs 暂存区/工作区实际改动证据（可 git diff P0-brief 提交范围判断改动规模与域）。声明与正文不一致 → 结论必须 needs-revision / rejected。
4. **评审结论**：文件 Header `status:` 初始 draft，评审完改为 `approved` / `rejected` / `needs-revision`。gate 脚本读 Header status。结论必须引用 BDD 编号锚点（裸 approved 无锚点 = 假完成）。
5. **产出路径**：`agate-workspace/tasks/TPV0095-team-visibility/P1-review.md`。
6. **只读原则**：不修改 P1-requirements.md 及其它任何文件（除 P1-review.md 与 P1-review-progress.md 落盘）。
7. **环境隔离**：禁碰生产 :8080 / ~/.peekview/。状态标记 `[PROD_NOT_TOUCHED]` / `[PROD_TOUCHED]`。

### 上游关联

- P1-requirements.md 由 analyst 产出（35 BDD，NO_NEED_CONFIRM，1 SUGGEST 版本 bump 建议）。
- 需求真相源 design-note v4（已双评审 PASS）——评审 P1 时如发现 P1 与 design-note 冲突，以 design-note 为权威并标注差异。

### 输入文件

1. `agate-workspace/tasks/TPV0095-team-visibility/P1-requirements.md`（评审对象）
2. `agate-workspace/tasks/TPV0095-team-visibility/P0-brief.md`（约束/风险来源，审声明用）
3. `docs/design-notes/team-visibility.md`（需求权威源，冲突时对照）
4. `/home/kity/.agate/assets/review-roles/requirements-review.md`（角色检查清单）

### 产出文件字段

用 `FILE=P1-review.md agate-md-field-set --list` 查看字段；逐个写入；失败报告主 Agent。

### 返回

只返回两行：① P1-review.md 路径；② Status: approved/rejected/needs-revision + 一句摘要（≤30 字）。
</dispatch_guide>
