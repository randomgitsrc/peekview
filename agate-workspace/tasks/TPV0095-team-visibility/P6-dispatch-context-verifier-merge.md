...
type: acceptance
parent: P6-evidence/
trace_id: TPV0095-P6-verifier-merge-20260902
status: draft
agent: verifier
---

# P6 派发上下文 — verifier 汇总（整合 P6-acceptance.md，TPV0095）
...
作为 P6 汇总 verifier，把 V1（backend/CLI/MCP 域 BDD-1~37）+ V2（frontend UI 域 BDD-38~44）的验收结果整合为唯一 `P6-acceptance.md`。

### 整合要求

1. **BDD 覆盖核对**：V1 results.md（37 行 BDD-1~37）+ V2 results.md（7 行 BDD-38~44）= 44 条 = P1 全部 `#### BDD-NN:` 数。核对无重复/遗漏，在 P6-acceptance.md 记录交叉核对结果。
2. **P6-acceptance.md 格式**：
   - Header frontmatter：phase=P6 / task_id=TPV0095 / type=acceptance / parent=P5-test-results.md / trace_id=TPV0095-P6-20260902 / status=draft→approved / agent=verifier + `pass: 44` / `fail: 0` / `ui_affected: true`
   - 正文：每条 BDD 一行 `- PASS BDD-NN: 描述 (证据路径)`（PASS/FAIL 后空格 + BDD 编号）
   - 证据路径**相对 P6-evidence/**（如 `backend/test-output.log` / `frontend/screenshots/bdd38-teams-tab-desktop.png` / `frontend/vision-reports/bdd-38.yaml`）
   - vision 引用独立括号 `(vision: frontend/vision-reports/bdd-NN.yaml)`
   - 总结行 `**Summary**: 44/44 PASS, 0 FAIL`（非 `- PASS` 前缀）
3. **UI 类 PASS 双证**：BDD-38~44 保留截图 + vision/manual-review 引用（V2 已备）。
4. **汇总后预检**（verifier 角色 gate 格式预检）：
   - `python3 /home/kity/.agate/scripts/check-p6-format.py --fix P6-acceptance.md`
   - `python3 /home/kity/.agate/scripts/check-p6-evidence.py $TASK_DIR`
   - `python3 /home/kity/.agate/scripts/check-p6-provenance.py $TASK_DIR`
   - 预检 exit 0 → 返回；exit 1/2 → 修复后重试（最多 2 轮）
5. **自查≠gate**：你产出 P6-acceptance.md，主 Agent 亲自验 gate。不要声称"验收通过"。

### 输入文件

1. `P6-evidence/backend/results.md`（V1：BDD-1~37）
2. `P6-evidence/frontend/results.md`（V2：BDD-38~44）
3. `P1-requirements.md`（44 BDD 权威核对）
4. V1/V2 证据文件（backend/test-output.log 等 + frontend/screenshots + vision-reports）

### 产出

- `P6-acceptance.md`（44 行 PASS + 交叉核对记录 + frontmatter pass:44/fail:0/ui_affected:true）

### 返回

两行：① P6-acceptance.md 路径；② 摘要（44/44 PASS 整合完成 + 预检结果，≤40 字）。
</dispatch_guide>
