...
# P1 派发上下文 — requirements-review 复审 rev1（TPV0095 team-visibility）
...
复审 analyst rev1 修订后的 `P1-requirements.md`（43 BDD），产出 `P1-review.md`（**覆盖写**上轮评审文件，Header status 改为 approved 或继续 needs-revision）。重点核对上轮 F1-F8 是否全部落实且无新引入问题。

### 复审重点（对照上轮 P1-review.md §三问题清单）

- **F1**：BDD-7（或拆分后的编号）是否消除与 design-note §5.1 的冲突——详情读权（owner+成员 200 / 无关 404）与管理操作（仅 owner）分开断言
- **F2**：原 BDD-20 是否拆为两条独立 G-W-T（成员移除后读 404 / 并发删除不 5xx）
- **F3**：原 BDD-23 是否拆条 + update team 归属口径是否显式统一为「成员」口径且全文（含散文）无矛盾
- **F4**：是否补 PeekClient / CLI 远程模式验收 BDD（可二值判定）
- **F5**：是否补 owner 账号禁用冻结 / 删除 CASCADE / EXPLAIN 索引命中 BDD
- **F6-F8**：slug 归属 / BDD-22 标题正文 / BDD-34 文案锚 是否落实
- **编号连续性**：43 条 BDD-1~43 连续无跳号、无编号引用残留（旧编号引用必须全文同步更新）
- **无新引入问题**：拆条/补条后每条仍单 G-W-T、可二值判定；与 design-note 无新冲突
- **frontmatter**：risk_level / phases / packages / domains / ceremony / ui 形态声明与修订后内容仍匹配

### 约束

1. **角色定义**：`/home/kity/.agate/assets/review-roles/requirements-review.md`（完整检查清单）。
2. **评审结论**：Header status 初始 draft，审完改 `approved` / `rejected` / `needs-revision`。结论引用 BDD 编号锚点。
3. **只读原则**：不修改 P1-requirements.md。
4. **产出路径**：覆盖写 `agate-workspace/tasks/TPV0095-team-visibility/P1-review.md`。
5. **环境隔离**：禁碰生产 :8080 / ~/.peekview/。

### 上游关联

- 上轮评审 P1-review.md 判 needs-revision（F1-F5 阻塞 + F6-F8 建议）
- analyst rev1 修订完成：35→43 BDD，声明已全量落实 F1-F8

### 输入文件

1. `agate-workspace/tasks/TPV0095-team-visibility/P1-requirements.md`（修订后评审对象，43 BDD）
2. `agate-workspace/tasks/TPV0095-team-visibility/P1-review.md`（上轮评审——先读它拿 F 清单再审新版）
3. `docs/design-notes/team-visibility.md`（权威源）
4. `agate-workspace/tasks/TPV0095-team-visibility/P0-brief.md`（约束）

### 分阶段落盘

每读完一个输入文件追加到 `P1-review-progress.md`（追加不覆盖）。

### 产出文件字段

`FILE=P1-review.md agate-md-field-set` 维护 Header。

### 返回

两行：① P1-review.md 路径；② Status: approved/rejected/needs-revision + 摘要（≤30 字）。
</dispatch_guide>
