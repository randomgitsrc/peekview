...
作为专家组组长，汇总两个 P2 评审意见为统一的 P2-review.md（gate 门槛文件）。**不发表新意见，只汇总。**

## 输入（两个专家评审文件）

1. `agate-workspace/tasks/TPV0093-star-lifecycle/P2-review-eng.md`（工程经理，status: approved，含 BLOCKER-4 闭合确认 + N1-N9 落实）
2. `agate-workspace/tasks/TPV0093-star-lifecycle/P2-review-design.md`（设计评审，status: approved，6 项 + 4 建议闭合确认）
...
`agate-workspace/tasks/TPV0093-star-lifecycle/P2-review.md`

## 门槛

- 文件存在且非空；Header agent 字段 = 组长角色名（非 main）；status: approved / rejected
- 汇总内容：专家结论（approved 确认）+ 锁定决策清单 + 遗留项（无）
