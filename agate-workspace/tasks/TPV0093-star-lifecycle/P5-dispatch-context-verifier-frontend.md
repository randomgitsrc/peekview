...
type: test-results
parent: P4-review.md
trace_id: TPV0093-P5-20260816-frontend-r2
status: ready
---

# P5 派发上下文 — verifier（frontend + E2E，串行版）
...
技术验证 TPV0093 **frontend 包 + E2E**：执行 gate_commands.P5_frontend（`make test-frontend && make typecheck`）与 P5_e2e（`E2E_SPEC=e2e/star*.spec.ts make debug-test`），产出 P5-test-results/unit.md（frontend）+ e2e.md + fail-list.txt。**只跑命令、只落盘结果，不做任何探索/诊断/额外工作。**

## 上游关联

- gate_commands（P2-design.md §8 固化）：
  - `P5_frontend: "make test-frontend && make typecheck"`
  - `P5_e2e: "E2E_SPEC=e2e/star*.spec.ts make debug-test"`
- 前置：P4 已 commit；backend verifier 已完成（串行第一步）；debug server :8888 已在跑（P0 make debug-quick）；Chrome CDP :18800

## 执行（严格按此流程，不多做，每步写 progress 心跳）

1. **frontend 单测 + 类型检查**：`cd /home/kity/oclab/peekview && make test-frontend && make typecheck 2>&1 | tail -15; echo "GATE_EXIT: ${PIPESTATUS[0]}"` —— timeout 300s。结果写入 `P5-test-results/unit.md`（frontend 部分，含输出签名）
2. **重建 static**：`cd /home/kity/oclab/peekview && make build-frontend 2>&1 | tail -5` —— timeout 300s（E2E 前置，debug-test 会校验 static mtime）
3. **E2E**：`cd /home/kity/oclab/peekview && E2E_SPEC=e2e/star.spec.ts make debug-test 2>&1 | tail -25; echo "GATE_EXIT: ${PIPESTATUS[0]}"` —— timeout 300s。结果写入 `P5-test-results/e2e.md`（Playwright 实跑结果 + 截图路径）
   - 注意：**任一命令超 300 秒未完成 → 立即停止**，写 progress 报告"命令超时"，返回主 Agent，不要重试、不要等待
   - E2E 失败时检查：是否 debug server 未跑（:8888 应已运行）/ static mtime 校验失败 / 真实断言失败——如实记录到 e2e.md，不掩盖
4. 有 failed → fail-list.txt 逐行列出（`FAILED ` 前缀）；无 → 空文件
5. 预存失败处理（如有）：unit.md 标注 + known-failures.md 登记

## 约束

- **只跑命令 + 落盘**：不修改任何源码/测试（build-frontend 构建产物允许）
- 环境隔离：debug :8888 隔离；严禁触碰生产 :8080 / ~/.peekview/
- Playwright CDP：连接 localhost:18800，connectOverCDP 模式；**不要 browser.close()**（会杀 Chrome）
- 每完成一个关键步骤（每步命令跑完/产出写完），**立即追加写 progress 心跳**（P5-progress.md）
- 日志末行格式：`EXIT_CODE: <n>`（0 成功）
- 状态标记：`[PROD_TOUCHED]`/`[PROD_NOT_TOUCHED]`

## 产出（路径约束）

1. `agate-workspace/tasks/TPV0093-star-lifecycle/P5-test-results/unit.md`（frontend 部分）
2. `agate-workspace/tasks/TPV0093-star-lifecycle/P5-test-results/e2e.md`
3. `agate-workspace/tasks/TPV0093-star-lifecycle/P5-test-results/fail-list.txt`
4. 如需：`known-failures.md`

⚠️ 路径硬约束：写入上述任务目录，不写 /tmp。

## 门槛

- unit.md 存在且含测试运行器输出签名（passed/failed 计数）
- e2e.md 存在（E2E 已执行，ui_affected 必填）
- fail-list.txt 存在（可为空）
- 日志末行 EXIT_CODE: 0（命令超时/失败 → 如实报告，不伪造）

## 返回给我（重要）

只返回两行：
  1. 产出文件路径
  2. 一句话摘要（不超过 30 字：frontend N passed + E2E M passed / 失败 / 超时）
