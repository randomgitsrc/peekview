...
# P2 派发上下文 — plan-design-review（复核轮 r2）
...
- 上轮评审：`P2-review-design.md`（needs-revision：6 项 + 4 项补充建议）
- 修订后方案：`P2-design.md`（architect 已闭合）
- 本轮只复核 6 项 + 4 项补充建议是否闭合，不必重复完整评审

## 复核目标

1. **design-1**：移动端星标按钮落点（EntryDetailMobileBar 底部栏 mobile-star-toggle / OverflowMenu 兜底）
2. **design-2**：管理页三态（空/加载/错误）+ Starred tab 空态 + 批量按钮禁用条件 + 移除确认
3. **design-3**：E1 跳转查看入口二选一明确（Toast action 扩展 vs tooltip 内嵌链接）
4. **design-4**：client.ts transformListItem/transformEntry 新字段映射（starCount/isStarred/countdown）
5. **design-5**：a11y 节（aria-pressed/aria-label/语义色 token/button 可键盘/❓ 可点击/checkbox aria-label）
6. **design-6**：归档 Toast 双文案（即将归档 vs 已归档）
7. 4 项补充建议：Starred tab 不含墓碑 / filter 语义表固化 / 豁免标签 footer 条件 / URL 态耦合触点点名
8. 修订是否引入新问题
...
`agate-workspace/tasks/TPV0093-star-lifecycle/P2-review-design.md`（覆盖写回，r2）

## 门槛

- status 三选一；approved 时标注 6 项 + 4 建议闭合确认
