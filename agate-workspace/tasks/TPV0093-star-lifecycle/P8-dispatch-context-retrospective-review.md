...
trace_id: TPV0093-retrospective-review-20260816
status: ready
---

# 复盘独立评审派发上下文 — retrospective-review
...
独立评审复盘文档 `docs/reviews/TPV0093-retrospective-20260816.md`，验证：客观性（基于 session 证据）、机理定位准确性、处理措施可行性、建议意见有效性。**只审不写**（不修改复盘文档）。

## 评审对象

`/home/kity/oclab/peekview/docs/reviews/TPV0093-retrospective-20260816.md`

## 客观依据（可自行核查）

**⚠️ 先读证据包：`agate-workspace/tasks/TPV0093-star-lifecycle/P8-retrospective-evidence.md`**（主 Agent 已从 opencode.db 提取全部关键 session 证据——**禁止自行查询 opencode.db**，7.6GB 会挂起）

1. 证据包（上述路径）：E1/E2 卡死时间线 + P6 修复过程 + 关键命令耗时
2. 复盘文档：`docs/reviews/TPV0093-retrospective-20260816.md`
3. 小文件抽查（只读）：`scripts/run-e2e-tests.sh` / `frontend-v3/e2e/star.spec.ts` / `backend/peekview/services/entry_service.py`(line~590) / `frontend-v3/src/views/StarManageView.vue`(Math.ceil) / `git log --oneline -12`
4. 项目约定：`AGENTS.md`

## 命令纪律（强制）

- 任何 bash 命令必须设 timeout（≤30s）；命令超时 → 立即停止、记录"命令超时"、跳过该抽查项继续
- 禁止：查询 opencode.db、跑测试、跑长命令
- 抽查项读文件用 Read 工具（不快照大文件）

## 评审重点

1. **客观性**：复盘中的时间线/事件/根因是否与 session 记录一致（抽查 2-3 个关键断言，如 E1 卡死时长、BUG-1 根因、脚本错误 S1-S5）
2. **机理定位准确性**：管理/技术/agate 机制/agent 遵守 四维度划分是否合理、归因是否准确（是否甩锅给机制/是否漏掉主 Agent 责任）
3. **处理措施可行性**：已实施措施是否真实有效（抽查 run-e2e-tests.sh timeout 包裹、login 修复、P6 主 Agent 执行）
4. **建议意见有效性**：对 agate 协议 4 条建议（timeout 字段/超时兜底节/串行提示/心跳扩展）是否可落地、是否与 agate 现有机制冲突；对项目 4 条建议是否可执行
5. **遗漏检查**：复盘是否遗漏重要问题（如 backend verifier 访问生产库路径、cso subagent 无 Write 工具的两次落盘、TPV0092 历史教训未内化等是否已覆盖）

## 约束

- 只审不写
- 结论引用具体锚点（复盘章节号/证据来源）
- 产出文件 Header status: draft → approved / needs-revision / rejected

## 产出

`/home/kity/oclab/peekview/docs/reviews/TPV0093-retrospective-review-20260816.md`

## 门槛

- 文件存在且非空；status 三选一
- approved 时标注：客观性核实结论 + 机理定位判定 + 建议有效性评估 + 无遗漏确认
- needs-revision/rejected 时逐条列出问题（锚点 + 证据 + 修改建议）
