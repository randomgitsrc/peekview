...
# P2 派发上下文 — plan-eng-review（复核轮 r2）
...
- 上轮评审：`P2-review-eng.md`（rejected：BLOCKER-1/2/3 + N1-N7）
- 修订后方案：`P2-design.md`（candidate_count=6，architect 已闭合）
- 本轮只复核 BLOCKER 是否闭合 + N 项是否落实，不必重复完整评审

## 复核目标

1. **BLOCKER-1**：get_entry archived 短路 is_public 前置检查（§4.3 重构判定顺序）——代码语义是否自洽；「公开→星标→转私有→归档」链路是否被覆盖
2. **BLOCKER-2**：star 端点授权语义（§4.6 契约行）——"只允许对可读 entry 建星标"是否清晰；非公开仅 owner/admin；archived 私有仅 owner/admin/已星标；未知 slug 404
3. **BLOCKER-3**：迁移不再复用 user_version——数据幂等 backfill（archive_delete_at IS NULL 条件）或独立 migrations 表；BDD-27 用例（存量库 user_version=2 → 升级 → backfill 生效）是否写入 P3 计划
4. N1-N7 落实：delete_user 清扫顺序 / update_entry 双路径 / candidate_count=6 一致 / list_starred 可见性 / 时区 naive / P5_e2e E2E_SPEC 固化 / star 并发幂等
5. 修订是否引入新问题

## 约束

- 只审不写
- 闭合 → status: approved；仍缺 → rejected/needs-revision（写明剩余项）

## 产出

`agate-workspace/tasks/TPV0093-star-lifecycle/P2-review-eng.md`（覆盖写回，r2）

## 门槛

- status 三选一；approved 时标注 3 BLOCKER 闭合确认 + N 项落实
