...
# P1 派发上下文 — requirements-review（复核轮 r2）
...
- 上轮评审：`agate-workspace/tasks/TPV0093-star-lifecycle/P1-review.md`（status: needs-revision，REV-1..4）
- 修订后需求基线：`agate-workspace/tasks/TPV0093-star-lifecycle/P1-requirements.md`（28 BDD，analyst 已闭合 4 项修订）
- 本轮只复核修订是否闭合 + 是否引入新问题，不必重复完整评审（但 BDD 逐条判定表可快速复查）

## 复核目标

1. **REV-1 闭合确认**：share 独立授权通道声明（§2.3 M1 补充 + BDD-28）是否清晰、无歧义；BDD-28 可二值判定；与 SUGGEST 6 的"登录用户通道"限定是否自洽
2. **REV-2 闭合确认**：E7 已声明 reason=expired 为保留型枚举、当前不产出；BDD-10 是否因此需要调整（确认 BDD-10 语义不受影响）
3. **REV-3 闭合确认**：D8（deleted_by 引用安全策略）+ E10（作者账号删除路径）声明是否完整，是否覆盖 delete_user / delete_self 两条路径
4. **REV-4 闭合确认**：§3.4 同源继承说明 + BDD-15/16 补充是否到位
5. 修订是否引入新矛盾 / 新 NEED_CONFIRM / BDD 编号是否仍连续（1-28）

## 约束

- 只审不写
- 若修订闭合 → status: approved；仍缺 → needs-revision（写明剩余项）

## 产出

`agate-workspace/tasks/TPV0093-star-lifecycle/P1-review.md`（覆盖写回，r2）

## 门槛

- status: approved / rejected / needs-revision（三选一）
- approved 时逐条引用 BDD 编号锚点（可简化：标注"REV-1..4 已闭合" + 快速复查结论）
