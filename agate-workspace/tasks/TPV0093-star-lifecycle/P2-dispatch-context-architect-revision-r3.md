...
- 上轮产出：`P2-design.md`（r2 已闭合 3 BLOCKER + N1-N7 + design 6 项）
- 上轮评审：`P2-review-eng.md`（r2：3 BLOCKER 闭合确认 + N1-N7 落实确认，新增 BLOCKER-4 + N8/N9）
- design 评审已 approved（`P2-review-design.md` r2）
- 本轮修订极小（单点守卫），闭合 BLOCKER-4 后即可 approved

## 修复目标

1. **BLOCKER-4（必须闭合）**：§4.3 archived 分支恢复显式匿名守卫——ownerless legacy archived（owner_id IS NULL）条目不得匿名可读。修订伪代码为评审建议形式：
   ```
   if entry.status == EntryStatus.ARCHIVED:
       if not is_admin and current_user_id is None:
           raise NotFoundError
       if not is_admin and entry.owner_id != current_user_id and not _has_live_star(...):
           raise NotFoundError
   ```
   或合并条件。不变式文字「匿名（user=None）→ archived 404（BDD-16/E8 不变）」保持成立。
2. **N8（建议一并处理）**：非 archived 分支（else）`entry.owner_id != current_user_id` 收紧为 `(current_user_id is None or entry.owner_id != current_user_id)`——现有代码既有行为，顺手修复（本任务正重构此函数）
3. **P3 用例增补**：§7 补「ownerless archived（owner_id=NULL）→ 匿名请求详情 → 404」（BLOCKER-4 回归锚）；§7 补「转私有后取消星标仍 200」（N9，BDD-3 变体）

## 约束（沿用）

- 只改 P2-design.md；环境隔离；修订后自检 frontmatter 四字段 + candidate_count=6 一致 + dispatch_plan 不变

## 产出

`agate-workspace/tasks/TPV0093-star-lifecycle/P2-design.md`（修订版 r3）

## 门槛

- BLOCKER-4 闭合（§4.3 伪代码含匿名守卫）+ N8 处理 + P3 两用例增补
