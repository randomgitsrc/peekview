...
# P2 派发上下文 — plan-eng-review（复核轮 r3，极小复核）
...
- 修订后方案：`P2-design.md`（r3：architect 已闭合 BLOCKER-4 + N8 + P3 两用例）
- 本轮只复核 BLOCKER-4 + N8 + P3 用例闭合即可（其余已 approved 基线，不需重复）

## 复核目标

1. **BLOCKER-4**：§4.3 archived 分支恢复显式匿名守卫（ownerless legacy archived 匿名 → 404）——伪代码是否按建议形式；不变式文字保持
2. **N8**：非 archived 分支收紧为 `(current_user_id is None or entry.owner_id != current_user_id)`（顺手修复既有漏洞）
3. **P3 用例**：§7 补「ownerless archived → 匿名请求详情 → 404」（BLOCKER-4 回归锚）+「转私有后取消星标仍 200」（N9）
4. 修订是否引入新问题

## 约束

- 只审不写
- 闭合 → status: approved；仍缺 → needs-revision（写明剩余项）

## 产出

`agate-workspace/tasks/TPV0093-star-lifecycle/P2-review-eng.md`（覆盖写回，r3）

## 门槛

- status: approved / rejected / needs-revision
- approved 时标注 BLOCKER-4 闭合确认 + N8/N9 落实
