...
- 上轮产出：`agate-workspace/tasks/TPV0093-star-lifecycle/P1-requirements.md`
- 上轮派发上下文：`agate-workspace/tasks/TPV0093-star-lifecycle/P1-dispatch-context-analyst.md`（其目标/约束/输入沿用，本次只做修订）
- 上轮评审：`agate-workspace/tasks/TPV0093-star-lifecycle/P1-review.md`（status: needs-revision）

## 修复目标（评审 4 项修订，逐条闭合）

1. **REV-1【关键，安全】**：share×archived 读取规则未定义。当前代码 share 读取路径（`get_entry_with_share` / `_check_share_cookie` / files `_resolve_entry` share 兜底）绕过 archived 权限判定；`create_share` 拒绝在 archived 上新建但既有 share 归档后仍可用。请在 §2.3 M1 或 SUGGEST 6 处**显式声明 share token 是独立授权通道**（与星标无关）或反向声明，并**补 1 条 BDD**（Given archived entry + 有效 share / When 持 share 读取 / Then 200 或 404 二选一，与声明一致）。
2. **REV-2【自洽性】**：E7 与 SUGGEST 1 矛盾——「最后星标取消且剩余≤0」过渡路径删除时 0 星标，按 SUGGEST 1 不建墓碑，reason=expired 实际永不产出。请修订 E7 声明 reason=expired 为**保留型枚举、当前逻辑不产出**（BDD 不验收，推荐方案），或改 SUGGEST 1/BDD-10 语义（不推荐）。
3. **REV-3【边界】**：作者账号删除路径（`AdminService.delete_user` / `delete_self` 逐个调 `delete_entry`）若 entry 有星标将生成墓碑，deleted_by 指向被删作者 → FK 悬挂。请在 D2/E 维度补声明：作者账号删除仍生成墓碑 + deleted_by 处理策略（如存 username 快照 / 非 FK 存储 / ON DELETE SET NULL），交由 P2 落实。
4. **REV-4【轻度】**：BDD-15/16 只命名详情/raw/文件内容三处。请在 §3.4 注明 download/render/短链路径同源继承关系（P6 抽查其一）。

## 约束（沿用上轮）

- 只改 P1-requirements.md，不新建文件、不动其他文件
- 环境隔离：只读代码，不跑测试、不修改源码
- 修订处标注 `[BASELINE_CHANGE: REV-N 修订]` 或直接修订（本轮是评审打回，非基线保护场景）
- 修订后自检：BDD 编号保持 1-27 连续 + 新增 BDD 编号接续（如 BDD-28），不跳号；NO_NEED_CONFIRM 声明保持有效；frontmatter 机器字段不变（若新增 BDD 无需改 phases/packages）

## 产出

`agate-workspace/tasks/TPV0093-star-lifecycle/P1-requirements.md`（修订版）

## 门槛

- 4 项修订全部闭合（每项修订处可被评审对应到）
- 新增 BDD 可二值判定、编号连续
- 修订后无新的 NEED_CONFIRM
