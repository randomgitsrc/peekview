...
# P2 派发上下文 — plan-design-review（设计评审）
...
前端 UI 设计评审 P2-design.md §6（前端设计）：交互状态覆盖率 / AI Slop 风险 / 移动端考虑 / 可访问性 / 组件完整性。产出评审意见。

## 上游关联

- 评审对象：`agate-workspace/tasks/TPV0093-star-lifecycle/P2-design.md`（§6 前端设计 + §6.5 稳定测试标识）
- 需求基线：`P1-requirements.md`（BDD-18/19/20/21/22/23/24/25/26 前端相关）
- `docs/specs/peekview-star-function-20260812.md`（§4 产品交互设计）

## 评审重点（本任务特有）

- **详情页星标按钮**（§6.1/BDD-1/2/3/4/6/23）：乐观更新/回滚/重复提示/匿名引导/归档 Toast——loading/error/empty 状态是否说清；StarToggle 组件 input/output 描述
- **Starred tab**（§6.2/BDD-18/19）：tabs 机制复用、URL 态同步、登录可见性
- **星标管理页**（§6.3/BDD-20/21/22/14）：分类 tab/红色倒计时/墓碑卡片/批量移除——各分类空态、加载态、错误态；墓碑卡片交互（点击看原因/移除）
- **作者豁免标签**（§6.4/BDD-24/25/26）：豁免标签文案 + ❓悬停说明 + 强制删除二次确认弹窗（明示 N）——确认对话框交互完整性
- **移动端**：星标按钮/管理页/豁免标签在移动端布局（PeekView 有移动端优化历史）
- **可访问性**：星标按钮语义（aria-pressed?）、键盘导航、红色倒计时标签的色彩对比
- **组件完整性**：§6 涉及的每个 UI 组件（StarToggle/StarManageView/墓碑卡片/豁免标签/强制删除确认）是否有完整 input/output 描述（触发条件 + 用户输入 + 预期输出）；遗漏会让 P4 implementer 凭空实现
- **稳定测试标识**（§6.5）：data-testid 清单是否覆盖 P6 验收所有交互点

## 输入文件（必读）

1. `agate-workspace/tasks/TPV0093-star-lifecycle/P2-design.md`（§6 前端设计）
2. `agate-workspace/tasks/TPV0093-star-lifecycle/P1-requirements.md`
3. `docs/specs/peekview-star-function-20260812.md`（§4 产品交互设计）
4. 现有前端模式（判断"复用现有模式"是否成立）：`frontend-v3/src/views/EntryListView.vue`（tabs）、`components/EntryDetailHeader.vue`、`components/EntryCard.vue`、`components/ConfirmDialog.vue`、`composables/useToast.ts`

## 约束

- 只审不写
- 结论引用具体锚点（§6 小节/BDD 编号/组件名）
- Header status: draft → approved / rejected / needs-revision

## 产出

`agate-workspace/tasks/TPV0093-star-lifecycle/P2-review-design.md`

## 门槛

- 文件存在且非空；status 三选一
- 各维度评分（0-10）+ 是否有 BLOCKER/needs-revision（列出具体项 + 建议）
