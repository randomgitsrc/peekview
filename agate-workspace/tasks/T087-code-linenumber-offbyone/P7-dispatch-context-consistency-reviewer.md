...
# P7 dispatch-context — T087 consistency-reviewer

## 目标

跨文件一致性检查，产出 P7-consistency.md（无 BLOCKER / DESIGN_GAP 配对 / SCOPE+ 闭环）。

## 检查清单

1. **DESIGN_GAP 配对**：P4-implementation.md 的 DESIGN_GAP 声明 → 逐条转抄 + REVIEWED 标记（P4 无 DESIGN_GAP 声明，确认即可）
2. **SCOPE+ 闭环**：P1-requirements.md 无 [SCOPE+]（T087 无 SCOPE+ 增补，确认即可）
3. **跨文件一致性**：
   - P2 packages 与 P8 release bump 范围一致（P2 packages=frontend-v3，P8 待 bump）
   - P1 BDD 数（10）= P6 验收数（10 PASS）
   - P4 实现路径与 P2 方案设计吻合（方案 A 调用方共享 trim）
4. **未决项清零**：P1 无残留 [NEED_CONFIRM]、[BLOCKER]、[DEVIATION-CRITICAL]

## 跨文件检查项

| 检查项 | P2 声明 | P4 实现 | P6 验收 | 一致性 |
|--------|---------|---------|---------|--------|
| packages | frontend-v3 | useShiki.ts | - | 确认 |
| trim 点 | highlight + highlightCode 调用方 | line 185 + 203 | BDD-1/2/5/6/7/9 PASS | 确认 |
| renderLineNumbers | 不改 | 未改 | - | 确认 |
| gate_commands | P3=vitest P5=vitest+typecheck P5_e2e=viewer.spec.ts | - | P5 全执行 | 确认（viewer.spec.ts 预存失败登记 known-failures） |
| BDD 数 | P1=10 | - | P6=10 PASS | 确认 |
...
- P1-requirements.md（10 BDD + packages/domains/risk/phases）
- P2-design.md（方案 A + 四字段 + gate_commands）
- P3-test-cases.md（9 测试覆盖 BDD-1~7）
- P4-implementation.md（实现摘要 + 无 DESIGN_GAP）
- P5-test-results/（unit.md + e2e.md + typecheck.md）
- P6-acceptance.md（10/10 PASS）
- known-failures.md（viewer.spec.ts 预存失败）

## 输出

`docs/tasks/T087-code-linenumber-offbyone/P7-consistency.md`，无 [BLOCKER] / [DEVIATION-CRITICAL]。

P7-consistency.md Header：
---
phase: P7
task_id: T087-code-linenumber-offbyone
type: consistency
