...
执行 TPV0088 的 P6 验收：对照 P1 的 9 条 BDD 逐条实跑验证，产出 `P6-acceptance.md` + `P6-evidence/`。本任务虽是测试基础设施改动（ui_affected: false），但验收必须实跑验证 19 用例 + mtime 三态。

## 上游关联

- 输入文件（必读）：
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P0-brief.md`（环境约束）
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P1-requirements.md`（9 BDD）
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P2-design.md`（方案）
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P5-test-results/`（P5 已验证 + known-failures）
  - `AGENTS.md`（项目约定）

## 环境状态

- debug backend :8888 已运行（P5 启动），fixture 已 seed
- static 新鲜（P4 修复后已 build-frontend）
- CDP Chrome :18800 可用
- 严禁触碰生产 :8080 / ~/.peekview/

## BDD 验收清单（9 条）

### 子任务 A（viewer.spec.ts 19 用例）
- **BDD-1**：全部 19 用例在 debug backend 实跑通过（`E2E_SPEC=e2e/viewer.spec.ts make debug-test`，38/38 = 19×2）
- **BDD-2**：无 `/#/entry/` 残留（grep 判定）
- **BDD-3**：死选择器替换为现存 DOM（无 .code-header/.mobile-actions/.toc-btn/.list-header/.btn-icon）
- **BDD-4**：无 lu4prg/ngajri 残留（grep 判定）
- **BDD-5**：TC-041 单文件断言有数据支撑（json-api-config 单文件，.file-sidebar count=0）

### 子任务 B（static 新鲜度校验）
- **BDD-6**：过期 static 被拦截（touch src 不 rebuild → make debug-test Step 1 失败、提示 make build-frontend）
- **BDD-7**：新鲜 static 放行（build-frontend 后 → make debug-test 通过）
- **BDD-8**：先 build 后 debug-test 正常流程不被误伤（make debug-quick → debug-test）

### 通用
- **BDD-9**：数据隔离不被破坏（e2e-safety-check.sh 既有 5 项检查通过，生产无 e2e- 数据）

## 执行步骤

1. 确认环境（debug :8888 + fixture + static 新鲜）
2. 跑 `E2E_SPEC=e2e/viewer.spec.ts make debug-test`（BDD-1）——记录结果到 P6-evidence/
3. BDD-2/3/4：grep 判定（viewer.spec.ts 无残留）
4. BDD-5：实跑或引用 P5 E2E 结果（TC-041）
5. BDD-6：构造过期 static 场景（touch 一个 src 文件不 rebuild）→ 跑 make debug-test → 验证 Step 1 拦截 + 报错提示；**测完恢复**（touch/build 回新鲜）
6. BDD-7/8：build-frontend 后跑 make debug-test → 放行
7. 写 P6-acceptance.md（9 条 BDD，PASS/FAIL 二值 + 证据引用）

## 产出要求

1. `docs/tasks/TPV0088-e2e-test-infra-hardening/P6-acceptance.md`
   - frontmatter：pass: / fail: / ui_affected: false
   - 每条 BDD 行首 `- PASS` 或 `- FAIL` + 证据引用
2. `docs/tasks/TPV0088-e2e-test-infra-hardening/P6-evidence/`（非空，日志/断言文件）
...
- 无证据的 PASS = gate 不通过
- BDD-6 构造过期场景时**注意恢复**（避免污染后续验证）
- 状态标记：触发写 `[PROD_TOUCHED] {描述}`，未触发写 `[PROD_NOT_TOUCHED]`

## 返回给主 Agent

两行：P6-acceptance.md 路径 + 一句话摘要（X/9 PASS，不超过 30 字）
...
<objective_info>
- 环境状态：debug backend :8888 已运行；CDP Chrome :18800 可用；static 新鲜
- 关键标识：E2E spec viewer.spec.ts（19 用例 38 次运行）；e2e-safety-check.sh Check 6（--test-mtime 两态）
- 查证结果：P5 已验证（E2E 38/38 + --test-mtime 通过）；P1 9 BDD
</objective_info>
