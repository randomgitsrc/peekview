...
复核修订后的 P1-requirements.md（analyst 已按上轮评审意见修订：20→19 用例计数 + 3 建议项），确认全部修订落地后批准。

## 上轮评审意见（修订依据）

`docs/tasks/TPV0088-e2e-test-infra-hardening/P1-review.md`（needs-revision，1 硬性 + 3 建议）

## 修订内容核对清单

1. **修订项 1（硬性）**：全文档 "20 用例" → "19 用例"（§1/BDD-1/P6 节/风险表）；BDD-1 Then 改为 `19/19 用例 PASS` 或"全部用例 PASS"
2. **建议项 2**：mtime 比对口径统一（src 最新 mtime vs static/index.html mtime）
3. **建议项 3**：debug-stop 清理范围示例修正（清 /tmp/peekview-debug，不涉 static）
4. **建议项 4**：风险表引用同步（不再出现 20/20）
5. BDD 编号连续，frontmatter 机器字段未被破坏

## 上游关联

- 输入文件（必读）：
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P1-requirements.md`（修订后被评审对象）
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P1-review.md`（上轮评审意见，核对依据）
...
`docs/tasks/TPV0088-e2e-test-infra-hardening/P1-review.md`（覆盖写，gate 只认此文件名）
...
- 关键标识：P1-requirements.md 修订轮完成（20→19 用例计数 + 3 建议项）
- 查证结果：需逐一核对修订落地情况
</objective_info>
