...
执行 TPV0088 的技术验证（gate_commands.P5，P2 已固化），产出 `P5-test-results/`。验证范围：后端 pytest（确认未波及）+ typecheck + E2E（viewer.spec.ts 19 用例）。

## 上游关联

- 输入文件（必读）：
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P0-brief.md`（环境约束）
  - `docs/tasks/TPV0088-e2e-test-infra-hardening/P2-design.md`（gate_commands 声明 + env_constraints）
...
1. **P3 自检确认**：`bash scripts/e2e-safety-check.sh --test-mtime`（Check 6 两态）
2. **P5 后端全量**：`make test-quick`（backend pytest，确认未波及后端——本任务只改测试资产 + shell 脚本）
3. **P5_typecheck**：`make typecheck`（vue-tsc --noEmit，CI 强制）
4. **P5_e2e**：`E2E_SPEC=e2e/viewer.spec.ts make debug-test`（19 用例——这是本任务核心验证！需先启动 debug backend + seed）
   - 启动顺序：`make debug-start` → `make debug-seed`（若无 fixture）
   - 若 19 用例不全过，需分析每例失败原因（真失败 vs 断言仍假绿）

## 环境状态

- debug backend :8888 当前**未运行**——需启动（`make debug-start`，P4 已 build-frontend 反映最新 static）
- CDP Chrome :18800 可用
- 严禁触碰生产 :8080 / ~/.peekview/

## 产出要求

1. `docs/tasks/TPV0088-e2e-test-infra-hardening/P5-test-results/unit.md` — 后端 pytest 结果（failed 计数）
2. `docs/tasks/TPV0088-e2e-test-infra-hardening/P5-test-results/fail-list.txt` — failed 测试 id 逐行（无失败则为空）
3. `docs/tasks/TPV0088-e2e-test-infra-hardening/P5-test-results/e2e.md` — viewer.spec.ts 19 用例逐条结果（这是核心！）
4. `docs/tasks/TPV0088-e2e-test-infra-hardening/P5-test-results/typecheck.md` — typecheck 结果
5. 若发现预存失败：登记到 `known-failures.md`（从 `~/.agate/assets/templates/known-failures-template.md` 拷贝）
...
两行：P5-test-results/ 路径 + 一句话摘要（19 用例结果，不超过 30 字）
...
<objective_info>
- 环境状态：debug backend :8888 未运行需启动；CDP Chrome :18800 可用；static 已 build-frontend（P4）
- 关键标识：viewer.spec.ts 19 用例（修复后）；e2e-safety-check.sh --test-mtime（两态已通过）；Makefile debug-test Step 1 新 Check 6
- 查证结果：P4 实现 approved；P3 红灯全转绿
</objective_info>
