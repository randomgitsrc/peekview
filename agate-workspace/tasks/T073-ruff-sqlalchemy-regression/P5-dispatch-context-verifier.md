...
### 目标
执行 P2-design.md 的 gate_commands.P5，验证 19 处 SQLAlchemy 修复后全量测试通过。

### 约束
- 只读验证，不改代码
- gate_commands.P5: `cd backend && .venv/bin/python -m pytest tests/ -q --tb=no`
- 验证 ruff E711/E712: `cd backend && python3 -m ruff check peekview/ --select E711,E712`
- 环境隔离：conftest.py autouse

### 上游关联
- P4 已完成 19 处 SQLAlchemy 修复 + E711/E712 ignore
- 预期结果：0 failed
