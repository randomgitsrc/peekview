# P5 派发指引 — T075 frontend verifier

## 目标

执行 P2-design.md gate_commands.P5 的 frontend 部分，产出 `P5-test-results/frontend-unit.md` + `P5-test-results/frontend-fail-list.txt` + `P5-test-results/e2e.md`。

## 任务背景

T075 前端改动：TableView/TreeView + 切换机制 + 格式检测 + 5 个新 spec + E2E spec。P4 已验证 vitest 1177 passed、vue-tsc 0、build success。P5 独立验证（不信 P4 自报）。

## 约束

- 只跑 frontend 验证（后端由另一个 verifier 并行负责）
- 只读验证，不改任何代码
- E2E 用 make debug-test（需 debug backend :8888）——若 debug backend 未启动，先启动
- 测试隔离：debug 环境 /tmp/peekview-debug/，不触生产

## 上游关联

- P2-design.md gate_commands.P5:
...
- P4-implementation-frontend.md：实现产出

## 输入文件

1. `docs/tasks/T075-structured-data-viewer/P2-design.md`（gate_commands）
...
# E2E（需 debug backend）
E2E_SPEC=e2e/structured-data-viewer.spec.ts make debug-test
```

## 产出

1. `docs/tasks/T075-structured-data-viewer/P5-test-results/frontend-unit.md` — 标注 failed 数量 + test runner 输出签名
2. `docs/tasks/T075-structured-data-viewer/P5-test-results/frontend-fail-list.txt` — failed 测试 id 逐行（无失败可为空）
3. `docs/tasks/T075-structured-data-viewer/P5-test-results/e2e.md` — E2E 实跑结果（Playwright 输出 + 截图路径）

日志格式：文件末行必须是 `EXIT_CODE: <n>`（0=成功）。

## 门槛

- 3 个产出文件存在
- frontend-unit.md 含 test runner 输出签名（passed/failed 计数 >0）
...
只返回两行：产出路径 + 一句话摘要（passed/failed + E2E 结果）。
