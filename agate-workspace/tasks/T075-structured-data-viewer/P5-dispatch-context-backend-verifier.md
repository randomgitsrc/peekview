# P5 派发指引 — T075 backend verifier

## 目标

执行 P2-design.md gate_commands.P5 的 backend 部分，产出 `P5-test-results/backend-unit.md` + `P5-test-results/backend-fail-list.txt`。

## 任务背景

T075 后端改动：language.py .tsv→'tsv' + PLAIN_TEXT_LANGS 加 tsv。P4 已验证 test_language.py 63 passed，全量 1008 passed。P5 独立验证（不信 P4 自报）。

## 约束

- 只跑 backend 验证（前端由另一个 verifier 并行负责）
- 用 venv Python（`.venv/bin/python`），严禁系统 python / pipx
- 只读验证，不改任何代码
- 测试隔离：pytest conftest autouse 隔离，不触生产库

## 上游关联

- P2-design.md gate_commands.P5_backend: `cd backend && .venv/bin/python -m pytest tests/test_language.py -q --tb=no`
- P4-implementation-backend.md：实现产出

## 输入文件

1. `docs/tasks/T075-structured-data-viewer/P2-design.md`（gate_commands）
2. `docs/tasks/T075-structured-data-viewer/P4-implementation-backend.md`

## 验证命令

```bash
cd backend && .venv/bin/python -m pytest tests/test_language.py -q --tb=no
cd backend && .venv/bin/python -m pytest tests/ -q --tb=no   # 全量回归
cd backend && python3 -m ruff check peekview/ tests/
```

## 产出

1. `docs/tasks/T075-structured-data-viewer/P5-test-results/backend-unit.md` — 标注 failed 数量 + test runner 输出签名
2. `docs/tasks/T075-structured-data-viewer/P5-test-results/backend-fail-list.txt` — failed 测试 id 逐行（无失败可为空文件）
...
- 两个产出文件存在
- backend-unit.md 含 test runner 输出签名（PASSED/FAILED/passed/failed 计数 >0）
- 全量测试无回归（或标注预存失败）
- [PROD_NOT_TOUCHED] 标记

## 返回给主 Agent

只返回两行：产出路径 + 一句话摘要（passed/failed 计数）。
