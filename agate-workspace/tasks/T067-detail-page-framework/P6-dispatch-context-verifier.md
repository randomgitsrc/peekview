...
逐条验证 P1 的 12 条 BDD 验收条件，产出 P6-acceptance.md + P6-evidence/。

## 约束

- 每条 BDD 只允许 PASS 或 FAIL
- 所有 PASS 必须有文件引用
- UI 任务：截图需 Playwright CDP 验证
- 先验证功能（用户视角），再满足 gate 格式

## 12 条 BDD

1. BDD-1: 详情页 Sign in 入口（匿名用户）
2. BDD-2: 详情页 Sign in 隐藏（已登录用户）
3. BDD-3: 详情页 Sign in 登录后响应式消失
4. BDD-4: 品牌字标显示（桌面端）
5. BDD-5: 移动端品牌元素
6. BDD-6: Explore 导航入口
7. BDD-7: 移动端底栏文案修正
8. BDD-8: reads 计数格式统一
9. BDD-9: 首页 Sign in 视觉权重
10. BDD-10: 桌面端 tooltip hover 验证
11. BDD-11: authState loading 态无闪烁
12. BDD-12: zen mode 下品牌/Sign in 隐藏

## 验证环境

- Debug backend: http://127.0.0.1:8888
- Playwright CDP: http://127.0.0.1:18800
- Test data: alice/bob/carol (testpass123)
- Public entry example: 任何公开 slug

## 输入文件

1. docs/tasks/T067-detail-page-framework/P1-requirements.md — 12 条 BDD
2. docs/tasks/T067-detail-page-framework/P2-design.md — 方案设计
3. frontend-v3/src/views/EntryDetailView.vue — 实现代码
4. frontend-v3/src/views/LandingView.vue — 首页实现
