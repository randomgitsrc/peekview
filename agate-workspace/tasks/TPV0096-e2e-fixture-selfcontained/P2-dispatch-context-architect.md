# P2-dispatch-context-architect — TPV0096
...
产出 P2-design.md：3 个 E2E spec「fixture 自建 + afterEach 清理 + 死选择器/goto 迁移 + 假绿修复 + 防生产护栏」的实施方案。P1 已声明 design_trivial: true + follows_existing_pattern（teams-page 清理队列 + render-regression createEntry），可只写 1 个候选方案（frontmatter `candidate_count: 1`，正文须附简化理由）。

## 约束

- 只改 `frontend-v3/e2e/*.spec.ts`（3 个 spec）与 `docs/process/debug-workflow.md`（新增「E2E 编写规范」节）；不改产品代码（backend/、frontend-v3/src/）；不 bump 版本
- fixture slug 用 `e2e-` 前缀（主 Agent 已采纳 SUGGEST-3），与 seed 24 条隔离；具体命名本阶段定稿
- 认证配对：匿名创建（ALLOW_ANONYMOUS_CREATE=true，强制 is_public）+ 匿名删除（debug 未配 API key → allow_local）必须同上下文，不得混用登录态（P1 §2.3 已实证）
- mermaid-visual.spec 自起 chromium 的结构保留还是归一到 test fixture——本阶段定稿（注意 BDD-9 双遍执行口径：归一到 fixture 后只在 chromium/Mobile Chrome 各跑一遍，可能简化口径，但改动面与收益自行权衡）
- 防生产护栏（BASE_URL 非 :8080/prod fail-fast）三个 spec 都要加（P1 §2.6）
- gate_commands 在本阶段固化，P4-P6 不得更改——按项目 Makefile target 声明（不手写裸命令），并遵守「禁止 && 链」「禁止 P3_xxx 检测键」的 P2 卡规则
- mermaid 渲染时序不做硬等待重构（waitForTimeout 保留，TPV0097 范围）
- 影响面梳理节（改什么/不改什么/风险在哪）必须写在候选方案之前，证据落 grep/读码清单
- 子派发能力：不启用
- 环境清理：minimal_validation 若创建探针 entry，用 e2e- 前缀 slug 并当场删除，验证 DB 无残留

## 上游关联

- P1 评审 approved：13 BDD 全部可二值判定；死选择器迁移/假绿修复/路由迁移已并入基线（P1 §2.1/2.2/2.4）
- 主 Agent 采纳 SUGGEST-1~4（见 P1-review.md 与 orchestrator-log）
- BDD-9 口径：3 spec 在 chromium 与 Mobile Chrome 两个 project 下分开统计均 0 failed

## 输入文件

- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P1-requirements.md（需求基线，含 13 BDD）
- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P0-brief.md
- frontend-v3/e2e/teams-page.spec.ts（创建+清理队列完整范式）
- frontend-v3/e2e/render-regression.spec.ts（createEntry + gotoEntry helper 参照）
- frontend-v3/e2e/mermaid.spec.ts / mermaid-check.spec.ts / mermaid-visual.spec.ts（改造对象）
- frontend-v3/playwright.config.ts（project 定义）
- scripts/run-e2e-tests.sh（E2E 运行入口，gate_commands 依据）
- Makefile 中 debug-test / test-frontend / lint / typecheck 相关 target（gate_commands 依据）
- docs/process/debug-workflow.md（规范节落点）

## 客观查证信息

- debug :8888 已在运行（主 Agent 已启动，24 条 seed 已灌入）；minimal_validation 可直接 curl 验证匿名创建/删除配对（P1 §2.3 是代码级推断，建议实测确认）；探针 entry 用 e2e- 前缀 slug，验证完删除并确认 0 残留
- CDP Chrome :18800 可用（run-e2e-tests.sh 自动注入 CDP_ENDPOINT）
- 页面路由 `/:slug`；API `/api/v1/entries`
- 死选择器与存活替代（P1 实证）：.mermaid-content→.diagram-viewer/.diagram-code；.mermaid-action-btn→.diagram-action-btn.fullscreen-btn；.diagram-modal-overlay→.diagram-modal
- 环境注意：本平台 bash 调用为一次性会话，服务由持久 job 保活；你只做 HTTP 层验证，不要启停服务、不要跑 make debug-stop

## 产出文件字段

frontmatter 已知值：phase: P2 / task_id: TPV0096 / parent: P1-requirements.md / trace_id: TPV0096-P2-20260907 / agent: architect / status: draft；机器字段 candidate_count: 1 / packages: [frontend-v3, docs] / domains: [frontend] / ui_affected: false（不改 UI 组件；但因被测对象是 E2E，仍须声明 gate_commands.P5_e2e）。用 `FILE=<产出路径> python3 /home/kity/oclab/agateon/agate/scripts/agate-md-field-set.py --list` 查看后逐个写入，不要手写 frontmatter。

P2-design.md 正文必备：影响面梳理节（Modify/Not Modify/Risk，先于候选方案）→ 候选方案（1 个 + design_trivial/follows_existing_pattern 简化理由）→ files_to_read（控制 P4 implementer 上下文，只列确实要参考的）→ env_constraints → minimal_validation（实测匿名配对结果或声明纯代码逻辑+理由）→ gate_commands 块（建议含 P3 红灯运行命令、P5 单测命令、P5_e2e 命令 + 各自 timeout_seconds，档位参考：E2E 600s / 单测 120-300s）。
