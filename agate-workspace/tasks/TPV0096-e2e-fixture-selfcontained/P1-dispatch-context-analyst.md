...
为「E2E 红灯 spec 自建 entry 化」产出 P1-requirements.md：质疑需求、识别隐含依赖、产出 BDD 验收条件（Given/When/Then），标注 domains/risk/NEED_CONFIRM。

## 核心事实（P0 已实证，不要重复查证）

- 3 个红灯 spec 与缺失 entry：`frontend-v3/e2e/mermaid.spec.ts`→`test-mermaid-2`、`frontend-v3/e2e/mermaid-check.spec.ts`→`playwright-test`、`frontend-v3/e2e/mermaid-visual.spec.ts`→`e2e-test`（curl :8888/api/v1/entries/{slug} 全 404）
- 根因：entry 手动建于旧 debug DB，从未入 seed-data（git 无删除记录）；`make debug-stop` 清库后消失。失败形态 = Page not found
- 与产品代码无关（stash 基线对照：失败集合与修复前后一致）
- 自建模式现成参照：`frontend-v3/e2e/render-regression.spec.ts`（L39 `createEntry(request, slug, summary, files)` + afterEach 清理队列）、`frontend-v3/e2e/teams-page.spec.ts`（同模式 + clearCookies）
- seed 用户：alice/bob（testpass123），debug :8888 独立数据 /tmp/peekview-debug/

## 约束

- 只改测试代码与文档，不改产品代码；不 bump 版本
- fixture entry 内容需从旧 spec 断言**反推**（旧 entry 无处可查）——特别注意 mermaid-visual 的断言强度（疑似视觉/尺寸断言，P0 疑似像素级对比——若存在，BDD 中明确降级为存在性+尺寸断言并标注理由）
...
- `frontend-v3/e2e/render-regression.spec.ts`（自建模式参照）
- `AGENTS.md`（铁律 1-11 + 测试注意事项）

## 验收基线种子（P0 预埋，需 analyst 细化/质疑/编号）

1. Given 干净 debug（debug-start + debug-seed）When 跑 3 spec（chromium+Mobile）Then 全绿
2. When 连续重跑 2 次 Then 结果稳定且 DB 无残留 entry
3. Then 其余渲染 spec（svg-inline-render / render-regression 的 SVG 用例）不回归
4. Then E2E 编写规范落点完成（「spec 依赖 entry 必须存在（seed）或自建（清理）」写入项目文档——落点位置由 analyst 调研后定：project.md 或 docs/process/）

## 输出

- `agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P1-requirements.md`
- BDD 连续编号；NEED_CONFIRM 单列（无则写 [NO_NEED_CONFIRM]）；domains 预判 [frontend]（纯 e2e/）；risk 预判 low-medium
