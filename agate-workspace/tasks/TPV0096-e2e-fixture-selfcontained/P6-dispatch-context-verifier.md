# P6-dispatch-context-verifier — TPV0096
...
对 P1 的 13 条 BDD 逐条实跑验收：产出 P6-acceptance.md（每条只允许行首 `- PASS BDD-NN:` / `- FAIL BDD-NN:`）+ P6-evidence/（每条 PASS 引用真实证据文件）。本任务 ui_affected: false——不要求 vision 报告，证据以实跑日志/断言记录/查询结果/截图（可选）承载。

## 约束

**执行形态（平台适配，P5 已验证可行）**：需要 debug 服务的验证一律单调用自包含链：
`cd /home/kity/oclab/peekview && make debug-start && make debug-seed && <验证命令> > /tmp/p6-<n>.log 2>&1; rc=$?; timeout 120s make debug-stop > /dev/null 2>&1; exit $rc`（外层 timeout 900s）。串行执行；严禁漏 debug-stop。

**BDD→证据映射（最低要求，可加码）**：
- BDD-1（3 spec 全绿）：重跑 3 spec（P5_e2e 三键同命令），输出转录 P6-evidence/test-output.log（各 spec passed/failed 计数 + 尾行 EXIT_CODE）
- BDD-2（DB 无残留）：任一 spec 跑完后（debug-stop **之前**）对本次 fixture slug 逐条 sqlite3 count 查询（或 GET 404），输出落 P6-evidence/residue-check.log；查询后仍要 debug-stop
- BDD-3（重跑稳定）：3 spec 连续第 2 次重跑 + 二次残留检查 → 追加同日志（标注 run2）
- BDD-4（不与 seed 冲突）：seed 24 slug 清单 vs spec 内 fixture slug 对照（grep 命令+结果）+ 抽验 ≥3 条 seed entry HTTP 200 → P6-evidence/seed-isolation.log
- BDD-5（清理失败显式失败）：代码级断言记录——读改造后 spec 的 afterEach throw 逻辑，把关键代码行（throw 语句 + 容忍集）与推理写入 P6-evidence/cleanup-assertion.md（允许静态证据：断言记录文件）
- BDD-6/7/8（静态三判据）：亲跑 P3 §6 的 grep 命令，输出落 P6-evidence/static-checks.log（负向 0 命中 + 正向对照逐条）
- BDD-9（双 project 全绿）：E2E 运行不带 --project 过滤的计数拆分（chromium / Mobile Chrome 分别 passed）→ 引用 test-output.log 对应节
- BDD-10/11（渲染正确性/交互结束状态）：改造后 spec 的断言即判据（容器>200/svg>100/modal>500/切换可见性）——E2E 全绿 = 断言全过；引用 test-output.log + （可选加码）对 fixture 页面截图存 P6-evidence/screenshots/
- BDD-12（其余渲染 spec 不回归）：干净环境重跑 svg-inline-render + render-regression，与 P3 §5 基线对照（失败必须 ⊆ 基线家族并集 bdd_3/4/5/7/8；svg-inline-render 预期全绿）→ P6-evidence/regression-compare.log（含基线引用）
- BDD-13（规范落盘）：grep docs/process/debug-workflow.md 两条规则文本 → P6-evidence/doc-rules.log

**格式硬规则**：
- 验收 PASS 行的格式规范：行首连字符 + PASS/FAIL + BDD 编号 + 描述 + 证据路径（如 `- PASS BDD-1: … (test-output.log)`）；证据路径相对 P6-evidence/（如 `(test-output.log)`、`(screenshots/bdd-10.png)`）
- 总结行不得用行首 `- PASS`/`- FAIL` 格式
- frontmatter 机器汇总：pass: N / fail: N / ui_affected: false（agate-md-field-set 写入；agent 字段被拒则手写记录）
- 每个证据文件有实质内容（>1KB 或含实际输出），末行 `EXIT_CODE: <n>`（日志类）
- **post-test 残留检查（强制）**：BDD-2/3 的残留查询即本检查——最后一条 PASS 证据记录前确认无残留；发现残留先清理再记 PASS

**流程纪律**：
- 验收记录的是验收时事实：任何 BDD 实跑 FAIL 就写 FAIL，不修复不凑数（修复是回退 P4 的事）
- verdict 前自查≠gate：不声称"验收已通过"
- 严禁 :8080 与 ~/.peekview/；结束写 [PROD_NOT_TOUCHED]
- 环境失败重试上限 2 轮（verification_env 预算），超限如实报告
- 子派发能力：不启用

## 上游关联

- P5 全绿证据（P5-test-results/）：5 键 exit 0；但 P6 须**独立实跑**（验收是用户/行为视角复核，不是转抄 P5）
- P3 §5 基线（BDD-12 对照基准 + 红灯翻转对照）+ P3 §6 判据命令
- 被验收代码 = 当前 HEAD（P6 commit 13594c9f 后无代码改动；P5→P6 间仅 agate-workspace 产出文件变更，审计 7 允许场景）

## 输入文件

- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P1-requirements.md（13 BDD 原文）
- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P3-test-cases.md（§5 基线 + §6 判据命令）
- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P5-test-results/（参照，不转抄）
- /home/kity/oclab/agateon/agate/assets/execution-roles/verifier.md（角色定义）

## 产出文件（路径硬约束）

- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P6-acceptance.md
- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P6-evidence/（非空；文件清单见映射）
- 过程记录追加 P6-progress.md（每条命令执行前先追加一行）

frontmatter 已知值：phase: P6 / task_id: TPV0096 / parent: P5-verification.md → 按 --list 实际要求填（parent 可能要求 P2-design.md，以 --list 为准）/ trace_id: TPV0096-P6-20260907 / agent: verifier / status: draft / pass: N / fail: N / ui_affected: false。

## 门槛（什么算完成）

- 13 条 BDD 逐条有行首 PASS/FAIL 行（总数 ≥13，PASS+FAIL = P1 BDD 数）
- 每条 PASS 带真实存在的证据文件引用；P6-evidence/ 非空且文件有实质内容
- post-test 残留检查完成且无残留
- check-p6-format.py --fix 跑过（自查）+ check-frontmatter exit 0
- [PROD_NOT_TOUCHED]

## 返回给我（重要）
只返回：
1. P6-acceptance.md 路径
2. 一句话摘要（≤30 字：pass/fail 计数）
3. files_modified: [路径]
绝对不要返回文件全文。