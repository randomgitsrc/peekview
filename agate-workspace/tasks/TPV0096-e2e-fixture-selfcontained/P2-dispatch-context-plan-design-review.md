# P2-dispatch-context-plan-design-review — TPV0096
...
独立评审 TPV0096 的 P2 方案设计（`P2-design.md`）：按 plan-design-review 角色评审——本任务**无 UI 改动**（ui_affected: false，纯 e2e 测试与文档），UI 形态未声明（回落布局型默认），UI 相关维度不适用的显式标注不适用、不打分；评审重心放在与本任务形态匹配的维度：

1. **设计完整性 vs P1 基线**：13 条 BDD 是否每条都有设计落点（M1-M4 改动清单与 BDD 映射）；SCOPE+（死选择器/goto/假绿/护栏）是否全部覆盖
2. **影响面梳理质量**：Modify 落点是否具体到文件/小节；Not Modify 8 项是否显式且理由充分（尤其 t022/verify-mermaid 延后决策与 P1 同类扫描一致）；Risk R1-R9 每条是否有缓解措施
3. **gate_commands 合理性**：是否全为真实 Makefile target（对照 Makefile 核实）、裸 P3 键无 P3_xxx、per-key timeout 档位（E2E 600s/单测 300s）是否合理、有无 && 链反模式；P3 红灯命令语义（先红灯后绿灯）是否成立
4. **自包含执行模式**：「单调用内 start+seed+test+stop、严禁漏 debug-stop」是否足以约束 P3/P5/P6 执行；对平台「服务不跨调用驻留」的适配是否有遗漏场景（如 E2E 多次重跑）
5. **候选方案取舍**：slug 命名策略（e2e-<spec>-<case>-<project> 显式枚举）、mermaid-visual 保留自起 chromium 的决策是否与 BDD-9 双遍口径自洽、认证配对（匿名建+匿名删）是否落实到每个 spec
6. **files_to_read**：是否精简且足以支撑 P4 implementer（10 条是否过载/缺失）

## 约束

- 只审不写：不修改 P2-design.md；产出 P2-review.md，Header status: 初始 draft，完成后改 approved / needs-revision / rejected（gate 读 Header，与返回摘要必须一致）
- 结论必须引用锚点（BDD 编号 / P2-design 小节号 / Makefile target 名）
- UI 不适用维度显式标注「维度不适用」并说明依据（ui_affected: false + 无 UI 改动），不虚构打分
- 格式约束：正文避免行首 `- PASS`/`- FAIL` 预判格式
- 纯文档评审：不启动服务、不跑测试；可 grep Makefile / run-e2e-tests.sh / playwright.config.ts 核实 gate_commands 真实性
- 子派发能力：不启用

## 上游关联

- architect 报告：P2-design.md 236 行，check-frontmatter exit 0；minimal_validation confirmed（匿名 POST=201 / DELETE=200 / 复查 404 / 残留 0，HTTP 状态码级证据）；gate_commands 12 项全 Makefile target
- P1：13 BDD（P1-requirements.md）、review approved（P1-review.md）
- 主 Agent 已采纳 SUGGEST-1~4（死选择器并入 / 规范落点 debug-workflow.md / e2e- 前缀 slug / 同型缺陷延后）
- 平台事实：bash 调用不驻留服务（本会话实测两次），「单调用自包含」模式是唯一可靠执行形态

## 输入文件

- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P2-design.md（评审对象）
- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P1-requirements.md（基线对照）
- /home/kity/oclab/agateon/agate/assets/review-roles/plan-design-review.md（角色定义）
- /home/kity/oclab/peekview/Makefile（gate_commands 核实）
- /home/kity/oclab/peekview/scripts/run-e2e-tests.sh（E2E 命令核实）
- /home/kity/oclab/peekview/frontend-v3/playwright.config.ts（project 口径核实）
- AGENTS.md（项目测试约定）

## 产出

- agate-workspace/tasks/TPV0096-e2e-fixture-selfcontained/P2-review.md
- frontmatter：phase: P2 / task_id: TPV0096 / parent: P2-design.md / trace_id: TPV0096-P2-20260907 / agent: plan-design-review / status: draft（终态三选一）。用 `FILE=<产出路径> python3 /home/kity/oclab/agateon/agate/scripts/agate-md-field-set.py --list` 查看后逐个写入，不要手写 frontmatter
