# P7 派发指引 — T085 consistency-reviewer

## 目标

执行 T085 跨文件一致性检查，产出 `P7-consistency.md`。

## 输入文件

1. `docs/tasks/T085-render-regression-fix/P1-requirements.md`（11 BDD）
2. `docs/tasks/T085-render-regression-fix/P2-design.md`（方案）
3. `docs/tasks/T085-render-regression-fix/P4-implementation.md`（实现）
4. `docs/tasks/T085-render-regression-fix/P6-acceptance.md`（验收 11 PASS）
5. `docs/tasks/T085-render-regression-fix/P5-test-results/frontend-unit.md`

## 检查清单

1. DESIGN_GAP 配对（P4 有无声明）
2. SCOPE+ 闭环（P1 有无 [SCOPE_RESOLVED]）
3. 跨文件一致性（P2 packages vs P4 改动 vs P6 BDD 数量）
4. 未决项清零（无残留 [NEED_CONFIRM]/[BLOCKER]）

## 门槛

- P7-consistency.md status: approved/rejected
- 无 [BLOCKER]
- agent ≠ main
