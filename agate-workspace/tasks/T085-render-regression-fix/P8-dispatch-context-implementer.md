# P8 派发指引 — T085 releaser

## 目标

为 T085 做发布准备：产出 `P8-release.md` + 更新 CHANGELOG。**不执行 bump-version / git commit / git tag**（主 Agent 执行）。

## 版本 bump 判定

- **bump_type: patch**（v0.14.0 → v0.14.1）
- 理由：修复 5 个渲染缺陷（bug fix），不改 API 行为，不新增功能
...
- CHANGELOG [0.14.1] 段
- 临时资源清单
- 不执行 git commit/tag/bump-version
