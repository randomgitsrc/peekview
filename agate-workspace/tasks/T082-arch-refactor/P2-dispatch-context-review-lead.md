...
### 目标
汇总两个 P2 评审意见（plan-eng-review + plan-design-review），产出统一的 P2-review.md。这是 P2 评审组长职责。
...
- agent 字段填 review-lead
- P2-review.md 的 Header agent 字段必须是组长角色名（非 main）

### 上游关联
- plan-eng-review 产出 P2-review-eng.md：status=approved，BLOCKER-1 已修复（ParameterValidationError(422)），3 NON-BLOCKER 已处理
- plan-design-review 产出 P2-review-design.md：status=approved，BLOCKER-2/3 已修复（Pinia action 内引用 + props/emit 契约表），6 WARNING 已处理，3 新 WARNING 不阻塞
- 两个评审均为 approved，无 BLOCKER

### 输入文件
- docs/tasks/T082-arch-refactor/P2-review-eng.md（工程评审——必读）
- docs/tasks/T082-arch-refactor/P2-review-design.md（设计评审——必读）
- docs/tasks/T082-arch-refactor/P2-design.md（被评审的方案设计）
</dispatch_guide>
...
<objective_info>
- 两个评审均 status=approved
- eng review agent: plan-eng-review
- design review agent: plan-design-review
- 无 BLOCKER 残留
</objective_info>
