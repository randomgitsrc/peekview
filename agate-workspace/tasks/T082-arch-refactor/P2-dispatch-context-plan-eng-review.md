...
### 目标
工程评审 T082 架构重构的 P2 方案设计，产出 P2-review-eng.md。关注：数据流清晰性、状态机完整性、接口契约明确性、错误边界、测试策略、多方案探索质量、实现就绪度。

### 约束
- 只审不写——不改 P2-design.md，产出评审意见
- 评审结论引用具体方案节/文件/函数锚点
- agent 字段填 plan-eng-review
- 本任务是纯重构（不改行为），重点审"方案是否能保证零回归"
- 后端 4 项改动（DI 统一/去重/错误格式/事务修复）有依赖链，审依赖顺序是否正确

### 上游关联
- architect 产出 P2-design.md：6 项重构方案，每项含候选方案+权衡+选择
- P1-requirements.md：41 条 BDD，risk=high
- 四字段：packages=[backend, frontend], domains=[backend, frontend], ui_affected=false, gate_commands 用 Makefile target
- minimal_validation: not_needed（纯代码逻辑重构）

### 输入文件
- docs/tasks/T082-arch-refactor/P2-design.md（评审对象——必读）
- docs/tasks/T082-arch-refactor/P1-requirements.md（需求基线，判断方案是否覆盖 BDD）
...
<objective_info>
- P2-design.md 含 6 项重构方案设计
- 四字段已验证：packages, domains, ui_affected=false, gate_commands 齐全
- files_to_read 含 29 个文件路径
- minimal_validation: result=not_needed
- 后端依赖链：DI 统一 → 去重 → 错误格式 → 事务修复
- 前端依赖链：store 拆分 → component 拆分
