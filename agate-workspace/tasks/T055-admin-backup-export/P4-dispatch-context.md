...
- **目标**: 实现 admin backup/export/restore CLI 命令
- **关注点**:
  1. SQLite .backup() API 一致性备份实现
  2. restore ID/slug 重映射（merge 模式，多表 FK 链）
  3. 备份文件格式（tar.gz + metadata.json + SHA256）
  4. 文件系统操作与 DB 事务原子性
  5. export JSON/ZIP 格式
  6. dry-run 实现
- **已知约束**: 纯后端 CLI，ui_affected: false，Python stdlib only
- **gate_commands.P5**: cd backend && .venv/bin/python -m pytest tests/ -q --tb=no

## P2 files_to_read 清单
implementer 必须按此清单读取文件，不要在项目里乱翻。
