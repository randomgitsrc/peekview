...
## 目标
对 P4 代码实现做独立评审：代码质量、安全性、与 P2 设计一致性、边界处理。

## 约束
- agent≠main
- 评审结论必须含具体锚点引用（文件名 + 行号 / 设计方案编号）
- P4 评审按 C8 映射：backend domain → review 角色

## 上游关联
- P2-design.md：方案 A（trigger 降级 + text_utils.py + json_each + jieba 分词 + backfill 版本标记）
- P4-implementation.md：实现产出
- P3-test-cases.md：16 测试用例

## 输入文件
1. `docs/tasks/T083-cjk-search-fix/P2-design.md` — 设计方案（对照实现是否一致）
2. `docs/tasks/T083-cjk-search-fix/P4-implementation.md` — 实现产出
3. `backend/peekview/text_utils.py` — 新增 jieba 分词模块
4. `backend/peekview/database.py` — trigger 降级 + search_entries + rebuild_fts_index + backfill_fts_content
5. `backend/peekview/services/entry_service.py` — _update_fts_content + list_entries tag 过滤 + FTS 查询
6. `backend/peekview/main.py` — lifespan 预加载 jieba
7. `backend/pyproject.toml` — jieba 依赖
...
## 门槛
- P4-review.md status: approved 或 rejected
- 含具体锚点引用（文件名 + 行号）
- 覆盖维度：代码质量 / 安全性 / 设计一致性 / 边界处理
