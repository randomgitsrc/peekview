# P1 Dispatch Context — requirements-review（修订后复核）

## 任务目标

复核修订后的 P1-requirements.md（analyst 已按上轮评审意见修订 R1/R2 + 2 措辞建议），确认全部修订落地后批准。

## 上轮评审意见（修订依据）

`docs/tasks/TPV0089-unicode-filename-link-fix/P1-review.md`（needs-revision，2 修订项 + 2 措辞建议）

## 修订内容核对清单

1. **R1**：BDD-8（原 BDD-7）Given 是否已补充 "且映射到 fileId=3"
2. **R2**：是否新增字面 `%` 文件名 BDD（`a%2520b.png` → `a%20b.png` → 命中，decode 恰好一次）——现为 BDD-7
3. **措辞建议 1**：前端段 "raw HTML 引用 decode 恒等变换" 是否已改为区分无 `%` 序列（恒等）/畸形序列（抛异常走回退）
4. **措辞建议 2**：裁剪说明 P7 是否已改为 "单源文件改动"
5. BDD 编号 1-12 是否连续无跳号，frontmatter 机器字段是否未被破坏

## 上游关联

- 输入文件（必读）：
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P1-requirements.md`（修订后被评审对象）
  - `docs/tasks/TPV0089-unicode-filename-link-fix/P1-review.md`（上轮评审意见，核对依据）
...
- 若全部修订落地 → status: approved；仍有遗漏 → status: needs-revision（列出剩余项）
- 产出文件 Header 的 status 字段必须更新为 approved / rejected / needs-revision

## 产出要求

`docs/tasks/TPV0089-unicode-filename-link-fix/P1-review-rev2.md`
...
- 关键标识：修订后 P1-requirements.md 现含 BDD-1~12（12 条）
- 查证结果：上轮评审的 2 修订项 + 2 措辞建议的落地情况需逐一核对（见核对清单）
</objective_info>
