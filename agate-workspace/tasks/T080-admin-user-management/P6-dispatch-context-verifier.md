...
## 目标
逐条验收 24 BDD（PASS/FAIL 二值），产出 P6-acceptance.md + P6-evidence/ + vision-reports/。

## 约束
- 每条 BDD 只允许 PASS 或 FAIL（无中间态）
- 每条通过必须有证据文件引用（截图/response.json/assert.log）
- UI 任务：操作类 BDD 截图互不相同（md5 去重），至少一条通过基于 vision
- 失败数>0 → 回 P4（不直接改代码）
- 严禁碰 :8080/~/.peekview/，用 :8888（make debug）
- [PROD_NOT_TOUCHED]
- [NO_NEED_CONFIRM]（无待确认项）

## 上游关联
- P1-requirements.md：24 BDD（BDD-01..24）
- P5-test-results/：单元测试 + E2E 已通过（27 E2E passed）
- P5 E2E admin.spec.ts 已覆盖 BDD-01/02/06/12/14/15/20/21（UI 类）
- 后端 pytest 覆盖 BDD-03/04/05/07/08/09/10/11/13/16/22/23（API 类）
- CLI 覆盖 BDD-17/18/19/24

## 输入
1. P1-requirements.md（24 BDD）
2. P5-test-results/unit.md + e2e.md
3. P4-implementation.md（实现清单）

## 执行
1. make build-frontend + make debug-start + make debug-seed
2. 跑 E2E admin.spec.ts 收集截图到 P6-evidence/screenshots/（复用 P5 的 E2E，但加截图捕获）
3. 对 24 BDD 逐条判定：
   - UI 类（BDD-01/02/06/12/14/15/20/21）：E2E 截图 + vision 分析
   - API 类（BDD-03/04/05/07/08/09/10/11/13/16/22/23）：pytest 已通过 + curl 验证响应
   - CLI 类（BDD-17/18/19/24）：CLI 命令实跑 + 输出日志
4. 用 playwright-cdp skill 写截图脚本（desktop 1280x800 + mobile 390x844）
5. 用 vision-engine skill 分析截图（quick role）
6. 写 P6-acceptance.md（24 条 PASS/FAIL + 证据引用）
7. make debug-stop

## 产出
- docs/tasks/T080-admin-user-management/P6-acceptance.md
- docs/tasks/T080-admin-user-management/P6-evidence/screenshots/*.png
- docs/tasks/T080-admin-user-management/P6-evidence/*.json/log
- docs/tasks/T080-admin-user-management/vision-reports/*.yaml

Header: phase=P6, type=acceptance, parent=P5-test-results, status=draft, agent=verifier
