...
- **bump 类型**：minor（新增用户可见功能：TreeView 默认全展开 + 超阈值折叠降级——非 bug 修复、非破坏性变更）
- **受影响包**：peekview（VERSIONS.json 唯一版本源）。当前 peekview=0.18.6 → 目标 0.19.0
- **MCP 包**：`@peekview/mcp-server`（0.10.0）不受影响（本任务纯前端，MCP 零改动）——不 bump MCP
- **P7 SUGGEST 采纳**：不补 frontend-v3/scripts/measure-treeview-perf.ts 正式脚本（红线实测已闭环，p6-redline-bench.ts 可复用）

## 执行步骤（releaser，只产出文件）

1. 读 P2-design.md packages 声明（确认受影响范围）
2. 读 CHANGELOG.md 当前 [Unreleased] 区域状态
3. 写 P8-release.md（bump_type=minor / debt_check / 版本变更确认 / CHANGELOG 更新建议 / 临时资源清单）
4. **不修改** VERSIONS.json / CHANGELOG.md（bump-version + CHANGELOG 由主 Agent 执行）——只在 P8-release.md 中写清"版本号变更确认"（期望值 0.18.6 → 0.19.0）与"CHANGELOG 更新确认"（[Unreleased] 增加 TPV0094 条目）

## 约束

1. **releaser 只产出文件**：不执行 git commit / git tag / bump-version / 任何版本变更命令
2. **临时资源清单必须如实写**：本任务执行期间启动的临时服务/进程（debug backend :8888、Chrome CDP）、创建的临时数据（/tmp/peekview-debug/、P6 fixture、/tmp 脚本）、开发安装（若有）——供主 Agent READY 收尾清理
3. 环境隔离：只读 + 产出文件；状态标记 `[PROD_NOT_TOUCHED]`
4. 产出写 `agate-workspace/tasks/TPV0094-treeview-default-expand/P8-release.md`
5. 追加 `P8-progress.md`

## 输入文件

1. `agate-workspace/tasks/TPV0094-treeview-default-expand/P2-design.md`（packages 声明 + gate_commands）
2. `agate-workspace/tasks/TPV0094-treeview-default-expand/P7-consistency.md`（DEVIATION/SUGGEST 处理）
...
路径 + 一句话摘要（bump_type + 目标版本 + debt_check）。
