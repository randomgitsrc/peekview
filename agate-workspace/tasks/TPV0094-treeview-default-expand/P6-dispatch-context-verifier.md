...
按 P6 模式逐条实跑 P1 的 8 条 BDD（Given/When/Then），产出 `P6-acceptance.md`（每条 PASS/FAIL 二值 + 证据引用）+ `P6-evidence/`（证据目录）。**自查≠gate**——不要声称验收已通过。

## BDD 验收要点（8 条）

| BDD | 验收方式 | 证据类型 |
|-----|---------|---------|
| BDD-1 小 JSON 全展开 | Playwright 实跑（t075-json entry）：.tree-node 数=9、aria-expanded=true 计数=2、false 计数=0 | 截图 + 断言日志 |
| BDD-2 小 YAML/XML 全展开 | Playwright 实跑（t075-yaml + t075-xml）：各自全展开无折叠 toggle | 截图 + 断言日志 |
| BDD-3 超红线大 JSON 折叠+提示 | Playwright 实跑（t094-large）：banner 可见 + .tree-node 数 < 10021 | 截图 + 断言日志 |
| BDD-4 大文件手动展开 | Playwright 实跑：点根 toggle → sub_0 可见；点子树 toggle → leaf 可见 | 截图（操作类） |
| BDD-5 切文件重置 | Playwright 实跑（t094-multi）：大切小 → 小文件全展开 | 截图 + 断言日志 |
| BDD-6 toggle 可逆 | Playwright 实跑：折叠→再展开 | 截图 + 断言日志 |
| BDD-7 折叠态搜索 | Playwright 实跑：大文件折叠态搜索 → 计数非零 | 截图 + 断言日志 |
| BDD-8 红线实测 | 写红线实测脚本（临时位置 /tmp 或 P6-evidence/scripts/）跑 5 量级（100/500/1000/2000/5000）全展开耗时，输出证据 | test-output.log + redline-results.json |

## BDD-8 红线实测（P2 §8 redline_protocol 已定稿，按此执行）

1. **脚本**：写红线实测脚本（放 `/tmp/measure-treeview-perf.ts` 或 `P6-evidence/scripts/`——不要放 frontend-v3/scripts/ 项目源码目录，P6 阶段不引入源码变更）
2. **量级**：100 / 500 / 1000 / 2000 / 5000 节点紧凑 JSON entry（经 debug :8888 API 创建，平铺结构：单根 + N-1 叶子）
3. **测量口径**（页内 performance.now()，不跨进程）：
   - 导航 → 等 .tree-view 可见 → 若根已自动展开先折叠 → T0 → 点击根 toggle → 等 .tree-node == N → T1
   - 全展开渲染耗时 = T1 - T0
4. **判定标准**（P2 §8）：
   - 候选阈值 N 满足「全展开渲染耗时 ≤ 500ms」且无白屏/页面无响应（waitForFunction 10s 超时）
   - 阈值取值规则：取满足预算的最大实测量级；5000 超预算 → 降档 2000；2000 超预算 → 取 1000
   - **当前 DEFAULT_EXPAND_THRESHOLD = 2000**：若实测 2000 满足预算且 5000 超预算 → 阈值保持，BDD-8 PASS
   - 若实测显示 5000 也满足预算 → 阈值应更新为 5000（需回 P4 改常量）——**此时报告主 Agent，由主 Agent 决策回退，不要自己改代码**
5. **证据**：各量级耗时表 + 选定阈值 + 判定依据 → P6-evidence/redline-results.json（+ test-output.log）

## 约束

1. **验收环境**：debug backend :8888（make debug-quick 已可复用——P5 时启动的 debug 服务可能仍在；若不在，重新 make debug-quick）+ Chrome CDP :18800
2. **环境隔离**：严禁触碰 :8080 生产与 ~/.peekview/；状态标记 `[PROD_NOT_TOUCHED]`
3. **P6 阶段不引入源码变更**：验收脚本/红线脚本一律放 /tmp 或 P6-evidence/scripts/（不是 frontend-v3/ 下）
4. **P6-evidence/ 证据要求**：每条 PASS 引用的文件必须存在；操作类 BDD 截图互不相同（md5 去重）；截图 >1KB
5. **vision 引用**：UI 类 PASS 行须含 `(vision: vision-reports/xxx.yaml)`——由 vision-analyst 产出（主 Agent 另行派发，verifier 需在 acceptance.md 预留 vision 引用位置，或与主 Agent 协作）
   - 若 vision-reports 尚未产出，可先在 PASS 行引用 screenshots + 在 dispatch 返回时说明，主 Agent 派 vision-analyst 后补充 vision 引用
6. **PASS 行格式**：`- PASS BDD-NN: {描述} ({证据路径})`（行首，PASS 后空格）
7. **总结行**：`**Summary**: 8/8 PASS, 0 FAIL`（不得用 - PASS/- FAIL 开头）
8. 产出写 `agate-workspace/tasks/TPV0094-treeview-default-expand/P6-acceptance.md` + `P6-evidence/`
9. 每读完一个输入文件/完成关键步骤，追加 `P6-progress.md`

## 输入文件

1. `agate-workspace/tasks/TPV0094-treeview-default-expand/P1-requirements.md`（8 条 BDD——验收依据）
2. `agate-workspace/tasks/TPV0094-treeview-default-expand/P2-design.md`（§8 redline_protocol + §4 fixture 结构）
3. `agate-workspace/tasks/TPV0094-treeview-default-expand/P3-test-cases.md`（fixture slug 映射）
4. `agate-workspace/tasks/TPV0094-treeview-default-expand/P5-test-results/`（技术验证结果，可复用避免重复跑）
5. `frontend-v3/e2e/structured-data-viewer.spec.ts`（E2E 用例参考：fixture 创建方式）
6. `AGENTS.md`（铁律）

## 验证手段

- Playwright CDP :18800 实跑（前端 URL 路径是 `/:slug`，不是 `/entries/:slug`——AGENTS.md 铁律 7）
- 断言日志落 P6-evidence/
...
路径 + 一句话摘要（BDD 验收 X/8 通过 + 红线实测阈值判定）。
