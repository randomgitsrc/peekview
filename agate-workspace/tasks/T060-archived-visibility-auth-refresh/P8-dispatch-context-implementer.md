...
<dispatch_guide>
### 目标
产出 P8-release.md：判定 bump 类型、确认 CHANGELOG/version、列出临时资源清单。**不执行 git commit/tag**。

### 约束
- 只产出文件，不执行 bump-version / git commit / git tag
- P2 packages: [backend, frontend-v3, mcp-server] — 三包均需考虑 bump
- 改动本质：后端默认查询行为变更（bug fix）+ 前端刷新逻辑增强 + MCP 参数扩展
- 非破坏性变更，向后兼容
- debug 环境：make debug（:8888, /tmp/peekview-debug/），严禁触碰生产 :8080

### 上游关联
- P2-design.md §4 声明字段：packages, gate_commands
- 实际改动：backend (2 files), frontend (3 files), MCP (2 files)
- CHANGELOG [Unreleased] 已有 T060 条目
- P1-requirements.md: risk_level=medium, 非 breaking change

### 输入文件
- docs/tasks/T060-archived-visibility-auth-refresh/P2-design.md（packages 声明）
- CHANGELOG.md（检查 [Unreleased] 是否已有 T060 条目）
- VERSIONS.json（当前版本）
...
<objective_info>
- 当前版本：VERSIONS.json 中查看
- CHANGELOG [Unreleased] 已有 T060 条目
- 临时资源：可能启动了 debug backend (make debug)，需记录到临时资源清单
</objective_info>
