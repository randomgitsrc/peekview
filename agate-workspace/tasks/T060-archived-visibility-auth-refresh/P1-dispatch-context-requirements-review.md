...
- review 结论必须引用具体 BDD 编号锚点，不可裸 "approved"
- agent 字段必须为 requirements-review（≠main），否则 gate 拦截
- NEED_CONFIRM 项（Admin All tab archived）已由主 Agent 确认选 A（All tab 排除 archived），analyst 的 BDD-A4/A5 已按 A 编写

### 上游关联
- analyst 产出 14 条 BDD（BDD-A1~A7, BDD-B1~B2, BDD-C1~C2, BDD-D1, BDD-M1~M2）
- 8 个隐含需求（2.1~2.8）
- 1 个 NEED_CONFIRM 已确认（Admin All tab 排除 archived，选 A）
- risk_level: medium, domains: backend/frontend/mcp/security

### 输入文件
- docs/tasks/T060-archived-visibility-auth-refresh/P1-requirements.md（被审对象）
- docs/tasks/T060-archived-visibility-auth-refresh/P0-brief.md（任务简报）
- AGENTS.md（项目约定）
</dispatch_guide>
...
<objective_info>
- 环境状态：无运行时依赖，纯文档审查
- 关键标识：14 条 BDD 编号 BDD-A1~M2
- 查证结果：P1-requirements.md 已存在且非空，257 行
</objective_info>
