...
独立评审 `P1-requirements.md`（需求基线），产出 `P1-review.md`：确认 24 条 BDD 可二值判定、隐含需求覆盖充分（尤其安全维度）、无掺入方案设计。**只审不写**。

## 评审输入

1. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P1-requirements.md`（本次评审对象，24 BDD）
2. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P0-brief.md`（任务简报，核对需求一致性）
3. `packages/mcp-server/src/tools/getEntry.ts`（现有 getEntry：只认 slug + 返回元数据文本列表）
4. `packages/mcp-server/src/client.ts`（request 恒带 Bearer userToken——核对隐含需求[安全]凭据隔离判断）
5. `backend/peekview/api/files.py`（raw 端点 L456 附近——确认 ?share= 缺失 + raw_url 自引用格式）
6. `/home/kity/.agate/assets/review-roles/requirements-review.md`（你的角色定义）

## 评审重点（结合本任务特性）

- **BDD 可二值判定**：24 条逐条核对（尤其 BDD-16"软警告"是否可判定、BDD-18"清单+片段"的可验证性、BDD-8"mock 服务器记录请求头"的可实现性）
- **安全维度覆盖**：跨 host 凭据隔离（BDD-8）、分享 token 不泄露（BDD-5/21/22 隐含）、SSRF 双保险（BDD-9/10/11）、超时限流——是否充分；**response 结构校验的"不泄露响应体"是否可测**
- **隐含需求覆盖**：逐维度核对（数据/前端/多端/安全/边界/兼容）——特别注意：多端"get_entry 切换 raw 端点"（隐含 2.3 多端）、file 参数（隐含 2.5）、净化实现源（后端 ?purify= vs MCP 兜底）、URL 形态解析完备性
- **P1 纯净性**：SUGGEST 是否属合法倾向（不构成 P2 方案设计）
- **测试数据环境约束**：P6 需 :8889 第二实例（capability supplementable——评估是否真实可补）；share 创建 API 可用性

## 约束

1. 产出 `P1-review.md` 到 `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/`，Header：
   ---
   phase: P1
   task_id: TPV0092-mcp-get-entry-fetch
   type: review
