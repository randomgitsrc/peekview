...
trace_id: TPV0092-P3-retry2-20260815
status: draft
---

# P3 Dispatch Context — test-designer（修复轮 retry2：e2e 旧契约）

> 修复对象：P4-review 发现的 `packages/mcp-server/tests/e2e/mcp-e2e.test.ts` 旧契约残留
> 上报来源：`P4-review-eng.md` I-2（最重要 action——P6 `make debug-test-mcp` 会失败）

## 修复目标（review I-2）

**问题**：`packages/mcp-server/tests/e2e/mcp-e2e.test.ts:171`（`handler({slug:'zufvwz'})`）与 `:271`（`handler({slug:'nonexistent-entry-e2e-test'})`）仍用旧 `{slug}` 契约。getEntry schema 已改为 `{ref}`——zod `z.object({ref:...})` 默认剥离未知键 → `{}` → ref undefined → schema.parse 抛错 → 返回 isError。该 spec 在 P6 真实后端在线时（itIfReady）必失败。

**修复**：改为 `handler({ref: 'zufvwz'})` / `handler({ref: 'nonexistent-entry-e2e-test'})`（保持测试语义——验证裸 slug 兼容路径，BDD-4）。
- 检查该文件其他 get_entry 调用点是否也有旧契约（L171/271 之外），一并修
- 检查 `packages/mcp-server/tests/e2e/` 下其他文件是否引用旧契约

## 约束

1. **只改测试文件**：`packages/mcp-server/tests/e2e/mcp-e2e.test.ts`（+ e2e 目录下其他旧契约文件）
2. **严禁修改实现文件**
3. 修复后自查：`make test-mcp-unit` 全绿（e2e 测试 itIfReady 在无后端时 skip，不失败；但 schema 变化不应引入语法错误）
4. 环境隔离：vitest 隔离；状态标记 `[PROD_NOT_TOUCHED]`
5. 追加 `P3-progress.md`

## 返回

路径 + 一句话摘要（修复的调用点 + 自查结果）。
