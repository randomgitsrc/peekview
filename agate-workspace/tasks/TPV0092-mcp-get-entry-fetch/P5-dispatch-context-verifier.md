...
执行 P2-design.md §5 声明的 gate_commands（P5 + P5_typecheck），如实记录结果，产出 `P5-test-results/`。
...
1. **P5 双端单测**：`make test-quick`（后端 pytest 全量）+ `make test-mcp-unit`（MCP vitest）
2. **P5_typecheck**：`make typecheck`（vue-tsc）+ `make lint`（注意 ruff 不在 PATH——用 `~/.local/bin/ruff` 或 `cd backend && ~/.local/bin/ruff check peekview/ tests/`）
3. **全量回归**：后端 pytest 全量已含（test-quick = 全量）——确认覆盖 MCP + 后端 + 前端 typecheck
4. 可选：`make debug-test-mcp`（MCP 集成测试，需 debug backend :8888 + PEEKVIEW_API_KEY——P2 把它列为 P6 命令，P5 可不跑；若环境允许可跑一次记录）

## 约束

1. **环境隔离**：pytest/vitest 隔离环境；若跑 debug-test-mcp 必须指向 :8888 隔离；严禁触碰 :8080/~/.peekview/
2. **如实记录**：不掩盖失败；有失败写清失败测试 id + 失败原因（真 bug / 环境问题 / 预存失败）
3. **紧凑输出**：命令加 `2>&1 | tail -30` 或使用工具汇总模式
4. 产出写 `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P5-test-results/unit.md`（+ fail-list.txt 若有）
5. 每读完一个输入文件，把发现追加到 `P5-progress.md`
6. 状态标记：`[PROD_NOT_TOUCHED]` / `[NO_NEED_CONFIRM]`（行首）

## 输入文件

1. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P2-design.md`（§5 gate_commands 唯一真相源）
2. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P1-requirements.md`（26 BDD 范围）
3. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P3-test-cases.md`（测试映射）
4. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P4-implementation.md`（改动文件 + 修复轮）
5. `AGENTS.md`（铁律）

## 验证手段

- 跑 gate_commands（真实执行，非推断）
...
路径 + 一句话摘要（failed=N 其中预存 M，MCP/后端通过情况）。
