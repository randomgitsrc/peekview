...
对 TPV0092 做跨文件一致性审查（P1↔P6↔P2↔P4），产出 `P7-consistency.md`。

## 特别关注点（主 Agent 指定）

1. **DESIGN_GAP 配对**：P4-implementation.md 有 1 条 `[DESIGN_GAP: ...]`（体积断言）+ 1 条 `[SCOPE_GAP: ...]`（旧契约）——两者均已在 P4 记录中标注"闭环（P3 retry1/retry2 修复）"。P7 须：
   - 转抄 DESIGN_GAP + 配 `[DESIGN_GAP_REVIEWED: 已确认]`（P3 修复已闭环）
   - SCOPE_GAP 非 DESIGN_GAP（是 P3 测试遗漏），但 P4 记录有标注——说明处理
2. **SCOPE+ 闭环**：P2 声明无 SCOPE+。但 P6 准备期间主 Agent 新增 2 项基础设施（非 P1/P2 设计范围）：
   - `.gitignore` 加 `!packages/mcp-server/src/lib/`（lib/ 规则误伤 MCP 源码——**必要修复**，否则 entryRef.ts/purify.ts 不被 git 跟踪）
   - Makefile 加 `debug-extra`/`debug-extra-stop`/`debug-extra-status` target（多实例测试基础设施，P6 :8889 需要）+ dev-server.sh 参数化 PORT
   - 判定：这 2 项是否构成 [SCOPE+]/[DEVIATION]？.gitignore 修复是必要（否则 P4 产出缺文件）；make debug-extra 是测试基础设施（非产品行为）。给出判定 + 是否需登记
3. **P1 26 BDD ↔ P6 26 PASS**：逐条内容映射核对（非仅数量）
4. **P2 packages ↔ P4 改动**：packages=[backend, packages/mcp-server]；实际改动含 .gitignore/Makefile（基础设施）——核对一致性
5. **实现路径 ↔ 设计**：P4 实现（purify.py / files.py / entryRef / purify / client / getEntry / publishFiles / types）vs P2 §2 详细设计——逐条核对（含响应体上限 retry1 修复、redirect:manual 采纳）
6. **未决项清零**：P1 无 NEED_CONFIRM（[NO_NEED_CONFIRM]）+ 5 SUGGEST 定稿情况
7. **P6 验收事实**：26/26 PASS + 3 个预存前端 e2e 失败（DEBT0005）不影响 BDD 结论——核对

## 输入文件

1. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P1-requirements.md`（26 BDD + SUGGEST）
2. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P2-design.md`（§2 详细设计 / §5 gate_commands / [SCOPE+] 无）
3. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P3-test-cases.md`（测试映射）
4. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P4-implementation.md`（实现 + retry1 修复 + DESIGN_GAP/SCOPE_GAP 标注）
5. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P5-test-results/`（技术验证）
6. `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/P6-acceptance.md`（26/26 验收）
7. 实际代码：backend/peekview/services/purify.py + api/files.py + packages/mcp-server/src/lib/entryRef.ts + purify.ts + client.ts + tools/getEntry.ts + publishFiles.ts + types.ts
8. `.gitignore` + `Makefile`（基础设施改动）
9. `AGENTS.md`

## 约束

1. 产出 `P7-consistency.md` 到 `agate-workspace/tasks/TPV0092-mcp-get-entry-fetch/`，Header：
   ---
   phase: P7
   task_id: TPV0092-mcp-get-entry-fetch
   type: consistency
