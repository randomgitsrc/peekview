...
trace_id: TPV0091-P6-verifier-20260813
status: draft
---
...
逐条实跑 P1 的 8 条 BDD 验收，产出 `P6-acceptance.md` + `P6-evidence/`（真实证据）。这是用户视角验收（非代码正确性验证——P5 已完成）。

## 环境

- debug :8888 已运行且已加载 P4 新代码（curl 实测 41/43 → 200 + filename*）
- seed 已灌入（unicode-filenames，public，8 文件 id 36-43）
- CDP Chrome :18800 可用（vision 分析用截图）
- 严禁触碰 :8080 生产与 ~/.peekview/

## BDD 清单（P1-requirements.md §3，8 条）

### 图片预览（UI，Playwright 实跑点击 + 截图）
- **BDD-1** 中文图片点击后正常显示（unicode-filenames → 点击「中文图片.png」→ image-content visible + image-error 缺席）
- **BDD-2** 日文图片点击后正常显示（点击「概要図.png」）
- **BDD-3** café.png / report final.png / arch.png 不回归

### 下载端点（pytest/curl）
- **BDD-4** 41/42/43 download → 200 + 响应体与 /content 一致
- **BDD-5** Content-Disposition 含 filename*=UTF-8'' + unquote == 原名（Playwright suggestedFilename 或 curl header 断言）
- **BDD-6** 36/38/39/40/37 download → 200 + header 有效
- **BDD-7** test_security.py 净化用例保持绿（注入字符）

### markdown 内联
- **BDD-8** README 5 图内联渲染正常（走 /content）

## 验收方式

1. **UI BDD-1/2/3**：用 Playwright CDP 实跑（可复用 P5 的 e2e spec 结果，但需产出**独立截图证据**到 P6-evidence/screenshots/，含 vision 分析）
2. **下载 BDD-4/5/6**：curl 断言（状态码 + header + 响应体比对）→ 输出到 P6-evidence/*.log；BDD-5 可用 Playwright suggestedFilename 或 curl + python unquote
3. **BDD-7**：跑 test_security.py 断言文件输出
4. **BDD-8**：Playwright 打开 README full-page → 截图 5 图可见 + vision 分析

## 产出规格

### P6-acceptance.md
- frontmatter：phase/task_id/type/parent/trace_id/status/created/agent + pass/fail/ui_affected
- BDD 逐条：`- PASS BDD-NN: {描述} ({证据路径})` 或 `- FAIL BDD-NN: ...`
- 每条 UI 类 PASS 含 vision 引用：`(vision: vision-reports/bxx.yaml)`
- 总结行用 `**Summary**: N/8 PASS, 0 FAIL`（不能行首 - PASS）

### P6-evidence/
- screenshots/（截图 >1KB，互不相同，md5 去重）
- vision-reports/（vision 分析 yaml，ui_affected 必须至少 1 条 PASS 基于 vision）
- *.log（curl/断言输出）
- result.json / assert.log 等

## 约束

1. **功能验证优先**：每个 PASS 必须有真实证据（截图/断言日志），不许只凑格式
2. UI 任务必须 vision 分析截图（vision-engine skill 或 vision-helper subagent），blocker_count=0 才能 PASS；blocker>0 须追查根因写入验收文件
3. 环境隔离：只读，不改代码；debug :8888 可用
4. 每完成一条 BDD 追加 progress 到 `P6-progress.md`
5. 产出写 `agate-workspace/tasks/TPV0091-unicode-download-header-fix/P6-acceptance.md` + `P6-evidence/`
6. 自查：证据文件非空（截图 >1KB、断言文件含实际输出）
7. 返回时不要声称"验收已通过"——只返回路径 + 摘要

## 输入文件

1. `agate-workspace/tasks/TPV0091-unicode-download-header-fix/P1-requirements.md`（§3 BDD + §8 验证线索）
2. `agate-workspace/tasks/TPV0091-unicode-download-header-fix/P5-test-results/`（已过验证参考）
3. `frontend-v3/e2e/tpv0091-unicode-preview-download.spec.ts`（已有 Playwright 流程可参考）

## 返回

路径 + 一句话摘要（PASS/FAIL 数、证据文件数、vision 结论）。
