...
执行发布准备（只产出文件，**不执行 bump-version / git commit / git tag**——这些由主 Agent 在 gate 验证通过后亲自执行）。

## 版本决策

- 当前版本：peekview 0.18.4 / mcp_server 0.10.0（VERSIONS.json）
- 本次改动：用户可见 bug 修复（中文/日文文件名下载与图片预览 500）→ **bump_type: patch** → peekview **0.18.5**
- MCP server 无改动（P7 确认 MCP 无牵连）→ **不 bump mcp_server**（0.10.0 保持）

## 发布检查命令（P2 packages 逐包）

P2 packages = [backend/peekview/api/files.py, backend/tests, frontend-v3/src/api/client.ts, frontend-v3/src/components/ImageViewer.vue]
- backend：`cd backend && .venv/bin/python -m pytest tests/ -n auto --tb=short`（全量；已知 test_cli_remote 1 个预存失败，非本任务）
- frontend：`make typecheck` + `make lint`（ruff 缺失时 `cd backend && python3 -m ruff check peekview/ tests/`）

## 执行步骤

1. 读 P2-design.md packages 声明，确定需 bump 的包（peekview 仅，MCP 不动）
2. 跑发布检查命令（backend 全量 pytest + frontend typecheck/lint）
3. **更新 CHANGELOG.md**：[Unreleased] 区补 TPV0091 条目（中文/日文文件名下载与图片预览 500 修复），但**不要**把 [Unreleased] 改成 [0.18.5]——版本号变更由主 Agent bump-version 处理；你只需确保 Unreleased 区有条目记录本任务改动（releaser 不执行 bump-version）
   - 注：先看 CHANGELOG 现有 [Unreleased] 内容，若无 TPV0091 条目则补写（遵循现有条目风格）
4. **读 `agate-workspace/debt/tech-debt.md`**（存在），在 P8-release.md 写入 `debt_check:` 字段（none / reviewed）
5. 产出 P8-release.md（含 bump_type、版本号变更确认、CHANGELOG 更新确认、debt_check 字段、临时资源清单）

## 临时资源清单（releaser→主 Agent 交接）

记录本任务启动的临时资源（供主 Agent READY 收尾清理）：
- debug backend :8888（/tmp/peekview-debug/）
- debug 日志 /tmp/peekview-debug.log
- 临时文件（/tmp/tpv0091-*.log、/tmp/env-check/、/tmp/e2e-results/）
- CDP Chrome :18800（chrome-agent.service，系统服务不清理）

## 约束

1. **不执行** bump-version / git commit / git tag / make publish
2. 只改 CHANGELOG.md（如需要补条目）+ 产出 P8-release.md
3. 环境隔离：只读代码；debug :8888 可用可验证（勿停）；严禁触碰 :8080 生产与 ~/.peekview/
4. 每完成一步追加 progress 到 `P8-progress.md`
5. 产出写 `agate-workspace/tasks/TPV0091-unicode-download-header-fix/P8-release.md`

## 输入文件

1. `agate-workspace/tasks/TPV0091-unicode-download-header-fix/P2-design.md`（packages + gate_commands）
2. `agate-workspace/tasks/TPV0091-unicode-download-header-fix/P7-consistency.md`（一致性结论）
...
- 版本号变更确认（0.18.4 → 0.18.5，MCP 0.10.0 不动）
- CHANGELOG 更新确认（Unreleased 区含 TPV0091 条目）
- 临时资源清单
