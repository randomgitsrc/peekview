...
type: test-results
parent: P2-design.md
trace_id: TPV0091-P5-verifier-20260813
status: draft
---
...
执行 P2 固化的 gate_commands.P5 全量验证，产出 `P5-test-results/`（unit.md + e2e.md + fail-list.txt）。

## gate_commands（P2-design.md §3 固化）
...
注意：
- P2 里 e2e spec 名写的是 `t091-` 前缀（P2-review 观察③），**实际文件是 `tpv0091-unicode-preview-download.spec.ts`**——执行时用实际文件名
- `make lint` 里 ruff 不在 venv，若报 `ruff: 未找到命令` 用 `cd backend && python3 -m ruff check peekview/ tests/` 替代

## 执行步骤

1. **后端全量**：`cd backend && .venv/bin/python -m pytest tests/ -n auto --tb=short`（make test-quick 的实质；注意 test_cli_remote.py 在 -n auto 下有已知预存失败——TPV0090 待办，若出现将其登记为预存失败，不是本任务引入）
2. **lint**：`make lint`（ruff 缺失时用 `cd backend && python3 -m ruff check peekview/ tests/`）
3. **typecheck**：`make typecheck`
4. **E2E**：`E2E_SPEC=e2e/tpv0091-unicode-preview-download.spec.ts make debug-test`（debug :8888 已运行，需 seed 数据已在——unicode-filenames entry 已存在）
5. 产出 `P5-test-results/unit.md`（标注 failed 数量 + passed 计数）、`P5-test-results/e2e.md`（Playwright 结果）、`P5-test-results/fail-list.txt`（FAILED 逐行，可空）
6. 预存失败登记：若发现 test_cli_remote.py 或其它既有失败 → 在 unit.md 标注"预存失败：X（与本次改动无关）"，并登记到 `agate-workspace/tasks/TPV0091-unicode-download-header-fix/known-failures.md`（模板：~/.agate/assets/templates/known-failures-template.md）

## 判定规则

- **exit 0 + failed=0**：全通过 → 在 unit.md/e2e.md 标 PASS
- **exit ≠0 或 failed>0**：
  - 预存失败（与本任务无关）→ 登记 known-failures.md，不阻断
  - 真 bug（本任务引入）→ 报告主 Agent 回 P4
- **PROD_TOUCHED**：任何生产环境触达 → 立即写 `[PROD_TOUCHED] {描述}`；未触发写 `[PROD_NOT_TOUCHED]`
- 测试环境隔离：对比测试前后生产库状态（~/.peekview/peekview.db 不应被触碰）——用 `stat -c %Y ~/.peekview/peekview.db` 前后对比即可（只读 stat 不触碰数据）

## 约束

1. 只读验证，不改任何代码
2. E2E 需要 debug :8888 运行（已运行，勿停、勿重启）
3. 每完成一步追加 progress 到 `P5-progress.md`
4. 产出写 `agate-workspace/tasks/TPV0091-unicode-download-header-fix/P5-test-results/`
5. 返回时报告各步 exit code + failed 计数 + 预存失败清单 + [PROD_NOT_TOUCHED]

## 输入文件

1. `agate-workspace/tasks/TPV0091-unicode-download-header-fix/P2-design.md`（§3 gate_commands）
2. `agate-workspace/tasks/TPV0091-unicode-download-header-fix/P3-test-cases.md`（预期）
3. `frontend-v3/e2e/tpv0091-unicode-preview-download.spec.ts`（新 e2e spec）

## 返回

路径 + 摘要（各步 exit code、failed 计数、预存失败清单、PROD_TOUCHED 状态）。
