...
产出 P8-release.md，包含 bump_type、版本号变更确认、CHANGELOG 更新确认、临时资源清单。

## 约束

- releaser 不执行 bump-version / git commit / git tag
- 只产出文件，由主 Agent 亲自执行发布步骤
- P2 packages: ["@peekview/mcp-server"]
- 当前 mcp_server 版本：0.9.3
- 建议 bump_type: minor（加功能 + bug fix，向后兼容）
- 新版本号：0.10.0

## 输入文件

1. `docs/tasks/T070-mcp-docker-deployability/P2-design.md` — packages 声明
2. `docs/tasks/T070-mcp-docker-deployability/P6-acceptance.md` — 验收结果
3. `docs/tasks/T070-mcp-docker-deployability/P7-consistency.md` — 一致性检查
4. `CHANGELOG.md` — 当前 [Unreleased] 内容
5. `VERSIONS.json` — 当前版本号
6. `packages/mcp-server/package.json` — MCP 包版本
...
`docs/tasks/T070-mcp-docker-deployability/P8-release.md`
