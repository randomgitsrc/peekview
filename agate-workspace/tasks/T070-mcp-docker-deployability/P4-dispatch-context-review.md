...
# P4 派发指引 — review（MCP 接口契约）

## 目标

审查 P4 实现的 MCP 接口契约变更：CWD guard 修复、allowed_paths 容错、/health 增强、config list/verify 增强、publish_files 工具描述增强。

## 约束

- 关注 MCP 接口契约：工具输入/输出是否向后兼容、错误信息格式是否一致
- 不关注文档内容（文档由 docs 域评审覆盖）
- 不关注安全语义（安全由 cso 评审覆盖）

## 输入文件

1. `docs/tasks/T070-mcp-docker-deployability/P2-design.md` — 方案设计
2. `docs/tasks/T070-mcp-docker-deployability/P1-requirements.md` — BDD 验收条件
3. `packages/mcp-server/src/tools/publishFiles.ts` — CWD guard 修复
4. `packages/mcp-server/src/config/merge.ts` — allowed_paths 容错
5. `packages/mcp-server/src/server.ts` — /health 增强
6. `packages/mcp-server/src/cli/config.ts` — config list/verify 增强
...
`docs/tasks/T070-mcp-docker-deployability/P4-review-mcp.md`
