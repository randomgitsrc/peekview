...
# P7 派发指引 — architect（一致性检查）

## 目标

双向一致性检查：实现 vs P2 设计，P2 设计 vs 实现。检查 DESIGN_GAP 配对。

## 约束

- 以批判的第三方视角检查，假设 P2 设计可能有错
- 逐项找实现与设计的偏差
- 偏差优先归类为问题而非"可接受的调整"
- P4 的 [DESIGN_GAP:] 必须在 P7 被转抄 + 配对 [DESIGN_GAP_REVIEWED:]

## 输入文件

1. `docs/tasks/T070-mcp-docker-deployability/P2-design.md` — 方案设计
2. `docs/tasks/T070-mcp-docker-deployability/P4-implementation.md` — 实现记录
3. `docs/tasks/T070-mcp-docker-deployability/P6-acceptance.md` — 验收结果
4. `packages/mcp-server/src/tools/publishFiles.ts` — CWD guard 实现
5. `packages/mcp-server/src/config/merge.ts` — allowed_paths 容错实现
6. `packages/mcp-server/src/server.ts` — /health 增强实现
7. `packages/mcp-server/src/cli/config.ts` — config list/verify 增强实现
8. `packages/mcp-server/README.md` — 文档修正
9. `README.md` — Docker 场景指引
...
`docs/tasks/T070-mcp-docker-deployability/P7-consistency.md`
