...
### 目标
产出 P8-release.md——发布准备（bump_type + CHANGELOG + 临时资源清单）

### 约束
- 不执行 git commit/tag/bump-version——这些由主 Agent 亲自执行
- 只产出文件（P8-release.md + 更新 CHANGELOG）
- bump_type 判定：bugfix → patch
- 版本源：VERSIONS.json，当前 peekview=0.9.4, mcp_server=0.9.3
- 这是纯前端 bugfix，只需 bump peekview patch 版本
- CHANGELOG 更新 [Unreleased] → 新版本号

### 上游关联
- P2 packages: frontend-v3/src/views/LandingView.vue（前端 bugfix）
- P5: 934 unit + 10/10 E2E 全绿
- P6: 6/6 BDD PASS

### 输入文件
- docs/tasks/T065-login-state-bug/P2-design.md（packages 声明）
- docs/tasks/T065-login-state-bug/P4-implementation.md（改动文件）
- CHANGELOG.md（当前 [Unreleased] 区域）
- VERSIONS.json（版本源）
</dispatch_guide>
...
<objective_info>
- 当前版本: peekview=0.9.4, mcp_server=0.9.3
- bump_type: patch（bugfix，不改 API 行为）
- 新版本: 0.9.5
- 改动文件: frontend-v3/src/views/LandingView.vue
- 测试: 934 unit passed + 10/10 E2E passed + 6/6 BDD PASS
</objective_info>
