...
### 目标
逐条实跑 P1 的 6 条 BDD，产出 P6-acceptance.md + P6-evidence/

### 约束
- 每条 BDD 只允许 PASS 或 FAIL，不允许"调整/跳过/覆盖"
- 所有 PASS 必须有证据文件（截图 >1KB 或断言日志）
- UI 任务：操作类 BDD 截图必须互不相同（md5 去重）
- BDD 结果格式：行首 `- PASS` 或 `- FAIL`（大写）
- 使用 Playwright CDP 连接（chromium.connectOverCDP('http://127.0.0.1:18800')）
- 截图存到 P6-evidence/screenshots/
- vision 分析由主 Agent 派发 vision-helper（你只负责截图和断言）

### 上游关联
- P1 有 6 条 BDD：BDD-1 到 BDD-6
- P4 实现：LandingView.vue onMounted+nextTick 认证重定向 + Sign in 条件渲染
- P5 测试全绿（934 unit + 10/10 E2E）

### 输入文件
- docs/tasks/T065-login-state-bug/P1-requirements.md（BDD 验收条件）
- docs/tasks/T065-login-state-bug/P4-implementation.md（实现说明）
- docs/tasks/T065-login-state-bug/P5-test-results/（技术验证结果）
</dispatch_guide>
...
- Chrome CDP: http://127.0.0.1:18800
- 6 条 BDD: BDD-1(已认证刷新重定向) BDD-2(匿名Sign in可见) BDD-3(已认证Sign in不可见) BDD-4(已认证可见用户标识/登出) BDD-5(匿名登录后跳转) BDD-6(fetchMe期间正常渲染)
- Playwright 脚本要求：hardTimer + lastStep + try/finally + process.exit(0)
</objective_info>
