...
### 目标
执行 P2 gate_commands.P5 全量测试 + P5_e2e E2E 测试，产出 P5-test-results/

### 约束
- gate_commands.P5: "cd frontend-v3 && npx vitest run"
- gate_commands.P5_e2e: "E2E_SPEC=e2e/landing-auth.spec.ts make debug-test"
- 紧凑输出模式：--reporter=dot 或 tail -N
- 测试结果落盘到 P5-test-results/
- UI 任务必须跑 E2E
- 不要在生产环境跑

### 上游关联
- P4 实现完成：LandingView.vue onMounted+nextTick + Sign in 条件渲染
- P3 测试：4 个红灯测试应变绿

### 输入文件
- docs/tasks/T065-login-state-bug/P2-design.md（gate_commands 来源）
...
- vitest: cd frontend-v3 && npx vitest run
- E2E: E2E_SPEC=e2e/landing-auth.spec.ts make debug-test (需 debug backend 运行)
</objective_info>
