...
### 目标
审查 P2-design.md，产出 P2-review.md（status: approved/rejected，agent≠main）

### 约束
- 必须检查：候选方案 ≥2 + 权衡 + 选择理由 + 四字段齐全 + gate_commands.P5_e2e
- agent 字段必须写 plan-design-review
- P1 domains=frontend, risk=medium → 本评审是 C8 机械映射触发

### 上游关联
- P1 确认 6 条 BDD，根因=beforeEach 时序 + Sign in 无条件渲染
- architect 选定方案 A（watch immediate + Sign in 条件渲染）

### 输入文件
- docs/tasks/T065-login-state-bug/P2-design.md（待审查的方案设计）
- docs/tasks/T065-login-state-bug/P1-requirements.md（需求基线，BDD 对照）
- docs/tasks/T065-login-state-bug/P0-brief.md（风险声明）
</dispatch_guide>
...
<objective_info>
- P2 声明 3 个候选方案，选定方案 A
- P2 gate 需：候选方案 ≥2 + 四字段 + P2-review.md status:approved
</objective_info>
