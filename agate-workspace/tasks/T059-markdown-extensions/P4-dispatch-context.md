...
## 任务上下文
- 目标：按 P2 方案 A 实现 4 项 Markdown 扩展
- P2 选定方案：全局注册 + 全局 CSS + scrollIntoView 拦截
- 实现范围：~100 行代码变更（主要是 CSS），纯前端

## 派发前查证的客观信息
- P2 files_to_read 指定的 7 个文件路径
- P3 测试代码：frontend-v3/src/composables/__tests__/useMarkdown.extensions.spec.ts
- DOMPurify 配置不变（P2 minimal_validation confirmed）
- KaTeX CSS 24KB，全局 import 在 main.ts

## 关注点
- 按 P2 files_to_read 清单读取文件
- 自查≠P5 gate
- 范围外改动走 SCOPE 增补机制
...
> 完成 → commit 产出 + .state.yaml → 读 phase-cards/P5-verification.md
<!-- AGATE_CARD_END -->
