...
## 任务上下文
- 纯前端变更：4 项 Markdown 扩展（KaTeX / Task List / Footnote / Sub-Sup）
- domains: frontend
...
- KaTeX 0.16.47 已在 node_modules（Mermaid 间接依赖）
- DOMPurify 3.x 默认白名单已覆盖大部分扩展输出（P1-progress.md 实测确认）

## 关键决策
- DOMPurify 白名单扩展可能为零（需 P5 实测确认）
- $ 分隔符冲突由插件智能处理（P1-progress.md 实测确认）
- 脚注 SPA 锚点使用原生浏览器滚动（无需额外 JS）
