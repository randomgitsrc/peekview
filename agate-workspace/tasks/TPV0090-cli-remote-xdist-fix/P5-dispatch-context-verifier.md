...
执行 P2 固化的 gate_commands.P5 全量验证，产出 `P5-test-results/`（unit.md + fail-list.txt）。**无 E2E**（ui_affected: false）。

## gate_commands（P2-design.md §4 固化）
...
1. **P5_cli_remote**：`cd backend && .venv/bin/python -m pytest tests/test_cli_remote.py -n auto -q --tb=no` **连续 3 次**（BDD-1 验收核心：修复后必须稳定全绿；若出现失败记录具体次数）
2. **P5_serial**：`cd backend && .venv/bin/python -m pytest tests/test_cli_remote.py -q --tb=no`（BDD-3 单跑回归，23/23）
3. **P5 全量**：`cd backend && .venv/bin/python -m pytest tests/ -n auto --tb=short`（BDD-2，全量套件；本任务修复后 test_cli_remote 应零失败——**注意这是修复后的验证，不应再有预存失败**；若仍有失败判定是否本任务未完全修复）
4. **lint**：`cd backend && python3 -m ruff check tests/test_cli_remote.py`（ruff 用系统 python3）
5. **残留进程检查**（I6）：`pgrep -f "peekview serve --port 1888"` 应为空（无残留 server 进程）
6. 产出 `P5-test-results/unit.md`（标注 failed 数量 + passed 计数 + 各步结果）、`P5-test-results/fail-list.txt`（FAILED 逐行，可空）

## 判定规则

- **exit 0 + failed=0**：全通过 → 在 unit.md 标 PASS
- **exit ≠0 或 failed>0**：
  - 本任务引入 → 报告主 Agent（可能回 P4）
  - 与本任务无关的其它预存失败 → 登记 known-failures.md，不阻断（但 test_cli_remote 本身不应再有失败）
- **PROD_TOUCHED**：任何生产环境触达 → 立即写 `[PROD_TOUCHED] {描述}`；未触发写 `[PROD_NOT_TOUCHED]`
- 测试环境隔离：测试全部走 tmp 临时目录；对比 `stat -c %Y ~/.peekview/peekview.db` 前后一致

## 约束

1. 只读验证，不改任何代码
2. 无 debug :8888 依赖（纯 pytest），不启 debug 服务
3. 每完成一步追加 progress 到 `P5-progress.md`
4. 产出写 `agate-workspace/tasks/TPV0090-cli-remote-xdist-fix/P5-test-results/`
5. 返回时报告各步 exit code + failed 计数 + [PROD_NOT_TOUCHED]

## 输入文件

1. `agate-workspace/tasks/TPV0090-cli-remote-xdist-fix/P2-design.md`（§4 gate_commands）
2. `agate-workspace/tasks/TPV0090-cli-remote-xdist-fix/P1-requirements.md`（4 BDD）
3. `agate-workspace/tasks/TPV0090-cli-remote-xdist-fix/P3-test-cases.md`（预期）

## 返回

路径 + 摘要（各步 exit code、failed 计数、残留进程检查、[PROD_NOT_TOUCHED]）。
