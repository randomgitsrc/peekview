...
逐条实跑 P1 的 4 条 BDD 验收，产出 `P6-acceptance.md` + `P6-evidence/`（真实证据）。**无 UI**（ui_affected: false）→ 不需要 vision/截图，证据用命令输出日志。

## 环境

- 纯 pytest 验证（backend/.venv），无 debug :8888 依赖
- 严禁触碰 :8080 生产与 ~/.peekview/

## BDD 清单（P1-requirements.md §3，4 条）

- **BDD-1**：`pytest tests/test_cli_remote.py -n auto` 连续 3 次零失败（23 用例全过）
- **BDD-2**：`make test-quick`（`pytest tests/ -n auto`）全绿，test_cli_remote 零失败
- **BDD-3**：单跑（无 xdist）`pytest tests/test_cli_remote.py -q` 17 用例全过（不回归）——注意修复后是 23 用例（17 既有 + 6 新），BDD-3 断言「与修复前行为一致」= 17 既有全过 + 无新增跳过/error
- **BDD-4**：server 子进程启动阶段死亡时 fixture 快速失败且报错含诊断（P3 的 TC-B4a/B4d 测试已覆盖——fixture 级测试实测）

## 验收方式

1. **BDD-1**：跑 `-n auto` 连续 3 次（或 4 次更稳）→ 每次 0 failed → 输出到 P6-evidence/bdd1-xdist-consecutive.log
2. **BDD-2**：跑全量 `pytest tests/ -n auto` → 0 failed → P6-evidence/bdd2-full-suite.log
3. **BDD-3**：跑单跑 `pytest tests/test_cli_remote.py -q` → 23 passed 0 failed（17 既有全过）→ P6-evidence/bdd3-serial.log
4. **BDD-4**：跑 fixture 级测试（TC-B4a/B4d）确认死亡检测 + 诊断信息 → P6-evidence/bdd4-fixture-death.log（可单独跑这两个用例或引用 P3/P5 已跑结果，但需本阶段实跑留证）
...
- 总结行用 `**Summary**: N/4 PASS, 0 FAIL`（不能行首 - PASS）
- ui_affected: false（无 UI 任务）

### P6-evidence/
- *.log（命令实跑输出，含实际内容）
- 不需 screenshots/vision-reports（非 UI 任务）

## 约束

1. **功能验证优先**：每个 PASS 必须有真实证据（命令输出日志），不许只凑格式
2. 环境隔离：只读，不改代码
3. 每完成一条 BDD 追加 progress 到 `P6-progress.md`
4. 产出写 `agate-workspace/tasks/TPV0090-cli-remote-xdist-fix/P6-acceptance.md` + `P6-evidence/`
5. 自查：证据文件非空（日志含实际输出）
6. 返回时不要声称"验收已通过"——只返回路径 + 摘要

## 输入文件

1. `agate-workspace/tasks/TPV0090-cli-remote-xdist-fix/P1-requirements.md`（§3 BDD）
2. `agate-workspace/tasks/TPV0090-cli-remote-xdist-fix/P5-test-results/`（已过验证参考）
3. `backend/tests/test_cli_remote.py`（fixture 级测试 TC-B4a/B4d）

## 返回

路径 + 一句话摘要（PASS/FAIL 数、证据文件数、[PROD_NOT_TOUCHED]）。
