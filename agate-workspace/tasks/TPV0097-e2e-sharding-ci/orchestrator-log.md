...
- EVIDENCE: 2026-09-14 本轮实测定量（未改任何代码，全部为预存失败）：`E2E_SPEC=e2e/debug-server.spec.ts make debug-test` → 18 failed / 34 passed（9 用例 × chromium+Mobile Chrome，CDP 模式，耗时 4.0m）；`frontend-v3/src/router.ts` 无 `/entries` 路径（仅 `/:slug`）→ DEBT0011 的 `goto('/entries/...')` 必 404；`make debug-seed` 后 DB 真值 entries=20（seed-data 24 条）、teams=2、team_members=1（应 ≥3）、日志 3× HTTP 422
