# P6 派发指引 — T084 verifier

## 目标

逐条验收 P1 的 14 条 BDD，产出 `P6-acceptance.md` + `P6-evidence/`。

## BDD 清单（14 条）

| BDD | 描述 | 验收方式 |
|-----|------|----------|
| BDD-01 | MarkdownViewer 内容超出视口时由 content-area 滚动 | Playwright + DOM 检查 |
| BDD-02 | CodeViewer 内容超出视口时由 content-area 滚动 | Playwright + DOM 检查 |
| BDD-03 | CodeViewer 保留横向滚动 | Playwright + DOM 检查 |
| BDD-04 | 移动端向下滚动隐藏 meta-tags-bar | Playwright + 截图 |
| BDD-05 | 移动端向上滚动恢复 meta-tags-bar | Playwright + 截图 |
| BDD-06 | 桌面端 meta-tags-bar 不在 DOM 中 | Playwright + DOM 检查 |
| BDD-07 | TOC 锚点跳转到正确位置 | Playwright + DOM 检查 |
| BDD-08 | 移动端 markdown-body paddingTop=0px | Playwright + getComputedStyle |
| BDD-09 | HtmlViewer iframe 撑满 content-area | Playwright + DOM 检查 |
| BDD-10 | ImageViewer 图片正确显示 | Playwright + 截图 |
| BDD-11 | vitest 全部通过 | 测试日志 |
| BDD-12 | typecheck 零错误 | 测试日志 |
| BDD-13 | build 成功 | 测试日志 |
| BDD-14 | DESIGN.md 包含 Scroll Architecture | 文件检查 |
...
- 查询类 BDD 可不截图但须有断言记录
- debug backend 在 :8888 运行（当前已启动）
- [NO_NEED_CONFIRM]

## 上游关联

- P1-requirements.md：14 条 BDD（含 [SCOPE+] 修订的 BDD-08）
- P5-test-results/：技术验证结果

## 输入文件

1. `docs/tasks/T084-detail-scroll-architecture/P1-requirements.md` — BDD 验收条件
2. `docs/tasks/T084-detail-scroll-architecture/P5-test-results/` — 技术验证结果
3. `docs/tasks/T084-detail-scroll-architecture/P2-design.md` — 方案设计

## 客观查证信息

- debug :8888 可用
- make debug-seed 已灌入测试数据（12 entry，含 markdown/code/html/image 格式）
- vitest: 1128 passed + 1 flaky（预存）
- typecheck: 0 errors
- build: success
- E2E A-BDD-3/4/5: 6/6 passed

## 门槛

- P6-acceptance.md：14 条 BDD 逐条 PASS/FAIL
- P6-evidence/：非空，每个文件含实质内容
- 无 FAIL（FAIL > 0 → 回 P4）
- [NO_NEED_CONFIRM]
