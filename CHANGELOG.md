...
### 升级说明（v0.21.0 → v0.22.0 / MCP 0.11.0 → 0.12.0）

**升级顺序：先升后端（peekview 0.22.0），再升 MCP（0.12.0）**，不要颠倒。

- 后端 0.22.0 是安全升级：数据迁移幂等（`CREATE TABLE IF NOT EXISTS` + 列存在检查），旧库（无 teams 表、entries 无 team_id）升级启动成功、存量数据完好；CLI/前端跟随服务端版本，旧命令与旧 API 调用不变
- MCP 0.12.0 向后兼容（新增 `list_teams` 工具 + 可选 `team_id` 参数），但 `list_teams` 依赖后端 `/api/v1/teams` 端点——**若 MCP 先升而后端仍为 0.21，`list_teams` 会 404**
- ⚠️ **窗口期风险**：0.21 后端 + MCP 0.12 时，agent 传 `team_id` 会被 Pydantic 静默忽略（0.21 的 `CreateEntryRequest` 无 `extra=forbid`），内容按默认 `is_public=true` 发布——**团队内容意外变 public**。两端无法同时升级时，MCP 0.12 配 0.21 后端的窗口期内不要让 agent 传 `team_id`
...
- 结构数据查看器 TreeView 默认全展开：节点总数 ≤ 2000 时自动展开所有含子节点路径，小/中 JSON、YAML、XML 打开即全部可见，无需逐层点开；超阈值自动折叠根并显示折叠提示 banner（`data-testid="tree-collapse-banner"`，文案「内容较大，已折叠部分」），大文件首屏渲染防撑爆 DOM。阈值经红线实测（100/500/1000/2000/5000 节点，500ms 预算：2000 节点 297ms 达标、5000 节点 788ms 超预算）定为 2000，导出常量 `DEFAULT_EXPAND_THRESHOLD` 便于后续调整 (TPV0094)
...
> ⚠️ 本版本未实际发布到 PyPI（发布流程在 `make bump-version` 之后中断，`make publish`/`git push` 未执行；根因排查见 v0.18.2 修复记录），已被 v0.18.2 覆盖取代。若需要这些改动，请安装 v0.18.2 或更新版本。

### 变更

- 移动端详情页顶部的标签/信息条改为随内容一起滚动展示，不再有滚动时的跳变 (T090)
- 移动端详情页底部操作栏改为固定悬浮在屏幕底部，并适配全面屏安全区，滚动内容不再被遮挡 (T090)
- 移动端 Markdown 正文左右留白大幅缩减，屏幕空间利用更充分 (T090)

## [0.18.0] - 2026-08-07

### 新增

- `/settings` 新增"用户管理"tab（管理员可见）：整合原 `/admin` 独立页面的用户列表分页/禁用启用/角色升降级/重置密码/删除全部功能 (T086)
- UserMenu 新增管理员可发现入口：无需手敲 URL 即可到达用户管理 tab (T086)

### 变更

- **破坏性**：`/admin` 独立路由已删除，不再重定向，直接落 404 页面（原路由无 UI 入口，仅限手敲 URL 访问的用户受影响）(T086)
...
- **playwright-cdp / vision-engine 全局 skills**：从项目 symlink 改为全局独立目录，任何会话可直接调用
...
- **vision-engine 全局独立**：从项目 symlink 改为全局目录，playwright-cdp 同步独立
