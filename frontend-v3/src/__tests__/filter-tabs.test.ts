...
describe('EntryCard', () => {
  it('TC-B13: displays @username format', () => {
  ...
  it('TC-B16: username=null does not show username', () => {
  ...
  it('TC-B15: @username is a router-link', () => {
    ...
    expect(link.attributes('to')).toBe('/users/alice')
  })

  it('TC-C08: expired-but-active shows expired badge', () => {
    const entry = makeEntry({ status: 'active', expiresAt: new Date(Date.now() - 86400000).toISOString() })
    const wrapper = mount(EntryCard, { props: { entry, isOwner: true }, global: { stubs: { BaseTag: true, BaseBadge: true } } })
    const badge = wrapper.findComponent({ name: 'BaseBadge' })
    expect(badge.props('status')).toBe('expired')
  })

  it('TC-D03: time element has title attribute for absolute time', () => {
