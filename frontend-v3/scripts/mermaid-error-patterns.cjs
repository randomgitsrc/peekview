 ...
 * 
 * Usage: node scripts/mermaid-error-patterns.cjs
 * Output: TSV — STATUS\tNAME\tERROR
 * 
 * DOMPurify must be set up BEFORE requiring mermaid.
 */
...
const tests = [
  ...
  ['sankey-beta (correct)', 'sankey-beta\n  A,B,10\n  B,C,5'],
  ['Sankey-beta (wrong case)', 'Sankey-beta\n  A,B,10\n  B,C,5'],
  ['quadrantChart (correct)', 'quadrantChart\n  title Test\n  x-axis Low --> High\n  y-axis Low --> High\n  quadrant-1 Q1\n  quadrant-2 Q2\n  quadrant-3 Q3\n  quadrant-4 Q4\n  A: [0.5, 0.5]'],
  ['QuadrantChart (wrong case)', 'QuadrantChart\n  title Test\n  x-axis Low --> High\n  y-axis Low --> High\n  quadrant-1 Q1\n  quadrant-2 Q2\n  quadrant-3 Q3\n  quadrant-4 Q4\n  A: [0.5, 0.5]'],
  ['xychart-beta (correct)', 'xychart-beta\n  title "Test"\n  x-axis [1,2,3]\n  y-axis "V" 0 --> 10\n  bar [5,8,3]'],
  ['Xychart-beta (wrong case)', 'Xychart-beta\n  title "Test"\n  x-axis [1,2,3]\n  y-axis "V" 0 --> 10\n  bar [5,8,3]'],
  ['block-beta (correct)', 'block-beta\n  columns 3\n  A:1\n  B:2'],
  ['Block-beta (wrong case)', 'Block-beta\n  columns 3\n  A:1\n  B:2'],
  ...
  // === FULL-WIDTH CHARS IN SYNTAX POSITIONS (outside []) ===
  ...
  // === FULL-WIDTH CHARS IN LABELS (inside []) ===
  ['full-width parens （）in node label', 'graph TD\n  A[数据（中文）] --> B'],
  ['full-width brackets 【】in node label', 'graph TD\n  A[数据【中文】] --> B'],
  ['full-width comma ，in label', 'graph TD\n  A[数据，信息] --> B'],
  ['full-width period 。in label', 'graph TD\n  A[数据。信息] --> B'],
  ['full-width semicolon ；in label', 'graph TD\n  A[数据；信息] --> B'],
  ['full-width exclamation ！in label', 'graph TD\n  A[注意！] --> B'],
  ['full-width question ？in label', 'graph TD\n  A[问题？] --> B'],
  ['full-width tilde ～in label', 'graph TD\n  A[范围～50万] --> B'],
  ['full-width equals ＝in label', 'graph TD\n  A[条件＝true] --> B'],
  ['full-width plus ＋in label', 'graph TD\n  A[加＋减] --> B'],
  ['full-width minus —in label', 'graph TD\n  A[从—到] --> B'],
  ['full-width slash ／in label', 'graph TD\n  A[路径／文件] --> B'],
  ['full-width hash ＃in label', 'graph TD\n  A[＃标签] --> B'],
  ['full-width percent ％in label', 'graph TD\n  A[％比] --> B'],
  ['full-width asterisk ＊in label', 'graph TD\n  A[＊通配] --> B'],
  ['full-width pipe ｜in label', 'graph TD\n  A[｜管道] --> B'],
  ['full-width less ＜ greater ＞in label', 'graph TD\n  A[＜条件＞] --> B'],
  ['full-width underscore ＿in label', 'graph TD\n  A[变量＿名] --> B'],
  ['full-width at ＠in label', 'graph TD\n  A[＠用户] --> B'],
  ['full-width ampersand ＆in label', 'graph TD\n  A[＆合并] --> B'],
  ['full-width backslash ＼in label', 'graph TD\n  A[路径＼文件] --> B'],
  ['full-width space (ideographic) in label', 'graph TD\n  A[数据　信息] --> B'],
  ['full-width angle brackets 《》in label', 'graph TD\n  A[《标题》] --> B'],
  ['full-width double quotes \u201c\u201d in label', 'graph TD\n  A[\u201c中文\u201d] --> B'],
  ['full-width single quotes \u2018\u2019 in label', "graph TD\n  A[\u2018中文\u2019] --> B"],
  ...
  ['-x> (invalid arrow combo)', 'graph TD\n  A -x> B'],
  ...
  ['sequenceDiagram valid (user error #2 ref)', 'sequenceDiagram\n  participant main\n  participant GitHub\n  main->>GitHub: commit\n  GitHub-->>GitHub: CI start'],
